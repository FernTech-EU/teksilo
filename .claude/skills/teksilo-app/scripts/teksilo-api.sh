#!/usr/bin/env bash
# teksilo-api.sh — the EXACT public API of a Teksilo widget for the version THIS app
# pins. Works from any app depending on `teksilo` / `teksilo-widgets`, whether the dep
# is a crates.io, git, or path dependency — including external adopters with NO checkout.
#
#   teksilo-api.sh <Widget> [<Widget> ...]   # e.g. Button HStack Dialog
#   teksilo-api.sh --list                    # every widget
#   teksilo-api.sh -f json ComboBox          # JSON for tooling
#   teksilo-api.sh --crate data ListModel    # other crates: widgets|data|settings|scene
#                                            #   (only those present in the dep graph)
#
# How it works
#   1. `cargo metadata` resolves where THIS app's teksilo-* crates live on disk.
#   2. If they resolve inside a Teksilo checkout that ships tools/, run the framework's
...
echo "teksilo-api: resolving dependency graph (cargo metadata)…" >&2
META_ERR=$(mktemp "${TMPDIR:-/tmp}/teksilo-api.err.XXXXXX")
...
# name<TAB>dir for every teksilo-* package resolved in this app's graph.
if ! PKGS=$(printf '%s' "$META" | python3 -c '
import json, os, sys
m = json.load(sys.stdin)
for p in m["packages"]:
    if p["name"].startswith("teksilo-"):
...
WIDGETS_DIR=$(printf '%s\n' "$PKGS" | awk -F'\t' '$1=="teksilo-widgets"{print $2; exit}')
if [ -z "${WIDGETS_DIR:-}" ]; then
  if [ -n "$PKGS" ]; then
    echo "error: 'teksilo' is in the dependency tree but the 'widgets' feature is disabled." >&2
    echo "       Enable it:  teksilo = { version = \"…\", features = [\"widgets\"] }" >&2
  else
    echo "error: 'teksilo' is not in this app's dependency tree (add it, or run from the app dir)." >&2
  fi
  exit 1
fi
...
if [ ! -d "$WIDGETS_DIR/src" ]; then
  echo "teksilo-api: fetching crate sources (cargo fetch)…" >&2
  cargo fetch >/dev/null 2>&1 || true
fi
if [ ! -d "$WIDGETS_DIR/src" ]; then
  echo "error: teksilo-widgets source not found at '$WIDGETS_DIR/src' (try 'cargo fetch')." >&2
  exit 1
fi
...
if [ -n "${REPO_ROOT:-}" ] && [ -f "$TOOL" ] && [ -d "$REPO_ROOT/crates/teksilo-widgets" ]; then
...
STAGE=$(mktemp -d "${TMPDIR:-/tmp}/teksilo-api.XXXXXX")
...
# Symlink every resolved teksilo-* crate source into the fake monorepo layout, so
