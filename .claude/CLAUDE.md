...
- Debug inspector (fern-inspector: in-app introspection panel, debug builds only, gated by `cfg(debug_assertions)`; F12 toggles a bottom panel with 9 tabs (Tree, Properties, Accessibility, Theme, Locale, Focus, Shortcuts, Overlays, Models); bounds-overlay visualization (Off/Selection/All), picker tool, theme + locale switching, persistence via `__fern_inspector.*` settings keys when `SettingsStore` is wired. Apps opt in with `FernAppBuilder.install_inspector_in_debug()` (no-op in release). See `docs/inspector.md`. Data models opt into the Models tab via `ListModel::debug_named("…")`.)
