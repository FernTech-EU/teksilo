...
cargo run -p drag-and-drop                      # Drag-and-drop showcase (in-view reorder + cross-widget export: Library ListView .exportable(Copy) → Playlist .accept_foreign_rows → Trash DropTarget)
...
**Layout primitives** (in [crates/bastyde-widgets/src/primitives/](crates/bastyde-widgets/src/primitives/)): `HStack`, `VStack`, `ZStack`, `Grid`, `Wrap`, `Padding`, `Spacer`, `Center`, `Expand`, `Shrinkable`, `FixedSize`, `MinSize`, `MaxSize`, `AspectRatio`, `Switcher`, `Divider`, `DeadZone`, `IconWidget`, `ImageWidget`, `ImageMask`, `MasonryLayout`, `FormLayout`, `ValidationStrip`, `TextInputField`
