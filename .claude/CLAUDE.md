...
bastyde-webview         Embeddable `WebView` widget. The one widget that can't render into the
...
**Layout primitives** (in [crates/bastyde-widgets/src/primitives/](crates/bastyde-widgets/src/primitives/)): `HStack`, `VStack`, `ZStack`, `Grid`, `Wrap`, `Padding`, `Spacer`, `Center`, `Expand`, `Shrinkable`, `FixedSize`, `MinSize`, `MaxSize`, `AspectRatio`, `Switcher`, `Divider`, `DeadZone`, `IconWidget`, `ImageWidget`, `ImageMask`, `MasonryLayout`, `ColumnFlow`, `FormLayout`, `ValidationStrip`, `TextInputField`
...
- **Drop-in control:** [`TextScaleControl`](crates/bastyde-widgets/src/text_scale_control.rs) — a specialized `SpinBox` (80 %–200 %, step 10 %). `ctx.add(TextScaleControl::new(ctx.settings().signal_for(&TEXT_SCALE_KEY)).label(tr!(text_size())))` both persists and applies app-wide on edit. `TEXT_SCALE_KEY` (`"accessibility.text_scale"`) is re-exported in `bastyde::prelude`.
...
When a window loses OS focus, its widgets change appearance — the modern desktop convention (SwiftUI `@Environment(\.appearsActive)`, Jetpack Compose `LocalWindowInfo.isWindowFocused`, GTK4 `:backdrop`, Qt `QPalette::Inactive`). Bastyde does this automatically for stock widgets and exposes an opt-in hook for custom content. A window is **active** when `focused AND not occluded`; this is **per-window** (no app-wide fan-out, unlike theme / locale / text-scale).

- **The primitive (mirrors the global text-scale plumbing):** `WidgetTree::window_active_signal() -> Signal<bool>` (starts `true` — a window must not be born inactive), written by `set_window_active` ([crates/bastyde-core/src/widget_tree.rs](crates/bastyde-core/src/widget_tree.rs)). Surfaced as `ctx.window_active()` / `ctx.window_active_signal()` on `BuildContext`, `ctx.window_active: bool` on `PaintContext`, and `ctx.window_active()` on `EventContext`. App layer feeds it from `WindowEvent::Focused`/`Occluded` ([crates/bastyde-app/src/app.rs](crates/bastyde-app/src/app.rs), already computing `focused && !occluded`).
...
- **Opt-in for custom content:** `.dim_when_inactive(factor)` (and `dim_when_inactive_default()`, 70 %) on the `WidgetBuilder` trait wraps a subtree in `DimWhenInactive` ([crates/bastyde-core/src/dim_when_inactive.rs](crates/bastyde-core/src/dim_when_inactive.rs)), driving a node-level opacity scope from `window_active_signal`. Layout-/a11y-transparent; opacity snaps (reduced-motion-correct). Re-exported in `bastyde::prelude` as `DimWhenInactive`.
...
- ColumnFlow (responsive reflowing columns): [crates/bastyde-widgets/src/primitives/column_flow.rs](crates/bastyde-widgets/src/primitives/column_flow.rs) (widget + `balance_columns` + the private `ColumnFlowItem` a11y wrapper + inline tests). Shared column-count solver: [crates/bastyde-widgets/src/common/column_geometry.rs](crates/bastyde-widgets/src/common/column_geometry.rs) (`ColumnGeometry` / `WidthPolicy`), reached from GridView via the `GridSizing → WidthPolicy` door at [crates/bastyde-widgets/src/grid_view/layout/columns.rs](crates/bastyde-widgets/src/grid_view/layout/columns.rs). Reference: [docs/layout-primitives.md §5.4](docs/layout-primitives.md)
...
- TreeTableView: [crates/bastyde-widgets/src/tree_table_view.rs](crates/bastyde-widgets/src/tree_table_view.rs) (reuses table_view's column/header/keyboard modules; adds `TreeNavigator` + `TwistArrow`). Demo: [examples/tree_table_view/src/main.rs](examples/tree_table_view/src/main.rs)
...
- WebView: [crates/bastyde-webview/src/](crates/bastyde-webview/src/) — `backend.rs` (`WebViewBackend`/`WebViewHandle` traits, `WebViewId`/`WebViewAttributes`/`WebViewEvent`/`WebViewEventPayload`, `WebViewRegistry` app-state service, `MemoryWebViewBackend`/`NoopWebViewBackend`, shared `post_event` + `js_string`), `lib.rs` (`WebView` widget + `is_wayland()`), `styles/recipe_web_view_style.rs` (default overlay), `wry_backend.rs` (feature `wry-backend`), `servo_backend.rs` (feature `servo-backend`), `tests/basic_lifecycle.rs`. Style trait: [crates/bastyde-core/src/styles/web_view_style.rs](crates/bastyde-core/src/styles/web_view_style.rs). Framework hooks (bastyde-core): `WidgetNode::activation_signal` + `BuildContext::activation_signal` + `WidgetTree::flush_activation_signals` (fired from `set_dormant`/`activate`); `BuildContext::run_after_mount` + `WidgetTree::run_mount_actions` (post-mount EventContext). App wiring: `try_route_web_view_payload` + `process_pending_mount_actions` + `run_in_window` helper in [crates/bastyde-app/src/app.rs](crates/bastyde-app/src/app.rs); per-window purge in `window_manager.rs`'s `close_window`. Install ext: [crates/bastyde/src/webview_install.rs](crates/bastyde/src/webview_install.rs) (`BastydeAppBuilderWebViewExt`). Reference: [docs/web-view.md](docs/web-view.md). Demo: [examples/web_view_demo/src/main.rs](examples/web_view_demo/src/main.rs).
