...
cargo run -p widget-catalog -- --theme macos-dark    # …in a preset's true chrome (also macos-light, fluent-{light,dark}, material3-{light,dark}, intui-{light,dark}); the title-bar ThemeSwitcher re-tints live but keeps the shapes the tree was built with
...
cargo run -p multi-window                      # Multi-window demo
...
cargo run -p tool-box                          # Tool box widget demo
...
cargo run -p scene-showcase                     # Scene viewport: pan/zoom + heavyweight+lightweight tier mix
cargo run -p scene-corkboard                    # Scene-based story corkboard: `SceneCard` notes with real editors — drag the title strip to move a beat, drag its prose to select text, pan a beat off-screen and keep typing (the camera follows the caret), "Fit to text" hands every beat's height to its words
...
cargo run -p scene-ink                          # Freehand ink on a scene page: every pen sample via `ctx.coalesced()`, a `WetLayer` that repaints alone, a dried stroke in `SceneLayer::Interleaved` between two notes. See docs/ink.md
...
cargo run -p docking                             # DockingLayout: VS Code-style dockable IDE shell — centre editor + 4 collapsible/splittable/draggable sides, activity rail, per-corner ownership, drag-to-dock, Lock-layout toggle (DockPolicy) + disable-side, export/restore. See docs/widgets/docking.md
...
cargo run -p teksilo-widgets-previewer --release -- --export-docs --density=compact,touch   # the catalog page images (needs a GPU)
...
- The generated **Widget Catalog** pages under `docs/widgets/*.md`, their preview images under `docs/widgets/img/*.png`, and the populated `<!-- BEGIN/END GENERATED WIDGETS -->` region of `docs/SUMMARY.md` **are committed** (so the book + its widget sidebar + its pictures build on any checkout); regenerate with the two commands above. Only `book/` (build output) is gitignored.
- **Catalog page images** come from `cargo run -p teksilo-widgets-previewer -- --export-docs` — a headless batch that renders each widget through the production wgpu renderer into `docs/widgets/img/<slug>.png` (2×, light theme, sized to content). It needs a GPU adapter but no display server, and is **not** part of the docs CI job: the PNGs are committed, so regenerate them locally whenever a widget's appearance changes. `extract_widget_api.py` emits `![… preview](img/<slug>.png)` on a page iff that file exists, so a missing image degrades to a page without a picture. Subjects come from two registries, both keyed by the widget's *source file* (its stem is the page slug): the `WidgetCatalog` impls in [preview_catalog.rs](crates/teksilo-widgets/src/preview_catalog.rs), and the documentation-only one-liners in [doc_snippets.rs](crates/teksilo-widgets/src/doc_snippets.rs) (`doc_snippet!(path, { … })`, optionally `size = (w, h)` for a widget that fills its parent) — a snippet **wins** over a catalog entry for the same file. Flags: `--export-docs=DIR`, `--only=slug,…`, `--dark`, `--scale=N`, `--all-subjects`, `--pages=DIR`, `--density=NAME[,NAME…]`. **Density is a pass**: `compact` is canonical and writes `img/<slug>.png`, every other density is *additive* and writes `img/<slug>-touch.png` / `-comfortable.png` beside it, so a denser run can never move a committed image. `teksilo_preview::PreviewPass` owns that naming rule and `extract_widget_api.py`'s `DENSITY_IMAGE_SUFFIXES` mirrors it — a page grows a `## Density` section iff a suffixed file exists, and the stale-image report knows a suffixed variant is not an orphan. A subject that renders essentially blank is reported and skipped rather than published; `every_doc_snippet_lays_out_without_panic` in the previewer binary's tests keeps the snippets building.
...
teksilo-widgets         ~100 widget modules + 29 layout primitives (Button, ListView, TreeView, TableView,
...
teksilo-scene           Pannable/zoomable scene viewport (Qt QGraphicsScene equivalent). Two-tier content
                     ...
                     **Cards** (`scene_card.rs`): `SceneCard` is the container a heavyweight item
                     ...
                     what is inside it, which is why it is not called `NoteContainer`. The part
                     that cannot be written above the crate is the **gesture regime**, decided
                     structurally by the router's enrolment rules: the header owns its own
                     `on_drag` (so it moves the card), the body's press climbs to the card root's
                     `gesture_dead_zone` and stops (so a drag in an embedded editor selects text
                     and never marquees the page), and the trailing header slot gets a `DeadZone`
                     of its own (the `Accordion` precedent). A header drag previews as a node
                     transform, asks `SceneModel::constrain_move` per sample — the same door the
                     built-in drag uses — and commits ONCE through `apply_transform_delta`; it is
                     **anchored to the press and adds the preview back** (`D = (position − anchor)
                     + previewed`) rather than summing the recogniser's deltas, because the
                     preview moves the node the positions are localised against, so a summed delta
                     reads 40, 0, 40 for three even moves and loses the slop-crossing travel
                     permanently. Its a11y is node-for-node identical to the hand-rolled `Panel`
                     it replaces (the default `Card` surface asks for `Role::GenericContainer` so
                     the walker prunes it), with the container **named**. No `on_commit`: the
                     trigger is `focus_within`, written outside dispatch, so a callback there
                     could not be handed an `EventContext` — observe the mode signal.
                     **Content-driven size** (`SizePolicy::{Fixed,HeightForWidth,Intrinsic}` on the
                     entry, `scene.rs` + `view/layout_impl.rs`): a heavyweight entry can hand one
                     or both axes to its widget, measured in `place_children` (which runs before
                     any child's own layout, so the measurement is a pure query whose answer is
                     what the child is then given). **Only the cards a pass lays out are
                     measured** — the `ListView::auto_item_height` contract: an unrealised card
                     keeps the estimate `add_widget_item` was handed, so a pan still costs the
                     viewport and not the page. The write-back is `ItemChange::MeasuredSizeChanged`
                     — `is_edit() == false`, stamped `ephemeral`, counted out of
                     `structural_version` (so no AccessKit re-walk), and answered by a
                     **relayout** rather than a rebuild; `Scene::set_measured_size` is public for
                     ...
                     each ancestor's own space as it climbs; a card's arena bounds inside a
                     `SceneView` are its scene rect) and the applied shift is reported back in the
                     same space, so an outer `ScrollArea` is re-targeted to where the card will
                     land. Registered even on a non-interactive view (that flag gates camera
                     *input*), and it honours reduced motion, which the public
                     `SceneView::ensure_visible` cannot (no `EventContext`).
...
teksilo-theme-material3 / teksilo-theme-fluent / teksilo-theme-macos
                     Sibling design-language presets (tokens + Tier-3 chrome), reached as
                     `teksilo::prelude::{material3, fluent, macos}::{light, dark}` behind the
                     umbrella features `theme-material3` / `theme-fluent` / `theme-macos`.
                     See **Theming** below.
teksilo-inspector       In-app debug introspection panel (F12), `cfg(debug_assertions)` only.
                     Apps opt in with `install_inspector_in_debug()`. See docs/inspector.md.
teksilo-terminal        Embeddable terminal-emulator widget — see the fuller entry above.
teksilo-automation      GUI-free automation toolkit: serde DTOs, `execute(tree, ops, op, settle)`,
                     the 34-tool catalog, and the `wire` framing/token protocol. Core-only peer,
                     like teksilo-data.
teksilo-automation-mcp  The rmcp MCP server binary driving it (`--headless` / `--attach`).
teksilo-parse           Shared parser for the `teksu!` surface language.
teksilo-fmt / cargo-teksilo-fmt / teksilo-fmt-lsp
                     The `teksu!` formatter: library, `cargo teksilo-fmt` subcommand (a
                     pre-commit gate), and its LSP server.
```
...
One trait for all widgets — leaf, container, composite, hybrid. The core seven, which is all most widgets touch:
...
That block is an **excerpt** — the real trait in [crates/teksilo-core/src/widget.rs](crates/teksilo-core/src/widget.rs) carries ~25 defaulted methods. Read it before assuming a hook doesn't exist; the ones referenced elsewhere in this file are `type_name`, `cacheable_layout`, `as_any` / `as_any_mut` (needed for `ctx.with_widget_mut`), `preserves_children_on_rebuild`, `hit_shape`, `focus_reveal_rect`, `wants_after_paint` / `after_paint`, `wants_post_paint` / `post_paint`, `accessibility_children`, `wants_descendant_redirects` / `a11y_redirect_descendant`, `accessible_title_hint`, `initial_focus_hint`, `context_menu_key_target`, `tooltip_has_content`, `declare_shortcuts`, and `take_handler_set`.
...
`Signal`/`Prop` are the only reactivity vocabulary. The legacy `State<T>` / `DerivedState<T>` / `Reactive<T>` types and their `teksilo-core::state` module have been **removed** — if you find a reference to them anywhere, it is stale.
...
- **The positions the OS batched reach a handler.** `ctx.coalesced()` is the batch folded into the packet being dispatched, oldest first and excluding `pointer_position()` (the newest); each entry keeps its own `EventTime` **and** its own `PointerAxes`, because a digitizer varies pressure inside a batch. Window-logical, like `Scroll::window_position`. Empty for every producer that does not batch and for every dispatch that is not a sample (a timer gesture, a DnD tick, an AT action — written out rather than inherited, so it is a decision). The producer is `TeksiloAppBuilder::pen_batching(PenBatching::Coalesce)`: one dispatch per pen drain instead of one per packet, transitions never folded. A drawing surface fans out over `coalesced()` then `pointer_position()` and is correct under either mode. **Not `on_drag`** — `DragRecognizer` swallows every move inside the slop and reports the *press* position, so the start of a stroke never arrives there. See [docs/ink.md](docs/ink.md).
...
**`described_by` is heard as a description the tree writes.** No AccessKit 0.25 adapter exports the relation, and `accesskit_consumer` never derives a description from it, so the walker's last pass (`widget_tree/accessibility_description_impl.rs`) appends each target's text to the node's `description`, which all three platforms read on focus. Around focus it holds new text back: a node focus is on, or was on in the last update, gains nothing while focus stays or in the update focus leaves it, because Orca speaks a change to the description of the node it holds as focus, and still holds the old one while it hears that update's changes. A text a live region announces as focus arrives is left out of the arrival on Windows only: NVDA keeps the announcement, and Orca cuts it to read the new focus (`default.py:698-705`), so there the arrival is the one voice left (VoiceOver is unverified and treated as Orca). A message that must be heard as it appears needs a live region or `announce`; the description is for coming back to the field.
...
- **Opt-in for custom content:** wrap the subtree in `DimWhenInactive::new().factor(f).child(w)` (default factor 70 %) ([crates/teksilo-core/src/dim_when_inactive.rs](crates/teksilo-core/src/dim_when_inactive.rs)), driving a node-level opacity scope from `window_active_signal`. Layout-/a11y-transparent; opacity snaps (reduced-motion-correct). Re-exported in `teksilo::prelude` as `DimWhenInactive`.
...
Reference: [docs/window-activation.md](docs/window-activation.md). Demo: `cargo run -p multi-window` (two windows; click between them to watch the inactive one hide its caret, mute its selection, dim its panel, flip its status label).
...
**`Ctrl` means ⌘ on macOS.** A shortcut's **declared** default is read as *the platform's primary accelerator*: `KeyStroke::ctrl(Key::F)` resolves to ⌘F on macOS and Ctrl+F elsewhere (Qt's `Qt::CTRL` convention, applied once in `ShortcutRegistry::resolved_keystrokes` via `Shortcut::declared_keystrokes` → `KeyStroke::with_command_convention`, so dispatch, `find_conflict`, menu labels, tooltips, the a11y walker and the native menu bridge all agree). A declared `Ctrl` chord therefore does **not** fire on physical ⌃ on macOS. Corollaries: a **user override** is literal (physical ⌃F stays bindable); an explicit `Modifiers::SUPER`, and `Ctrl+Super`, are left alone; `ShortcutBuilder::literal_modifiers()` opts a declaration out entirely — needed for Ctrl+Tab (⌘⇥ is the application switcher) and any chord whose ⌘ form the OS takes. `KeyStroke::command()`/`command_shift()` state the same intent outright for chords built **outside** the registry (context-menu labels). Widget code testing a live `Modifiers` uses `Modifiers::command()` / `Modifiers::COMMAND` — used by select-all, accelerator-click multi-select, marquee-additive, Ctrl+Home/End and every text/code editor; the Explorer-style Ctrl+Space / Ctrl+Arrow cursor pair, Ctrl+Tab's focus escape, Ctrl+Space forced completion, Ctrl+wheel scene zoom, the terminal's VT encodings and the any-modifier type-ahead guards deliberately stay on literal `ctrl()` and say so at their site.

**Caret motion inside a text surface is a *separate* question** — the accelerator flag can't answer it. macOS spreads the motions across three modifiers (`⌥←/→` word, `⌘←/→` line edge, `⌘↑/↓` document, `⌥↑/↓` paragraph) where Windows/Linux use `Ctrl+←/→` plus bare `Home`/`End`. So `RichTextEditor`, `CodeEditor` and everything built on `TextInputField` read their arrows through [`common::text_nav`](crates/teksilo-widgets/src/common/text_nav.rs) — `CaretStep {Character, Word, LineEdge}` / `LineStep {Line, Paragraph, Document}` / `deletes_word`, whose `*_for(TextNavConvention, …)` twins keep both platform branches reachable from one host's test run — the `with_command_convention_using` pattern. Word-delete rides the same modifier (`⌥⌫` / `Ctrl+⌫`). `Home`/`End` are NOT routed through it: bare = line edge, accelerator = document reads correctly from `Modifiers::command()` on every platform. `Alt+↑/↓` stays on move-line in the code editor (every code editor ships that, macOS included), so only rich text reads `⌥↑/↓` as paragraph. `⌘⌫` (delete-to-line-start) is not implemented and falls through to a single-character delete. The chord→motion tables are pure functions (`horizontal_op`/`vertical_op`/`line_edge_op`/`horizontal_command`) so the macOS-only mappings are unit-tested from Linux. `LogView` is deliberately NOT routed through this — it scrolls a read-only view and has no caret to move.
...
**ICU coverage.** All four styles are ICU-backed (`icu_decimal` + `icu_datetime` + `icu_experimental`, all `compiled_data`). `Decimal` — locale grouping, digit shaping, signs. `Percent` — value × 100 through ICU's `PercentFormatter`, so the sign is the locale's and sits where the locale puts it (`"12,5 %"` fr-FR, `"%12,5"` tr-TR). `Currency` — `CurrencyFormatter` with the short symbol (`"$1,234.50"` en-US, `"1 234,50 $US"` fr-FR); two consequences: ICU applies the **currency's** CLDR fraction precision, which overrides `.fraction_digits(...)` (`JPY` → `"¥1,235"`, ECMA-402 behaviour), and `.use_grouping(false)` does not reach currency (its constructors build their own `DecimalFormatter` with no seam for ours). A `Currency` style with no/invalid code degrades to plain decimal rather than rendering a wrong currency. `DateTime` — `CompositeDateTimeFieldSet`.

**The parse direction — `NumberSymbols`.** `NumberFormatter` is display-only and `f64`-based; an *editable* numeric surface also needs to read the locale form back, and needs the two directions to agree exactly. [`NumberSymbols`](crates/teksilo-i18n/src/format/symbols.rs) recovers a locale's separators / signs / digits **from ICU's own formatted output** — it formats probe values through the same `DecimalFormatter` the display path uses and reads them out of the `icu_decimal::parts` annotations — so format and parse cannot drift, the same single-source property `DateEdit` gets from its one `ParsedPattern`. (The alternative, `icu_decimal::provider::DecimalSymbols`, is public but carries ICU's "unstable Rust representation, may change in SemVer minor" banner.) `delocalize(…)` rewrites a displayed number into a C-locale string; `localize(…)` does the inverse. Both take and return **strings**, never `f64`, so an `i64`/`u64` past 2^53 keeps full precision and the caller's own type decides range and exactness. Also recovers the locale's **group sizes**, so the Indic lakh system renders `12,34,567` rather than a hardcoded every-three-digits. Parsing is lenient (ICU's default): whitespace ignored anywhere, ASCII digits accepted even where the numbering system is not `latn`. One ambiguity resolves in CLDR's favour — where `.` is the *group* separator (de-DE), `"1.5"` reads as `15`.

**`SpinBox` is the first consumer.** `.localized(bool)` (**default on**) follows the active locale for the decimal separator, digits and minus sign — display, commit parse and the per-character input filter all resolve from one `NumberPresentation` so they cannot disagree; a French user sees `12,5`, can type `12,5`, and the numeric keypad's `.` still works. `.use_grouping(bool)` (**default off**, matching Qt's `isGroupSeparatorShown`) adds thousands separators — helpful on a large read-only quantity, in the way on a field being typed into. Turn `.localized(false)` on for a number that is an *identifier* rather than a quantity (port, version component, database id). With no `I18nManager` installed the active locale is the C locale, so this is a no-op in tests and in apps that never opted into i18n. A user-supplied `text_from_value` / `value_from_text` owns the whole convention and is not re-punctuated.

**Files:** [crates/teksilo-i18n/src/format.rs](crates/teksilo-i18n/src/format.rs) (Memoizable types, ICU bridge, `TeksiloDateTime`, public formatter types, bundle callback, `DATETIME` function); [crates/teksilo-i18n/src/format/symbols.rs](crates/teksilo-i18n/src/format/symbols.rs) (`NumberSymbols`, the parts probe, `localize` / `delocalize`) + [crates/teksilo-i18n/tests/number_symbols.rs](crates/teksilo-i18n/tests/number_symbols.rs); [crates/teksilo-i18n/src/manager.rs](crates/teksilo-i18n/src/manager.rs) `configure_bundle` (one helper, three call sites at the `FluentBundle::new` boundary); [crates/teksilo-i18n-macros/src/lib.rs](crates/teksilo-i18n-macros/src/lib.rs) (`tr_signal!` / `tr_signal_widget!` lowering — branches off `tr_impl(input, kind, signal)`); [crates/teksilo-i18n/tests/format_integration.rs](crates/teksilo-i18n/tests/format_integration.rs) (end-to-end tests for both paths plus the macro).
...
| 3 | `PathEntry` (tiny-skia) | Arbitrary paths, SVG icons. `Path` carries a rolling content **stamp** maintained as commands are appended (`commands` is private; use `commands()` / `push` / `from_commands`), and the mask cache keys on that one word — so a *cache hit* is O(1) instead of O(n): linear in the command count, to flat. Absolute figures move with the machine, so the gate asserts the *shape*: `a_cache_hit_does_not_scale_with_the_paths_length` in `crates/teksilo-render/tests/wet_stroke_cost.rs` (~1× with the stamp, 8.4× with the walk restored). That file's `measure` test prints the table in [docs/ink.md](docs/ink.md) §5, which is the one place the measured microsecond figures are written down — don't re-quote them here or in rustdoc, or the columns stop being one run of one machine. |
...
Migration status: **every themable widget is on Tier 3** — [`ComponentStyleSlots`](crates/teksilo-core/src/styles/component_style_slots.rs) now carries **42** slots (the authoritative roster; count it there rather than trusting this list). Controls: `Button`, `SplitButton`, `IconButton`, `Toggle`, `Checkbox`, `RadioButton`, `Slider`, `SegmentedControl`, `ProgressBar`, `Link`, `Avatar`, `Badge`. Inputs: `TextInput`, `SearchField`, `ComboBox`, `SpinBox`, `DateEdit`, `ColorPicker`, `Calendar`, `RichTextEditor`. Containers: `Panel`, `Card`, `TabBar`, `ListView`/`TreeView` (via `ListContainerStyle`), `TableView`/`TreeTableView` (via `TableStyle`), `DropZone`, `DropTarget`. Overlays: `TooltipWidget`, `Popover`, `Dialog`, `Snackbar`, `Banner`. Rows: `MenuItem`, `StandardListItem`/`StandardTreeItem`. Chrome: `ScrollBar`. Added since that roster was written: `Splitter`, `RadioTile`, `Toast`, `GridView`, `WebView`, and `chart` (whose default impl lives in teksilo-charts, not teksilo-widgets). No themable widget self-paints anymore; each delegates chrome to `style.make_*(cfg, ctx)`. Four traits are multi-method: `TabStyle` (`make_body` + `make_bar`), `DialogStyle` (`make_panel` + `make_scrim`), `TableStyle` (`make_header_cell` + `make_sort_indicator` + `make_row_background`), `CalendarStyle` (`make_day_cell` + `make_zoom_cell` + `make_header`). The per-widget dim structs are deleted and their IntUI constants live in `teksilo-widgets/src/styles/recipe_*_style.rs`.

**Sibling presets.** `teksilo-theme-material3` (feature `theme-material3`) and `teksilo-theme-fluent` (feature `theme-fluent`) are complete design languages — tokens plus Tier-3 widget chrome — reachable as `teksilo::prelude::{material3, fluent}::{light, dark}`. Fluent is transcribed from WinUI's own `Common_themeresources_any.xaml` + control theme-resource dictionaries: light/dark colour dictionaries (full set on the `FluentPalette` theme extension), `ControlCornerRadius` 4 / `OverlayCornerRadius` 8, the WinUI type ramp at zero tracking, and `Control*AnimationDuration` on `ControlFastOutSlowInKeySpline` (`cubic-bezier(0,0,0,1)`). 25 style slots installed; eight are real `impl FooStyle` (button **elevation edge** — heavier stroke bottom-in-light / top-in-dark, dropped on press; two-tone high-contrast focus ring, never the accent; `ToggleSwitch` off-outline + morphing knob; filled unchecked checkbox/radio; field **accent focus underline**; two-circle slider thumb; neutral menu hover; list **selection pill** 3×16). `fluent::{light,dark}_with_accent(Color)` rebuilds the accent family around a seed. Mica/Acrylic use WinUI's own opaque fallbacks; Segoe UI Variable is named (not bundled) under the optional `system-fonts` feature. `teksilo-theme-macos` (feature `theme-macos`, `teksilo::prelude::macos`) is the third complete preset — macOS **Aqua / Dark Aqua**, with every literal tagged `[HIG]` / `[measured]` / `[derived]` at its definition because Apple publishes almost none of this (no radius table, no control heights, no focus-ring geometry, one animation duration). AppKit's semantic set — four label grades, two selection families, the bezel, the eight System Settings accents — is on the `MacOsPalette` extension. 6/10 dp radii (menus at a measured 9), a **22 dp** control height (a third under Fluent's 32), the published SF type ramp with Apple's **signed** tracking (−0.08 at 13 pt, 0 at 12, +0.06 at 11 — the only Teksilo preset that tracks non-uniformly), and Core Animation's 0.25 s on the *symmetric* `cubic-bezier(0.42,0,0.58,1)`. 28 slots installed; eight are real `impl FooStyle` (push-button **bezel** — shadow + face gradient + hairline + Dark-Aqua catch-light, dropped on press, and deliberately absent from the accent-filled default button; a focus ring that **is the accent**, built as a solid contrast-bearing band plus a translucent halo so it still clears WCAG 1.4.11; `NSSwitch` 18 dp knob in a 22 dp track; 14 dp bezelled checkbox/radio; field **accent focus halo**; plain round slider knob; menu row **accent fill + white label**; list **selection capsule**). `macos::{light,dark}_with_accent(Color)` + the `SystemAccent` enum rebuild the accent family; `linkColor` deliberately does not follow. Four WCAG deviations from Apple's own numbers are documented at their assignment with the measurement that forced each. The OS accent is **not** read (the platform layer returns only light/dark on macOS); vibrancy uses each material's opaque fallback; San Francisco is named (not bundled) under the optional `system-fonts` feature. Still pending: image-backed styles, `ImageTheme` TOML loader, a GTK4-Adwaita preset.
...
- GPU rendering (7 render pipelines + the blur pass, glyph atlas, path atlas)
- All 29 layout primitives (including Grid, Wrap, AspectRatio, Switcher, MasonryLayout, ColumnFlow, FormLayout, DeadZone)
- Accessibility. **Text ranges are on by default**: every visible label emits AccessKit `Role::TextRun` children — one per visual line, each carrying per-character positions and widths, a bounding box and a reading direction — so a screen reader reviews any label by character, word and line, braille routes to the word under the cell, and `AXBoundsForRange` / UIA `GetBoundingRectangles` / AT-SPI `GetCharacterExtents` all answer. Runs are excluded from object navigation by `accesskit_consumer`'s own filter, so they add no traversal stops and no hit-test targets. One emitter does this for every text surface — [`teksilo_core::accessibility::text_runs`](crates/teksilo-core/src/accessibility/text_runs.rs) — and it owns the contracts that panic or go silent when broken: the 255-character cap (`word_starts` is a `Vec<u8>` and the consumer probes `character_index as u8`), the hard break as ONE character at the end of its line's last run (`\r\n` = one character of length 2), UAX #29 word starts segmented per line and sliced per chunk, `next_on_line`/`previous_on_line` links, deduplicated ids, and **universal geometry** — real when measured, a zero-width box when not, never absent, because `Range::bounding_boxes()` discards everything it has collected on the first run missing any of the four. Runs hang **directly** off the text root: a `Role::Paragraph` in between is an object-navigation stop (`common_filter` does not exclude it) and drops every text-change event on all three platforms. A label inside a control that owns the same name calls `.a11y_hidden()`; a container names itself from its visible title with `labelled_by` (`Widget::accessible_title_node`) rather than by copying the string — except a **live region**, whose announcement path reads the node's own value and which therefore keeps `set_name` and hides its title label. `teksilo_core::accessibility::audit` checks all of it against a real `TreeUpdate` through the consumer. Builder-level overrides: `.access_label`, `.access_description`, `.access_hidden`, `.access_role`, `.access_disabled`, `.access_controls`/`described_by`/`labelled_by`, `.access_live`, `.access_shortcut_id`/`access_shortcut_literal`, `.access_action`/`access_remove_action`/`access_custom_action`, `.access_exclude_subtree`/`access_merge_subtree`, `.access_customize` — see "Accessibility Overrides" above
...
- Debug inspector (teksilo-inspector: in-app introspection panel, debug builds only, gated by `cfg(debug_assertions)`; F12 — or `InspectorState::toggle()`, the public door F12 itself now routes through, or a hold on the window's bottom-trailing corner grip, which is mounted only once a session has produced a touch sample — toggles a bottom panel with 10 tabs (Tree, Properties, Accessibility, Theme, Locale, Focus, Shortcuts, Overlays, Models, Pointers); the **Pointers** tab shows each node's touch-action and pan-claim declarations, the effective fold along a hit path, and the active input tokens, plus live contacts behind an opt-in watch that takes the application's pointer input while armed; bounds-overlay visualization (Off/Selection/All) with cursor-following type+size tooltip and Padding/StackGap tinted bands; picker tool with multi-window subtree exclusion; theme JSON Export/Import; resizable panel with persisted height; tree filter input + auto-scroll-into-view; Properties Copy button + right-click `Copy value` context menu + Debug repr row; Models tab with click-to-select per row; panel-scoped Ctrl+P/Ctrl+B/Ctrl+Tab/Ctrl+Shift+Tab/Esc keyboard shortcuts; persistence via `__teksilo_inspector.*` settings keys when `SettingsStore` is wired. Apps opt in with `TeksiloAppBuilder.install_inspector_in_debug()` (no-op in release) — the extension trait is re-exported from `teksilo::prelude::*` behind the umbrella's default-on `inspector` feature, so no separate `teksilo-inspector` dep is needed. See `docs/inspector.md`. Data models opt into the Models tab via `ListModel::debug_named("…")` / `TreeModel::debug_named` / `SelectionModel::debug_named`.)
...
- Automation MCP (teksilo-automation + teksilo-automation-mcp): a Model Context Protocol server that lets an AI agent **observe** (semantic AccessKit tree + screenshots) and **drive** (AT actions + synthetic pointer/key/IME input) a Teksilo app — in-process, without the OS accessibility layer. Runs on **Linux, macOS and Windows**. `teksilo-automation` is the GUI-free toolkit (core-only peer like `teksilo-data`): serde DTOs + one `execute(tree, ops, op, settle) -> reply` function + `RecordingWindowOps` (records `open_window` instead of panicking) + the 34-tool catalog + `wire` (framing, token handshake, `Endpoint`/`EndpointFile` — pure std+serde, shared verbatim by both ends of the bridge). `teksilo-automation-mcp` is the rmcp (`rmcp 2.2`) server binary, `tokio`/`rmcp` confined to it. **Two modes:** `--headless` (default) owns a demo `HeadlessApp` on a dedicated `!Send`-tree thread (the async handlers marshal `Send` DTOs to it + a `oneshot`; screenshots render offscreen via `create_test_renderer` + `pollster::block_on` on that thread); `--attach` / `--attach-pid <pid>` / `--list` (and the explicit `--connect <endpoint> --token <uuid>`) drive a *live* app through its debug-only in-app bridge. **34 tools:** snapshot_tree/read_node/find_node/assert_node/list_windows, layout_tree/inspect_node (full widget/layout-arena tree + per-widget detail incl. Debug repr — the inspector's Tree/Properties data, richer than the AT snapshot since it includes widgets the AT tree prunes; keyed by the same node ids), invoke_action/focus_node/set_value/expand/collapse/scroll, inject_pointer/right_click/inject_key/type_text/type_ime/drag_node, inject_touch_sequence/pinch/fling/long_press/cancel_pointer/query_pointers/set_density, get_overlays/get_shortcuts/list_live_regions/pull_announcements, advance_clock/settle/wait_for_condition, screenshot (MCP image block + a `{width,height,scale}` metadata block — pixels are *physical*, every other coordinate is logical, so `scale` is what lets a caller click what it can see). The three injection tools take a **`command`** modifier beside `ctrl`: `command` is the platform accelerator (Control on Windows/Linux, ⌘ on macOS) and `ctrl` stays literal — a shortcut declared `Ctrl+S` resolves to ⌘S on macOS, so `ctrl` there injects a key that matches no binding *and still reports success*. The **live bridge** (`teksilo-app` `automation` feature, gated `#[cfg(all(feature="automation", debug_assertions))]`) is platform-agnostic policy — token, accept loop, main-thread routing — over `teksilo-platform::automation_transport` (feature `automation`), which supplies the per-OS endpoint: a `0600` Unix socket in a `0700` per-process dir under `$XDG_RUNTIME_DIR` (Linux) or `$TMPDIR` (macOS), or a **named pipe with an owner-only DACL** on Windows (the default descriptor grants read to Everyone + anonymous, so it is always built explicitly from the process token's SID; `PIPE_REJECT_REMOTE_CLIENTS` is a second layer, never a substitute). Discovery is an **endpoint descriptor** (`<runtime dir>/teksilo-automation/<pid>.json`, carrying the token) so `--attach` works identically on all three platforms with nothing to scrape from stderr — owner-only, and on Unix `create_new` with the `0600` *in the same open* (create-then-chmod would leave the token at the umask default for a window, and would follow a symlink planted at the temp path), into a directory checked to be a real, self-owned `0700` one *before* the token reaches disk, because the fallback for a Unix without `$XDG_RUNTIME_DIR` is the shared `/tmp` (Windows inherits `%LOCALAPPDATA%`'s per-user ACL). A descriptor outlives a process that exits without unwinding, so `--list`/`--attach` prune by probing — three-state (`Liveness::{Live, Busy, Dead}`), and only `Dead` unregisters, so a healthy bridge whose one connection slot another client already holds survives a read-only-looking `--list` instead of being deleted by it. 4-byte-LE+JSON framing, single connection/inflight, a 10 s handshake deadline bounding the handshake *end to end* (the transport's timeout is per-`read` and the token is read a byte at a time, so a per-read bound alone lets a peer drip bytes and hold the slot for `MAX_TOKEN_LINE` × 10 s), and a 15 s reply deadline (`BRIDGE_TIMEOUT`) so a main thread inside a native modal loop costs one request rather than the connection slot for the process's life — winit's macOS backend queues user events in `kCFRunLoopDefaultMode` only, so this is load-bearing there. Reads framed `AutomationRequest`s and posts an `AutomationPayload` (Send reply `SyncSender`) through the existing `AppEvent::External` path → the winit main thread runs `execute` (or, for screenshots, `PlatformWindow::capture_offscreen`, which swizzles BGRA→RGBA for the DX12/Metal surface formats); the settle runs synchronously on the main thread. `wait_for_condition` spends its budget as **simulated** frames, not wall clock, so it resolves identically everywhere (a 1 ms poll sleep costs ~15.6 ms on Windows, which used to buy ~15× fewer frames). Framework deltas (all additive): `WidgetTree::dispatch_access_action`/`at_version() -> Signal<u64>`/`announcements_since(seq)` (live-region diff ring buffer + `Announcement` type)/`last_proposal()`; `PlatformWindow::capture_offscreen`; `teksilo_render::test_support::{create_test_renderer, try_read_texture_rgba}` (adapter selection is a *search* — preferred then software fallback — because a host can enumerate an adapter it cannot open, which is what made screenshots unavailable on GPU-less Windows hosts and CI). Apps opt in with `TeksiloAppBuilder::install_automation_bridge_in_debug()` (umbrella `automation` feature; a no-op in release on every platform). **Debug-only, zero release surface** (CI canary greps the release binary for the bridge banner). Reference: [docs/automation-mcp.md](docs/automation-mcp.md). Smoke: `cargo run -p automation_bridge_smoke` (or `-- --serve` to keep it alive for an external `--attach` client). Run the server: `teksilo-automation-mcp --headless`.
...
- Scene viewport (teksilo-scene: `Scene`, `SceneView`, `SceneItem` trait, built-in `RectItem` and friends, `minimap`). Two-tier — heavyweight `Widget` (full focus/animation/DnD/AT survives embedding) + lightweight `SceneItem` (paint-only, no arena cost). Pan / zoom gestures with per-axis policy, drag modes, exact-shape hit-test via `SceneItem::shape() -> ItemShape` (one geometry descriptor, answering containment, the four Qt `ItemSelectionMode`s and a `SceneRegion` lasso; it replaced the old `shape_contains` + `clone_shape_test` pair), per-item `CacheMode::ItemCoordinate` GPU cache, collision API, reactive `item_change_signal`, background/foreground paint hooks, signal-driven dynamic bounds, selection, z-order, removal. **Shared model + per-view delegate (multi-view)**: `SceneModel` (cloneable `Rc<RefCell<Scene>>` handle) drives N `SceneView::with_model(model.clone())` panes; heavyweight content is a type-erased payload (`add_widget_item(payload, rect)`) built per-view by `delegate_typed::<P>(|&P, ItemId| Box<dyn Widget>)`, so each pane owns its own arena instances. Single-view `Scene::add_widget(w, rect)` (the `Once` path, drained by the first view) stays as sugar. **Live runtime mutation**: every `SceneModel` mutator is `&self`, so a handler holding a clone (`view.model()`) drives the scene directly — no `with_widget_mut` needed for content; `with_widget_mut::<SceneView>(id, Relayout, |v| v.ensure_visible(..))` is for per-view camera. Each view self-reconciles on every `ItemChange` variant (add materialises, remove destroys the orphaned arena widget + cleans maps, move/transform/reparent/visibility re-place + re-walk, `PayloadChanged` rebuilds that item's widget) AND on a separate `Scene::a11y_change_signal` for pure-a11y mutations (group add, reparent, live, landmark, categories) — so the visual tree and the separate AccessKit tree both follow per-view (`build()` calls `request_accessibility_update()`, since relayout no longer re-walks AT). App-injectable view state via `view_state(pan_x, pan_y, zoom, rotation)` / `initial_pan` / `initial_zoom` / `initial_rotation` so view state survives a rebuild-from-state; selection is per-view, opt-in shared via `.selection_model(SceneSelection)`. **Accessibility-complete**: every visible heavyweight widget participates as a normal child; every visible lightweight item gets a synthetic AT node with role + screen-projected bounds; Tab cycles in scene-insertion order. Override surface mirrors widget-level: logical groups, reparenting (visual tree ≠ AT tree), relations (controls/described_by/labelled_by), live regions, landmark roles, rotor/quick-nav categories, subtree mode (`Merge`/`Exclude`), custom focus order, `access_*` builder chain. Intended for story corkboards, mind maps, node-graph editors, timeline views, CAD canvases, simple maps. **Magnetism** (typed snap-and-connect between item anchors): `Magnet` (local point + advisory `MagnetRole` Source/Target/Bidirectional + type-erased payload), per-view `SceneView::magnetism(MagnetismConfig)` with a consumer accept/reject predicate + `on_connect`; three input methods sharing one mechanism — item-drag-snap (mouse, lightweight items) + port-drag wires (grab a magnet handle) + a keyboard connect flow (`m` then arrows/Enter, works for any item kind); reusable `SceneModel::compute_item_snap`/`compute_port_snap` for heavyweight consumers driving their own drag; built-in feedback renderer (markers + ghost wire, `MarkerVisibility`); mechanism-in-scene/policy-in-consumer (scene stores no connection state — persistent edges vs one-shot reparent is the consumer's choice); synthetic `SceneMagnet` AT nodes + roving `active_descendant` keyboard flow. **Three paint bands, not two**: `SceneLayer::{Under, Interleaved, Over}`. `Interleaved` shares the cards' `PaintKey` rank, so a lightweight item orders against the heavyweight tier by `z` — ink above note A and below note B, which the old binary band could not express. It is realised by one **paint-only** node per interleaved item (`view/paint_node.rs`): `event_pass_through`, left out of `accessibility_children`, not focusable — so the pointer still resolves through the lightweight hit snapshot, the AT tree is bit-identical (the item keeps its synthetic `SceneItem` node), and `tab_stops_within` does not move. The hit veto (`accepts_child_hit`) compares whole `PaintKey`s per child rather than ranks, so an interleaved claimant vetoes the cards it is painted over and leaves the ones over *it* alone. **Cards** — `SceneCard` (`scene_card.rs`) is the content-agnostic container: surface, header grab handle, selection ring, `CardMode::{Idle,Selected,Editing}`, one named `Role::Group`. Its gesture regime is structural rather than a recogniser race — header `on_drag` moves the card, the card root's `gesture_dead_zone` stops a body press from reaching the view's marquee, the trailing header slot gets its own `DeadZone`. The header drag is **anchored to the press with the preview added back** (the preview moves the node the positions are localised against, so summing the recogniser's deltas reads 40/0/40 for three even moves and permanently loses the slop-crossing travel), asks `SceneModel::constrain_move` per sample and commits once via `apply_transform_delta`. **`SizePolicy::{Fixed,HeightForWidth,Intrinsic}`** hands a heavyweight entry's axes to its widget, measured in `place_children` for the cards a pass actually lays out — an unrealised card keeps its estimate, so the viewport cull's promise holds. The write-back is `ItemChange::MeasuredSizeChanged`: not an edit, not structural, answered by a relayout. **`ScrollIntoView`** is answered at last: a caret inside an embedded `RichTextEditor` pans the camera, `target_bounds` arriving in scene coordinates and the applied shift reported back in the same space. **`WetLayer`** is a surface for content being authored right now — a node of its own, so `request_repaint` marks it and NOT the view (the render walker computes one `needs_paint` per node gating both `paint` and `post_paint`, so `SceneView::foreground` cannot be invalidated without re-running the whole `Under` band). Always the last child: above every card and interleaved item, under the `Over` band and the view's own chrome. Demos: `cargo run -p scene-showcase`, `cargo run -p scene-corkboard`, `cargo run -p scene-magnetism`, `cargo run -p scene-ink`. References: [docs/teksilo-scene.md](docs/teksilo-scene.md), [docs/teksilo-scene-a11y.md](docs/teksilo-scene-a11y.md), [docs/ink.md](docs/ink.md).
- **RichTextEditor / RichTextViewer** (rich_text/ module: state, keyboard, mouse, hit_test, paint, clipboard, context_menu, frame_loop, policy, image_cache) — a full QTextEdit-class rich editing surface over the external `text-document` model (blocks, lists, tables, formats, **undo/redo**) + `text-typeset` (shaping/bidi/line-break/atlas). **~322 `#[test]` fns** (count, not a guarantee of adequacy — see Caveats), exercising: IME composition (commit + cancel-leaves-document-clean), HTML + plain-text clipboard with self-fragment → HTML → plain paste preference + CRLF normalisation + paste-unformatted, tables (Ctrl+A scope in/out, Ctrl+Enter block insert), lists (Tab/Shift+Tab indent/dedent, backspace-at-list-start), blockquotes (enter/backspace/delete unwrap rules), caret affinity (upstream/downstream at wrapped-line boundaries), context menu (copy/cut/paste/paste-unformatted, slot override), undo/redo, and an accessible text-run tree carrying character positions and byte-vs-char multibyte lengths with signal-driven rebuild on edit. The walk over the flow lives in `rich_text/body/flow_walk.rs` and covers all three element kinds: blocks, **tables** (`Role::Table` → `Role::Row` → `Role::Cell`, with dimensions, per-cell coordinates, spans and boxes) and **frames** (a blockquote earns a `Role::Blockquote`; every other frame is a layout box and is flattened into the enclosing document). Runs may not hang off a `Role::Cell`, a `Role::Heading` or a `Role::Blockquote`: the platform adapters route a run's text-change event to its *filtered* parent and drop it unless that node `supports_text_ranges()`, which neither role does — so all three carry a `Role::Label` text container in between (the same rule holds in `code_editor/a11y.rs`), and a run's parent chain is the one thing to preserve when touching this walk. `RichTextEditor::editor` / `read_only` accept `.min_lines(n)` / `.max_lines(n)` for intrinsic-mode sizing (the messenger-composer pattern). **Colour overrides** — `.background(impl Into<ColorProp>)` / `.text_color(..)` / `.caret_color(..)` / `.selection_color(..)` all take `Color | theme role | Signal` (like the rest of the framework): `background` is threaded into the active `RichTextEditorStyle::make_body` so the common "give the editor a surface" case needs no custom style; `text`/`caret`/`selection` are resolved against the active theme on every paint (a role / `Signal` stays reactive, a bare `Color` pins it) — `None` tracks the theme's `editor_fg`/`editor_caret` roles. **Caveats:** those tests run against `MockTextBackend` (fixed metrics) — they verify editing logic and caret math, NOT real shaping/bidi/complex-script layout, which is delegated to `text-typeset`'s own test suite (external sibling crate); embedded-image support is minimal (45-line `image_cache.rs`, lightly tested); and `TextInput` / `TextInputField` / `PasswordField` all derive from this stack, so it is load-bearing for every text field in the framework. Demos: `cargo run -p rich-text-editor`, `cargo run -p rich-text-viewer`.
...
- Scene viewport: [crates/teksilo-scene/src/](crates/teksilo-scene/src/) — `scene.rs` (the core scene model, `WidgetSource` Once/Delegated, every `ItemChange` variant, `SizePolicy` + `set_measured_size`), `scene_model.rs` (`SceneModel` = cloneable `Rc<RefCell<Scene>>` handle; `&self` mutators, `add_widget_item`/`set_payload`/`payload`, `transaction`/`set_edit_sink`, `constrain_move`/`constrain_frame`, `write_guard`), `view.rs` (the `SceneView` widget; `with_model`/`delegate_typed`/`selection_model`/`model`, per-view delegate materialisation in `build()`, `MeasureTrack`'s convergence guard) plus `view/` — `build_impl` / `builder_impl` / `layout_impl` / `paint_impl` / `a11y_impl` / `camera_impl` / `gestures_impl` / `widget_trait`, and three modules worth naming: `view/hit_snapshot.rs` (the pick snapshots, built once and invalidated per item from the change stream — why a pan costs the viewport and not the model), `view/paint_node.rs` (the paint-only node behind `SceneLayer::Interleaved` and the `WetLayer`), `view/transform.rs` (the transform controller's per-view runtime), and `view/magnetism.rs`. **Geometry and picking**: `shape.rs` + `shape/` (`ItemShape`, `ShapeGeometry`, `ItemSelectionMode`, `SceneRegion`), `pick.rs` (`PaintKey`, `Pick`, `PickFilter`, `claims_press` — one paint-and-hit order across both tiers), `index.rs` (spatial index), `transform.rs`. **Editing seams**: `constrain.rs` (`ProposedChange` / `ChangeVerdict` — the one geometry rule, on the model), `journal.rs` (`SceneChange` / `TxnId` / `ChangeSource` / `HistoryMode` — the seam a data layer reverses an edit through), `salvage.rs` (`Scene::take`/`restore`, and the whole logical-AT slice they carry), `transform_session.rs` (`TransformConfig` / `TransformSession` / `TransformDelta` / `TransformHandle` / `TransformStep`). **Content and chrome**: `item.rs` + `items/` (`SceneItem` trait and built-in items), `scene_card.rs` (`SceneCard` / `CardMode` — the content-agnostic card, its gesture regime and its one AT group), `scene_list_adapter.rs`, `minimap.rs`, `scroll_view.rs`, `a11y.rs` (AT walker), `magnet.rs` (`Magnet`/`MagnetId`/`MagnetRole`/`MagnetRef`/`MagnetVerdict`/`MagnetConnection`/`MagnetSnap`/`MagnetismConfig`/`MagnetFeedback` + `Scene::compute_item_snap`/`compute_port_snap`/`nearest_magnet`), `cache.rs`, `selection.rs`, `flags.rs`, `item_handlers.rs`, `animation.rs`, `state.rs`. Core touchpoints: `SyntheticKind::SceneMagnet` in [crates/teksilo-core/src/accessibility.rs](crates/teksilo-core/src/accessibility.rs); `WidgetPlacement::dormant` + `Widget::culls_children` in [crates/teksilo-core/src/widget/paint_context.rs](crates/teksilo-core/src/widget/paint_context.rs). References: [docs/teksilo-scene.md](docs/teksilo-scene.md), [docs/teksilo-scene-a11y.md](docs/teksilo-scene-a11y.md), [docs/ink.md](docs/ink.md), and the generated per-type pages under [docs/scene/](docs/scene/). Demos: [examples/scene_showcase/src/main.rs](examples/scene_showcase/src/main.rs), [examples/scene_corkboard/src/main.rs](examples/scene_corkboard/src/main.rs), [examples/scene_magnetism/src/main.rs](examples/scene_magnetism/src/main.rs), [examples/scene_ink/src/main.rs](examples/scene_ink/src/main.rs)
...
- teksu! DSL macro: [crates/teksilo-macros/src/](crates/teksilo-macros/src/) (parse → IR → lower). Trybuild fixtures at [crates/teksilo/tests/teksi/](crates/teksilo/tests/teksi/) (`pass/` + `fail/`, driven by `teksi_trybuild.rs`)
...
// Pre-registered children (when you need the ID) — .child() takes a WidgetId too
let label_id = ctx.add(TextWidget::new(lit!("Status")).text(status_signal));
HStack::new().child(label_id)
