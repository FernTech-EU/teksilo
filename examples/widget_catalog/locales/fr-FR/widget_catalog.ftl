...
tab-layout-refs = Piles, grilles et distribution du jeu. Voir : cargo run -p text_and_layout, cargo run -p splitter.
tab-visuals-refs = Primitives de rendu, icônes, images, bandes de validation. Voir : cargo run -p simple_button.
tab-containers-refs = Panneaux, cartes, accordéons, zones de défilement, vues fractionnées. Voir : cargo run -p splitter, cargo run -p tool_box.
...
# ── Overlays tab ────────────────────────────────────────────────────────
ovr-tooltip-hover = Survolez ou maintenez
...
overlays-hover-me = Survolez ou maintenez
...
ovr-hover-level-1 = Survol ou appui long — niveau 1
ovr-hover-level-2 = Survol ou appui long — niveau 2
ovr-hover-level-3 = Survol ou appui long — niveau 3
...
ovr-rich-dwell-tip = Astuce : restez ~2 s pour épingler, puis cliquez les liens pour enchaîner. Un doigt ne survole jamais — maintenez le bouton.
...
# ── Onglet Tactile ────────────────────────────────────────────────
tab-touch-title = Tactile
tab-touch-refs = L'échelle de densité, les trois profils de gestes, et ce qu'un doigt obtient qu'une souris n'a pas. Voir : cargo run -p touch-playground.
touch-section-ladder = Échelle de densité
touch-ladder-note = Relancez avec --density comfortable ou --density touch pour construire tout le catalogue sur un autre échelon. Une densité se décide dans chaque build(), d'où un indicateur plutôt qu'un commutateur.
touch-section-profiles = Profils de gestes
touch-profiles-note = Les seuils sont toute la différence entre les périphériques — le chemin de code est le même. Une souris n'a aucun seuil de panoramique, et c'est pourquoi un glissement à la souris ne défile jamais.
touch-section-pan = Défiler au doigt, réordonner après un appui long
touch-pan-note = Glissez au doigt et la liste défile ; maintenez d'abord une ligne et le même glissement la réordonne. Une souris réordonne immédiatement et défile à la molette.
touch-section-manipulator = Un manipulateur n'est jamais un panoramique
touch-manipulator-note = Le curseur déclare qu'aucun comportement tactile par défaut ne s'applique à lui : glisser la poignée modifie la valeur sur tout périphérique. Glissez ailleurs dans le cadre et le cadre défile.
touch-slider-label = Valeur
touch-section-targets = Cibles de préhension
touch-targets-note = La gouttière est peinte à la taille de préhension du thème puis élargie pour un pointeur grossier par le débord de zone sensible du widget : ce que l'on voit et ce que l'on peut atteindre diffèrent volontairement.
touch-section-text = Texte au doigt
touch-text-note = Un appui place le caret au relâchement, un appui long sélectionne le mot sous le doigt et fait apparaître deux poignées, et glisser une poignée déplace cette extrémité de la sélection.
touch-text-placeholder = Appuyez, maintenez, glissez une poignée
touch-playground-note = cargo run -p touch-playground rapporte chaque pointeur reçu, nomme le concurrent qui a remporté chaque appui, et change la densité et les constantes cinétiques à chaud. docs/touch-verification.md est la liste de contrôle pour un appareil réel.
