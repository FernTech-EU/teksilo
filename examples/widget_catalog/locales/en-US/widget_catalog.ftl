...
tab-charts-title = Charts
tab-scene-title = Scene
tab-richtext-title = Rich Text
tab-dragdrop-title = Drag & Drop
...
tab-layout-refs = Stacks, grids, and slack distribution. See: cargo run -p text_and_layout, cargo run -p splitter.
tab-visuals-refs = Rendering primitives, icons, images, validation strips. See: cargo run -p simple_button.
tab-containers-refs = Panels, cards, accordions, scroll areas, splitters. See: cargo run -p splitter, cargo run -p tool_box.
...
# ── Overlays tab ────────────────────────────────────────────────────────
ovr-tooltip-hover = Hover or hold me
...
dat-treetable-note = TreeTableView combines TableView columns with TreeView hierarchy. See `cargo run -p tree-table` for the mock-filesystem demo.
...
overlays-hover-me = Hover or hold me
...
ovr-hover-level-1 = Hover or hold — level 1
ovr-hover-level-2 = Hover or hold — level 2
ovr-hover-level-3 = Hover or hold — level 3
ovr-plain-among-rich = Plain among rich
ovr-plain-among-rich-tip = Plain tooltip living in the rich column — diagnostic.
ovr-rich-dwell-tip = Tip: dwell ~2 s to pin, then click links to chain. A finger never hovers — hold the button instead.
...
ovr-toast-btn-success = Success
ovr-toast-btn-warning = Warning
ovr-toast-btn-error = Error
ovr-toast-btn-loading = Loading
ovr-toast-info-msg = Info notice
ovr-toast-success-msg = Saved
ovr-toast-warning-msg = Warning
ovr-toast-warning-body = Take a look when you have a moment.
ovr-toast-error-msg = Build failed
ovr-toast-error-body = Three errors, two warnings.
ovr-toast-error-action = Show errors
ovr-toast-loading-msg = Working…

# ── Drag & Drop tab ────────────────────────────────────────────────────
dnd-zone-images-title = Drop images here
dnd-zone-images-subtitle = PNG · JPEG · GIF
dnd-zone-any-title = Drop anything here
dnd-zone-any-subtitle = files, text, or links
dnd-target-body = DropTarget — wraps a Panel; drop a file to see the border highlight
dnd-target-hint = Release to drop
dnd-log-initial = Dropped items appear here.
dnd-section-zone-any = DropZone — files / text / URLs
dnd-section-zone-images = DropZone — images only
dnd-section-target = DropTarget — wrapping container
dnd-section-log = Drop log
# ── Touch tab ─────────────────────────────────────────────────────
tab-touch-title = Touch
tab-touch-refs = The density ladder, the three gesture profiles, and what a finger gets that a mouse does not. See: cargo run -p touch-playground.
touch-section-ladder = Density ladder
touch-ladder-note = Relaunch with --density comfortable or --density touch to build the whole catalog on another rung. A density is decided inside every build(), so it is a flag rather than a switch.
touch-section-profiles = Gesture profiles
touch-profiles-note = The thresholds are the whole difference between the devices — the code path is the same one. A mouse has no pan threshold at all, which is why a mouse drag never scrolls.
touch-section-pan = Pan to scroll, reorder behind a hold
touch-pan-note = Drag with a finger and the list scrolls; hold a row first and the same drag reorders it. A mouse reorders straight away and scrolls with its wheel.
touch-section-manipulator = A manipulator is never a pan
touch-manipulator-note = The slider declares that no touch default may run over it, so dragging the thumb moves the value on every device. Drag anywhere else in the box and the box pans.
touch-slider-label = Value
touch-section-targets = Grab targets
touch-targets-note = The gutter is painted at the theme's grab size and widened for a coarse pointer by the widget's own hit outset, so what you see and what you can hit are different sizes on purpose.
touch-section-text = Touch text
touch-text-note = Tap places the caret when the finger lifts, a hold selects the word under it and raises two handles, and dragging a handle moves that end of the selection.
touch-text-placeholder = Tap, hold, drag a handle
touch-playground-note = cargo run -p touch-playground reports every pointer it is handed, names which contender won each press, and switches density and the kinetic constants live. docs/touch-verification.md is the checklist for a real device.
