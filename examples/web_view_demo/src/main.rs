...
//!
//! Run: `cargo run -p web-view-demo`
//!
//! **Note:** the default build installs the headless `MemoryWebViewBackend`,
//! so no real page renders yet — the native `wry` / `servo` engine backends
//! are opt-in features pending the spike. The widget wiring, the tab/dormancy
//! bridge, and the JS↔Rust message plumbing are all exercised regardless.
