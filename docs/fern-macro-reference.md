# `fern!` Macro Reference

A block-structured DSL for FernUI widget trees. `fern!` is a thin syntactic
transform: every invocation desugars one-to-one to FernUI V2 builder calls
...
full grammar, see [fern-language-spec-v3.md](fern-language-spec-v3.md).
...
```rust
use fern_ui::prelude::*;        // also provides: fern!
// or:
use fern_ui::fern;
```
...
```rust
fern!(ctx => <root-element>)    // inserts the root via ctx.add, returns WidgetId
fern!(<root-element>)           // returns a widget value (for .child(...), etc.)
```
...
the call site. Pure fern! blocks with no bindings or escapes (just
...
it later. Bindings hoist to the enclosing `fern!` block:
...
`title` is in scope for any subsequent item in the same `fern!` block,
...
// Three or four arms: FernBranch3 / FernBranch4.
VStack {
    if count == 0 {
        TextWidget("Empty")
    } else if count == 1 {
...
// ↓ .child(match state { ... FernBranch3::{A,B,C}(...) })
```
...
| `if cond { ‹A› } else { ‹B› }` | `.child(if cond { FernBranch::L(‹A›) } else { FernBranch::R(‹B›) })` |
| `match x { p => ‹E›, … }` | `.child(match x { p => FernBranchN::…(‹E›), … })` |
...
- **Binding hoist scope**: bindings declared inside `if`/`else`/`match`/
  `for` bodies currently hoist to the outermost `fern!` block. The
...
- [fern-language-spec-v3.md](fern-language-spec-v3.md) — complete
...
- [crates/fern-ui-macros/src/](../crates/fern-ui-macros/src/) — the
