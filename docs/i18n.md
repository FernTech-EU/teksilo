...
every key the other two defined — they fall back to the source locale,
silently, so the translator watches most of the app revert to English
...
recursion, so pointing at `locales/` rather than `locales/fr-FR/` finds
nothing and errors instead of quietly loading a sibling locale's strings
into the wrong bundle. Files are merged **sorted by file name**, because
Fluent keeps the *first* definition of a key and `read_dir` order is
unspecified: without the sort, a key defined in two files could resolve
differently between two saves of the same unchanged directory.
...
`build_bundle_from_resources` tolerates them at startup — hot reload's
job is to reproduce what the shipped binary does, and refusing where the
...
Registering the same locale twice does **not** merge the two paths —
each change rebuilds that locale from whichever path fired. Pass the
