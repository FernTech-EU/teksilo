...
Bastyde's i18n stack (`bastyde-i18n` + `bastyde-i18n-macros`) is **reactive
...
[`I18nManager`](../crates/bastyde-i18n/src/manager.rs) that owns one
...
- `BastydeAppBuilder::i18n(config)` installs the resulting `I18nManager`
...
- The framework's own widget strings come from
  `bastyde::widgets::framework_locales()` — this is what gives
  `MenuBar`, `Dialog`, accessibility labels, keystroke names, etc.
  their localized text. Apps almost always want this.
- `bastyde::prelude::*` re-exports `tr` / `tr_widget` / `LocalizedString`
  / `I18nConfig` / `LanguageIdentifier` (gated on the `i18n` cargo
  feature). The reactive macros (`tr_signal`, `tr_signal_widget`) and
  the formatter types (`NumberFormatter`, `BastydeDateTimeFormatter`,
  `BastydeDateTime`, `NumberStyle`, `DateStyle`, `TimeStyle`) are
  reachable through the `bastyde::i18n::*` path — they're not in the
...
[Source](../crates/bastyde-i18n/src/config.rs). Builder for the manager's
...
| `.framework_locales(&'static [...])` | Register framework strings (`bastyde::widgets::framework_locales()`). |
...
[`I18nManager::resolve_initial_locale`](../crates/bastyde-i18n/src/manager.rs)
...
typically called by `BastydeAppBuilder` at startup, or by app code in
...
[`bastyde-i18n-macros`](../crates/bastyde-i18n-macros/src/lib.rs).
...
- The macro looks for the source `.ftl` in this order: directory at
  `BASTYDE_I18N_SOURCE_DIR` (env var, dir of `.ftl` files) → single file
  at `BASTYDE_I18N_SOURCE_PATH` (env var, one `.ftl` file) → directory
...
`tr!(...)` expands to a [`LocalizedString`](../crates/bastyde-i18n/src/localized_string.rs):
...
inside `bastyde-widgets` and any app crate that ships overrides for
framework strings.
...
standard numeric types, `String`, and `BastydeDateTime`). For static
...
[`tests/format_integration.rs`](../crates/bastyde-i18n/tests/format_integration.rs)
...
[Source](../crates/bastyde-i18n/src/format.rs).
...
```rust
use bastyde::i18n::{tr, BastydeDateTime};

tr!(price_display(v = 1234.56))     // "The price is 1,234.56"
tr!(last_saved(ts = BastydeDateTime::from(some_jiff_zoned)))
```

For numeric args pass any `f64`/`i32`/`u64`/etc. (already covered by
`FluentValue: From<T>`). For date/time args, wrap in
[`BastydeDateTime`](#bastydedatetime).

### Signal-side: `NumberFormatter` / `BastydeDateTimeFormatter`
...
```rust
use bastyde::i18n::{
    DateStyle, BastydeDateTimeFormatter, NumberFormatter, NumberStyle,
};
...
`BastydeDateTimeFormatter` builder methods:
...
BastydeDateTime::from(now)              // From<jiff::Zoned>
BastydeDateTime::from(civil)            // From<jiff::civil::DateTime>
BastydeDateTime::from_zoned(now)        // explicit constructor
BastydeDateTime::from_civil(civil)      // explicit constructor
```

`BastydeDateTime` implements `From<...> for FluentValue<'static>`, so
...
[`rtl_from_locale`](../crates/bastyde-i18n/src/direction.rs) when the
...
the manager wires an [`FtlFileWatcher`](../crates/bastyde-i18n/src/file_watcher.rs)
...
`tr_signal!`, `NumberFormatter::format`, `BastydeDateTimeFormatter::format`)
...
[`bastyde_i18n::thread_local::install(mgr)`](../crates/bastyde-i18n/src/thread_local.rs)
...
`install` and `clear` are exposed under `bastyde_i18n::thread_local::*`
for test harnesses; production apps shouldn't call them directly —
`BastydeAppBuilder::i18n(...)` does it for you.
...
```rust
use bastyde_i18n::compile_in_locales;
...
it's wrong, set `BASTYDE_I18N_SOURCE_PATH` (single file) or
`BASTYDE_I18N_SOURCE_DIR` (directory) in the calling crate's
environment.
...
| Topic | Path |
| --- | --- |
| `I18nConfig` builder | [`crates/bastyde-i18n/src/config.rs`](../crates/bastyde-i18n/src/config.rs) |
| `I18nManager` + bundle wiring | [`crates/bastyde-i18n/src/manager.rs`](../crates/bastyde-i18n/src/manager.rs) |
| `LocalizedString` + `to_signal` | [`crates/bastyde-i18n/src/localized_string.rs`](../crates/bastyde-i18n/src/localized_string.rs) |
| Locale-aware formatting (Number/DateTime) | [`crates/bastyde-i18n/src/format.rs`](../crates/bastyde-i18n/src/format.rs) |
| Hot-reload file watcher | [`crates/bastyde-i18n/src/file_watcher.rs`](../crates/bastyde-i18n/src/file_watcher.rs) |
| Layout direction (RTL) | [`crates/bastyde-i18n/src/direction.rs`](../crates/bastyde-i18n/src/direction.rs) |
| Thread-local + crate-root accessors | [`crates/bastyde-i18n/src/thread_local.rs`](../crates/bastyde-i18n/src/thread_local.rs) |
| `tr!` / `tr_widget!` / `tr_signal!` / `tr_signal_widget!` | [`crates/bastyde-i18n-macros/src/lib.rs`](../crates/bastyde-i18n-macros/src/lib.rs) |
| `compile_in_locales!` declarative macro | [`crates/bastyde-i18n/src/lib.rs`](../crates/bastyde-i18n/src/lib.rs) |
| End-to-end demo | [`examples/internationalization`](../examples/internationalization/src/main.rs) |
| Format integration tests | [`crates/bastyde-i18n/tests/format_integration.rs`](../crates/bastyde-i18n/tests/format_integration.rs) |
| `tr!` integration tests | [`crates/bastyde-i18n/tests/tr_macro.rs`](../crates/bastyde-i18n/tests/tr_macro.rs) |
