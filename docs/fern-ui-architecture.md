...
The host's interface is intentionally minimal: insets (`reserved_leading_inset`, `reserved_trailing_inset`), capability flags (`renders_custom_controls`, `needs_custom_resize_handles`), interactive operations (`begin_drag`, `begin_resize`, `show_window_menu`), and a per-frame `update_hit_regions(&HitRegions)` snapshot the Windows backend reads from `WM_NCHITTEST`. Three more methods exist to support synthetic-tap forwarding from non-client clicks (`title_bar_widget_id` resolves a `ControlTarget` to its `WidgetId`, `set_button_hover` injects `WM_NCMOUSEMOVE` hover into widget land, `register_hover_signal` lets `WindowControls` register the `Signal<bool>` per button at build time) — all default no-op so Wayland and macOS don't need them. State operations (`minimize`, `toggle_maximize`, `close`, `is_maximized`) live on `WindowState` instead; the `TitleBar` widget's maximize button binds directly to `ctx.window().placement()`. One consequence: the maximize glyph swap now works with `DecorationsMode::Native` too — previously custom-chrome-only.

The Windows backend (M4 + M5, shipped) replaces the native frame with the widget-painted bar: extends the DWM frame into the client area with a 1-pixel top inset (preserves Win11 rounded corners), then chains a `SetWindowSubclass` proc after winit's wndproc to intercept `WM_NCCALCSIZE`, `WM_NCHITTEST` (resize edges + drag region + button hit codes including `HTMAXBUTTON` for the snap-layout flyout), `WM_NCLBUTTONDOWN/UP` (post a `TitleBarSyntheticEvent` for button rects so the user's `on_tap` runs), `WM_NCMOUSEMOVE/LEAVE` (post `TitleBarHoverEvent`), `WM_DPICHANGED` (re-extends the DWM frame), and `WM_NCPAINT/NCACTIVATE` (suppress legacy caption-button artwork). Coexists with AccessKit's `WM_GETOBJECT` subclass via message disjointness.

`Widget::after_paint` is a deepest-first post-paint hook on `Widget` (gated on `wants_after_paint() == true` so non-overriders pay zero overhead); `TitleBar` overrides it to read its drag-region and three control-button bounds via the read-only `WidgetTreeView` and publish one complete `HitRegions` per frame. Replace-semantics on `update_hit_regions` is why aggregation has to happen in the parent — concurrent partial publishes from sibling controls would otherwise clobber each other.
