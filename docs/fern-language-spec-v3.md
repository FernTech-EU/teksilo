# The `fern!` Language Specification (v3)

**Status:** Design draft, revision 3
**Date:** April 17, 2026
**Companion to:** fern-ui-architecture.md §28.9
**Supersedes:** fern-language-spec-v2.md
...
The `fern!` macro is a thin syntactic transform. It is not a new runtime, not a new type system, and not a new reactivity model. Every `fern!` block desugars to a sequence of builder calls against the FernUI V2 API. There is no hidden allocation, no intermediate virtual tree, no diff step. The macro's only job is to remove syntactic noise from code that already expresses a widget tree.
...
Fourth, builder interop is symmetric. `fern!` expressions and builder chains can be freely nested in either direction. Neither is a superset of the other.

Fifth, the macro never introduces new capabilities. If a construct cannot be expressed by the V2 builder API, `fern!` will not invent a way to express it. Missing capabilities are fixed in the builder first, then surfaced in the DSL.

---

## 2. Lexical Structure

A `fern!` invocation takes one of two forms.

```rust
fern!(ctx => <root-element>)
fern!(<root-element>)
```
...
The binding is hoisted to the nearest enclosing statement-forming block (the `fern!` expansion here), where it remains in scope for the rest of the block. The container uses `.add_child(id)` to attach the bound element at its body position.
...
Every FernUI widget falls into one of two categories based on how it accepts content. The category determines which DSL form applies.
...
An `if/else` with two arms of different widget types desugars via `FernBranch<L, R>` to a type that implements `IntoFernChild` by dispatching to the active variant. Three- and four-way branches use `FernBranch3` and `FernBranch4`. Branches beyond four arms require explicit `Box<dyn Widget>`.

**Reactive conditionals.** If the condition is a bare identifier whose static type is `Signal<bool>` or `Prop<bool>`, the lowering is `.visible_when(signal)` on the child rather than an arena-level conditional. This is the one place where the macro performs type-directed inference. It is conservative: anything other than a bare identifier of the right type (a boolean expression, a function call, a dereferenced signal, an `if let`) falls into the regular `child_opt` / `FernBranch` path. To force the build-time conditional when the condition is a Signal identifier, write `if signal.get() { ... }`.
...
VStack::new()
    .child(match state {
        State::Loading => FernBranch3::A(Spinner::new()),
        State::Loaded(data) => FernBranch3::B(DataView::new(data.clone())),
        State::Error(msg) => FernBranch3::C(ErrorBanner::new(msg.clone())),
    })
```
...
**Expression-producing form.** The block ends with an expression without a trailing semicolon. The value is used as a child or spread across children via the `IntoFernChild` trait.
...
**Failure mode for forgotten `;`.** If the user writes a side-effect block without a trailing `;`, and the tail has type `()` (for example, a bare `if let { ...; }` without `else`), the macro treats it as expression form and tries to dispatch `()` through `IntoFernChild`. The compiler responds with "the trait `IntoFernChild` is not implemented for `()`" pointing at the block's tail expression. This is a survivable error: the message is clear and the fix is to add the missing `;`.
...
Each translation takes a block from one of the uploaded example files and shows the `fern!` equivalent against the actual post-refactor constructor and method names. Translations assume Appendix A has been applied.
...
fern!(
    TitleBar(host) {
        ...
        background: theme.colors.surface_pressed
        border: theme.colors.text_secondary, 2.0
        leading: TextWidget::new_literal("  FernUI — Title Bar Demo") {
            style: theme.typography.body_bold.clone()
            color: theme.colors.text_primary
        }
...
let root = fern!(ctx =>
    ScrollArea {
        ...
        VStack {
            ...
            TextWidget::new_literal("FernUI now resolves dialogs through a shared modal presentation pipeline, alongside anchored popovers and timed snackbars.") {
                style: t.body.clone()
                    ...
                    modal_trigger = Dialog::new_literal("Adaptive modal window") {
                        content: move || fern!(
                            DialogContent {
                                ...
                                body: TextWidget::new_literal("The app code does not branch on Wayland or window-system support here; it issues one modal request and lets FernUI resolve it.") {
                                    style: t.body.clone()
                                    color: c.text_secondary
                                }
...
Five things exercise the language here. First, `ScrollArea` is Category A post-refactor, so its VStack content is a body-block child. Second, the Dialog binding `modal_trigger =` uses the new assignment form to bind the dialog's id. Third, Dialog's `content:` property takes a `move ||` factory closure whose body contains a nested `fern!(...)` building the DialogContent. Fourth, DialogContent is Category B with `title_literal`, `supporting_text_literal`, `body`, and `footer` as slot properties. Fifth, `trigger:` in Popover takes a full element value, and all three trigger-like widgets (Popover, Snackbar, and Dialog as a button itself) appear at the same depth in the HStack.
...
impl Widget for App {
    fn build(&mut self, ctx: &mut BuildContext) -> Vec<WidgetId> {
        ...
        let label = self.write_signal.map(|text| format!("FernApp Widget Catalog {}", text));
...
`fern!` does not change this. Handlers written on an element attach via the builder methods of that element. Which attachment mechanism the builder uses internally is a per-widget implementation detail.

For the rarer case of attaching handlers to `self` inside a widget's own `build()` method, `fern!` is not the tool. That is infrastructure code that uses `HandlerSet` and `ctx.apply_self_handlers()` directly. `fern!` is for constructing trees, not for authoring internals.

---

## 9. Error Reporting Discipline

Every span the macro emits must be traceable to a user token. The `tr!` macro established the precedent: a missing translation key produces an error pointing at the key identifier. `fern!` adheres to the same discipline.
...
20  |     VStack { spacing: 8.0, TextWidget("x") }
    |                          ^
    |
    = help: fern! blocks separate items by newlines, not commas
...
**Hot-reload.** `fern!` expansions are Rust code. No runtime parser, no structural hot-reload. Translation hot-reload works through `--translation-dev`.
...
**Two-way binding syntax.** Two-way binding in FernUI is expressed by passing a `Signal<T>` to a widget's bind method; the widget commits changes back through its event handlers. No `:=` form.
...
The macro lives in a new crate `fern-ui-macros`, exported through the `fern_ui` umbrella as `fern_ui::fern!`. Four responsibilities.
...
**IR construction** produces a typed tree: `FernElement`, `FernProperty`, `FernBinding`, `FernStructural`, `FernSpread`, `FernLet`, `FernEscape`, `FernRust`. One IR node per grammar production.
...
**Supporting types.** `FernBranch<L, R>`, `FernBranch3`, `FernBranch4`, `IntoFernChild` live in `fern-core::widget_builder` as public types. They are not DSL-specific: hand-written builder chains can use them.
...
**Test strategy.** `trybuild` fixtures for every error class in §9. Golden-file tests for translation rules using `cargo expand`. Integration tests rewriting the seven uploaded examples to `fern!` form and verifying rendered output is bitwise identical.

---

## 12. Summary

The `fern!` language is a block-structured DSL for FernUI widget trees. It reads like QML or Kotlin, compiles to V2 builder calls with no runtime overhead, preserves existing reactivity and capture semantics without new syntax, and produces user-facing error spans.

The grammar has three primary forms: elements with explicit constructors, bindings via `name = Element`, and properties including named slots; structural control flow (`if`, `for`, `match`, `let`, `..spread`, `rust { }`); and one escape hatch (`#{ expr }`). Each form has a mechanical desugaring into existing FernUI infrastructure.
...
### A.4 FernBranch Types and IntoFernChild Trait

Add to `fern-core::widget_builder`:

```rust
pub enum FernBranch<L: Widget, R: Widget> { L(L), R(R) }
pub enum FernBranch3<A: Widget, B: Widget, C: Widget> { A(A), B(B), C(C) }
pub enum FernBranch4<A, B, C, D> { ... }
...
Category C dissolution: 4 widgets, roughly 20 lines of change each. Method renames: 5 widgets, roughly 3 lines each. Id-taking twins: 8 widgets, roughly 30 new methods total at 3 lines each. FernBranch infrastructure: 1 new file, roughly 200 lines including the impls.
