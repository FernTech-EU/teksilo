...
- **Outbound drag (Bastyde → other app).** Dragging *out* of a Bastyde window to an external target (e.g. `NSDraggingSource`) is still pending; the payload + handler APIs are stable.
...
- **External (OS) DnD backends.** *Inbound* OS drops are done via `ExternalDndBackend` (`install_external_dnd()`): macOS `NSDraggingDestination` (verified), Windows OLE `IDropTarget`, Wayland `wl_data_device`, X11 no-op. Files / text / URLs flow into the in-app pipeline as an external `DragPayload`; the `DropZone` widget consumes them. *Outbound* drag (Bastyde → other app, `NSDraggingSource` et al.) is still pending — payload + handler APIs are stable.
...
| 6 | Data-Driven Collections + Drag & Drop | ✅ Largely done | bastyde-data crate, ListModel/TreeModel/TreeSlice/SelectionModel, Repeater/ListView/TreeView with virtualization, typed DnD with preview overlay. Inbound external (OS) drops done via `ExternalDndBackend` + `DropZone` (macOS verified; Win/Wayland cfg-gated; X11 no-op). Outbound drag remaining. |
