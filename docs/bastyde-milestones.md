...
- (Done, inbound) **External (OS) drop backends in bastyde-platform.** The `ExternalDndBackend` trait + `ExternalDndHandle` (`install_external_dnd()`) deliver OS drops — files / text / URLs from a file manager or another app — into the in-app drag pipeline. macOS (`NSDraggingDestination` overlay) is verified; Windows (OLE `RegisterDragDrop`/`IDropTarget`) and Wayland (`wl_data_device`) ship cfg-gated; X11 is a documented no-op (the `DropZone` Browse button keeps it usable). The ready-made `DropZone` widget consumes them. Demo: `cargo run -p file-drop`.
- (Done) **Outbound drag (Bastyde → other app).** Dragging *out* of a Bastyde window into another application. Escalate-at-boundary: a `start_drag` whose payload carries MIME auto-escalates to a native OS drag when the pointer leaves the window (macOS `NSDraggingSource`, Wayland `wl_data_source` — both verified; Windows/X11 decline). Completion via `on_drag_ended(DropOutcome)`; typed payload recovered on re-entry, enabling DnD between two windows of the same app. Copy-only (no move-out). See [drag-and-drop.md §11.5](drag-and-drop.md).
...
| 6 | Data-Driven Collections + Drag & Drop | ✅ Largely done | bastyde-data crate, ListModel/TreeModel/TreeSlice/SelectionModel, Repeater/ListView/TreeView with virtualization, typed DnD with preview overlay. Inbound external (OS) drops done via `ExternalDndBackend` + `DropZone` (macOS verified; Win/Wayland cfg-gated; X11 no-op). Outbound (app → OS) export done (macOS + Wayland verified; Win/X11 decline) with typed re-entry / cross-window DnD. |
