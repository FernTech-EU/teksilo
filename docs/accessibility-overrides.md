...
HStack::new()
    ...
    .child(TextWidget::new(lit!("Teksilo")))
    .child(TextWidget::new(lit!("Pure-Rust GUI")))
    .access_label(lit!("Teksilo logo"))
    .access_exclude_subtree();
```
...
A widget that is only chrome around content a reader needs (a panel, a frame a style draws, a layout shell, a row wrapper) publishes a bare `Role::GenericContainer` instead. The walker prunes it (see [Automatic presentational collapse](#automatic-presentational-collapse-no-opt-in)) and, if something keeps it in the update, the adapters still drop it and promote its children (`ExcludeNode`). Every style protocol that wraps content (`DialogStyle::make_panel`, and `make_body` on `CardStyle`, `PanelStyle`, `SnackbarStyle`, `ToastStyle`, `PopoverStyle` for its `Menu` variant and `RadioTileStyle`) says so, and `accessibility::audit::focusable_nodes_hidden` flags a control that takes focus inside a hidden subtree, which is how the mistake shows.

Under a live region the wrapper has one more thing to say. A node's politeness is inherited by every descendant that sets none (`accesskit_consumer` node.rs:906-910), and the AT-SPI, UIA and macOS adapters all announce each named node that enters the filtered tree, or is renamed there, with a politeness other than off. So a region meant to be announced by its own name (a toast, a snackbar, a banner, a message box, the calendar grid, whose name is the month) is announced once for its name and once more for every named node its content holds, the moment that content can be reached. Orca speaks each announcement with interrupt set, so what is left at the end is whichever came last in the consumer's hash order. A hidden wrapper used to stop this by hiding the content; the wrapper that replaces it gives its content `Live::Off` instead: a `GenericContainer` with `Live::Off` is kept in the update, where its descendants inherit the `Off`, and the adapters still drop it and promote its children. `SnackbarStyle::make_body` and `ToastStyle::make_body` say so. A region whose content is meant to be heard as it changes, a `StatusBar` with `announce_changes(true)`, keeps the inheritance.
