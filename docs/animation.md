...
The reference for this offscreen-render pattern is [`png_export.rs`](../crates/fern-preview-ui/src/png_export.rs), which has been creating intermediate `RENDER_ATTACHMENT | COPY_SRC` textures and routing the renderer at them since the widget previewer shipped — the blur engine generalises that pattern into a recursive sub-pass.
