...
- **Default context menu.** The editor exposes [`context_target_at(point)`](../crates/fern-widgets/src/rich_text.rs) so applications can build their own right-click menu; a built-in menu with `Cut / Copy / Paste / Paste Unformatted / Select All` is intentionally deferred until the intent-dispatch + overlay-dismissal ordering is reworked in fern-core (dismiss runs before `drain_pending_intents`, which made the naive approach drop every menu-item click).
