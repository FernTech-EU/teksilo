...
and each row is a `Role::ListBoxOption` with its own `position_in_set`. The
arrows move a highlight and commit nothing, so `on_select` runs once, on
Enter: the highlighted row is the one `selected` option and the
`active_descendant` of the search field that keeps focus, which every platform
adapter follows as the focus, so a screen reader names each family as the
arrows reach it. Typing in the search field drops the highlight and hands the
