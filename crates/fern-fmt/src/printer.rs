...
pub fn print(source: &str, root: &FernRoot, trivia: &[Trivia]) -> String {
    let mut p = Printer {
        source,
        trivia,
        trivia_idx: 0,
...
impl<'a> Printer<'a> {
    ...
    fn print_root(&mut self, root: &FernRoot) {
    ...
    fn print_element(&mut self, e: &FernElement) {
    ...
    fn print_body_item(&mut self, item: &BodyItem) {
        match item {
            ...
            BodyItem::If(if_) => {
                self.write_verbatim_multiline(&fern_if_token_stream(if_));
                let r = item_byte_range(item);
                self.cursor = self.cursor.max(r.end);
            }
            BodyItem::Match(m) => {
                self.write_verbatim_multiline(&fern_match_token_stream(m));
                let r = item_byte_range(item);
                self.cursor = self.cursor.max(r.end);
            }
            BodyItem::For(f) => {
                self.write_verbatim_multiline(&fern_for_token_stream(f));
                let r = item_byte_range(item);
                self.cursor = self.cursor.max(r.end);
            }
    ...
    fn print_property(&mut self, p: &fern_parse::FernProperty) {
...
fn fern_if_token_stream(if_: &FernIf) -> TokenStream {
...
fn fern_match_token_stream(m: &FernMatch) -> TokenStream {
    let mut ts = TokenStream::new();
...
fn fern_for_token_stream(f: &FernFor) -> TokenStream {
    let mut ts = TokenStream::new();
...
fn body_item_to_tokens(item: &BodyItem, ts: &mut TokenStream) {
    match item {
        ...
        BodyItem::If(if_) => ts.extend(fern_if_token_stream(if_)),
        BodyItem::Match(m) => ts.extend(fern_match_token_stream(m)),
        BodyItem::For(f) => ts.extend(fern_for_token_stream(f)),
