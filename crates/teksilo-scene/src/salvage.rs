...
//! The **owning salvage** door: everything [`Scene::remove`](crate::Scene::remove)
//! destroys, moved out instead of dropped.
//!
//! # Why a second channel exists at all
//!
//! [`ItemChange`](crate::ItemChange) travels through a `Signal`, and
...
//!
//! That door is [`Scene::take`](crate::Scene::take) (the caller keeps the
//! salvage) and the [`SceneTransactionRecord`](crate::SceneTransactionRecord)
//! delivered to the edit sink (the framework hands it over). Both produce
//! [`RemovedItem`]s; [`Scene::restore`](crate::Scene::restore) puts one back.
//!
...
use crate::a11y::{A11yCategory, A11yNode, A11yRelation};
use crate::flags::ItemFlags;
use crate::item::{ItemId, SceneItem};
use crate::magnet::{Magnet, MagnetId};
use crate::scene::{SceneEntry, SceneEntryKind, SceneLayer, WidgetSource};

/// The per-item slice of the **logical accessibility tree**, harvested by
/// [`Scene::take`](crate::Scene::take) and re-attached by
...
pub struct ItemA11yDecorations {
    /// The node this item was AT-parented under (`Scene::set_a11y_parent`).
    pub parent: Option<A11yNode>,
    /// Nodes that were AT-parented **under** this item and survived its
    /// removal. `remove` re-roots them at the view root; a restore re-adopts
    /// each one that still exists.
    pub adopted: Vec<A11yNode>,
    /// Every relation with this item at *either* endpoint, in declaration
    /// order. A relation between two removed items is recorded once, on the
    /// `from` endpoint's salvage.
    pub relations: Vec<(A11yNode, A11yRelation, A11yNode)>,
    /// Live-region politeness (`Scene::set_a11y_live`).
    pub live: Option<accesskit::Live>,
    /// Landmark role (`Scene::set_a11y_landmark`).
    pub landmark: Option<accesskit::Role>,
    /// Rotor / quick-nav categories (`Scene::set_a11y_categories`).
    pub categories: Vec<A11yCategory>,
}

impl ItemA11yDecorations {
...
/// One item lifted whole out of a [`Scene`](crate::Scene): its entry, its
...
/// missed by [`Scene::restore`](crate::Scene::restore) — the same argument the
/// single-construction-site rule makes for insertion.
///
/// # Ordering
///
/// [`Scene::take`](crate::Scene::take) returns a subtree deepest-first, the
/// order `remove` already announces in.
/// [`Scene::restore_all`](crate::Scene::restore_all) reverses it for you;
/// restoring by hand means roots first, or a child hits
/// [`RestoreError::MissingParent`].
pub struct RemovedItem {
    pub(crate) entry: SceneEntry,
    /// Where the entry sat in the scene's **declaration order** — the order
    /// the accessibility walk publishes siblings in, i.e. the order a screen
    /// reader reads the scene in. A restore puts the entry back at this index
    /// when it still exists, so an undone deletion does not silently move the
    /// item to the end of the reading order.
    pub(crate) entry_order: usize,
    pub(crate) magnets: Vec<(MagnetId, Magnet)>,
    pub(crate) a11y: ItemA11yDecorations,
}

impl std::fmt::Debug for RemovedItem {
    /// Hand-written: `SceneEntry` holds a `Box<dyn SceneItem>` **or** a
    /// `Box<dyn Widget>` plus an `Option<Box<SceneItemHandlerSet>>`, and
    /// `Magnet` is `Clone` only — none of the three is `Debug`, so this cannot
    /// be derived.
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RemovedItem")
            .field("id", &self.entry.id)
            .field("kind", &if self.is_widget() { "widget" } else { "item" })
            .field("local_pos", &self.entry.local_pos)
            .field("local_bounds", &self.entry.local_bounds)
            .field("z", &self.entry.z)
            .field("layer", &self.entry.layer)
            .field("parent", &self.entry.parent)
            .field("children", &self.entry.children)
            .field("entry_order", &self.entry_order)
            .field("magnets", &self.magnets.len())
...
impl RemovedItem {
    ...
    /// [`Scene::restore`](crate::Scene::restore) puts it back at.
    ...
    pub fn local_pos(&self) -> Point {
        self.entry.local_pos
    }

    /// AABB in local coordinates.
    pub fn local_bounds(&self) -> Rect {
        self.entry.local_bounds
    }
    ...
    /// coordinates. Use
    /// [`Scene::reparent_keeping_scene_pos`](crate::Scene::reparent_keeping_scene_pos)
    ...
    /// and all of them present in the same [`Scene::take`](crate::Scene::take)
    /// result.
    ...
    ///
    /// So [`Scene::restore`](crate::Scene::restore) always accepts, and this
    ...
    pub fn widget_instance_present(&self) -> bool {
        match &self.entry.kind {
            SceneEntryKind::Widget(WidgetSource::Once(slot)) => slot.is_some(),
            _ => true,
        }
    }
...
/// Why [`Scene::restore`](crate::Scene::restore) could not put a salvage back.
///
...
pub enum RestoreError {
    /// An entry with this id is already in the scene. Restoring over it would
    /// alias two items onto one identity.
    ///
    /// **Defensive.** [`Scene::take`](crate::Scene::take) retires the id, so a
    /// second salvage for it cannot exist while the first is held, and the
    /// restore that succeeds consumes the one that does — which makes this
    /// unreachable through the public API today. It stays because it is the
    /// only thing standing between a future door that breaks that invariant and
    /// a corrupted `entry_index`, and refusing is better than aliasing.
    #[error(
        "cannot restore {0:?}: an item with that id is already in the scene. A salvage \
         may only be restored once, and only while its id is free."
    )]
    IdAlreadyLive(ItemId),
    /// The parent the item was removed from is not in the scene. Restore
    /// roots-first — [`Scene::restore_all`](crate::Scene::restore_all) orders a
    /// whole `take` result for you — or clear the recorded parent with
    /// [`RemovedItem::detach`] before restoring, which is what a move into a
    /// *different* scene normally wants.
    ///
    /// The salvage is consumed by the refused call, so a caller that may hit
    /// this checks `parent()` (or calls `detach()`) **before** handing it over.
    #[error(
        "cannot restore {id:?}: its recorded parent {parent:?} is not in the scene. \
         Restore roots before their children (Scene::restore_all does that for a \
         whole Scene::take result)."
    )]
    MissingParent {
        /// The item that could not be restored.
        id: ItemId,
        /// The parent it names, which is absent.
        parent: ItemId,
    },
}

/// A refused [`Scene::replace_item`](crate::Scene::replace_item): why, and the
...
pub struct ReplaceRejected {
    ...
    #[source]
    pub error: ReplaceItemError,
    /// The item that was not inserted, handed straight back.
    pub item: Box<dyn SceneItem>,
}

/// Why [`Scene::replace_item`](crate::Scene::replace_item) could not swap an
...
pub enum ReplaceItemError {
    ...
    #[error("cannot replace {0:?}: no such item in the scene")]
    UnknownItem(ItemId),
    /// The entry is a heavyweight `Widget`, which holds no
    /// [`SceneItem`] box. Replace its *data* with
    /// [`SceneModel::set_payload`](crate::SceneModel::set_payload) instead.
    #[error(
        "cannot replace {0:?}: it is a heavyweight widget entry, not a lightweight \
         item. Use SceneModel::set_payload to swap a Delegated entry's data."
    )]
    NotLightweight(ItemId),
}
