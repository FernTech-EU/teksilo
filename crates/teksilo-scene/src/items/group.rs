    ...
    fn paint(&self, canvas: &mut Canvas, ctx: &SceneItemPaintContext<'_>) {
        ...
        if let Some(prop) = &self.fill {
            ...
            if self.corner_radius > 0.0 {
                canvas.fill_rounded_rect(
                    lb,
                    teksilo_tokens::CornerRadius::uniform(self.corner_radius),
                    fill,
                );
            } else {
            ...
            && let Some(label) = &self.label
        {
            ...
            canvas.draw_text(
                label,
                label_bounds,
                &teksilo_tokens::TextStyle::default(),
                color,
            );
        }
    }
...
mod tests {
    ...
    fn group_item_role_fill_resolves_against_theme() {
        ...
        let ctx = SceneItemPaintContext::new(Transform2D::identity(), None, &theme);
        let mut c = teksilo_canvas::Canvas::new();
        g.paint(&mut c, &ctx);
        assert!(
            c.into_render_frame()
                .decorations
