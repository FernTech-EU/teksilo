...
//! [`Color`], a theme role
//! ([`TextRole`](teksilo_tokens::TextRole)), a reactive `Signal<Color>`, or a
...
///
/// A scale never reflows scene text — one logical layout is painted
/// larger — so the scene-space extents AT reads are the layout's own,
/// scaled by the item's own `local_to_scene`.
...
impl TextItem {
    ...
    /// at the authored [`TextStyle`](teksilo_tokens::TextStyle). Lets a
...
impl SceneItem for TextItem {
    ...
    fn paint(&self, canvas: &mut Canvas, ctx: &SceneItemPaintContext<'_>) {
        let text = self.text.current();
        let mut style = teksilo_tokens::TextStyle::default();
...
mod tests {
    use super::*;
    use teksilo_canvas::{DrawCommand, Point};
    use teksilo_i18n::lit;
    ...
    fn ctx<'a>(theme: &'a teksilo_core::styles::Theme) -> SceneItemPaintContext<'a> {
