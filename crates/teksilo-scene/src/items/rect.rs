...
//! [`Color`](teksilo_tokens::Color), a theme role
//! ([`SurfaceRole`](teksilo_tokens::SurfaceRole) / `TextRole` / `BorderRole`),
...
impl RectItem {
    ...
    /// Fill colour. Accepts a plain [`Color`](teksilo_tokens::Color), a theme
...
impl SceneItem for RectItem {
    ...
    fn thumbnail_color(&self) -> teksilo_tokens::Color {
        ...
        crate::items::fill_or_stroke_hint(self.fill.as_ref(), self.stroke.as_ref())
            .unwrap_or_else(|| teksilo_tokens::Color::new(0.6, 0.6, 0.6, 1.0))
    }
...
mod tests {
    use super::*;
    use teksilo_canvas::{Canvas, Point, Transform2D};
    use teksilo_core::signal::Signal;
    use teksilo_tokens::{Color, SurfaceRole};

    fn test_ctx<'a>(theme: &'a teksilo_core::styles::Theme) -> SceneItemPaintContext<'a> {
