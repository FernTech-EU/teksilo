...
fn idle_drain_zero_frames_at_rest() {
    ...
    // teksilo-scene-specific contract is: no animation scheduler
    ...
    assert!(
        ...
        "teksilo-scene must not call request_frame() at rest"
    );
    assert_eq!(
...
fn panned_scene_viewport_is_fully_hittable() {
    ...
    view_handle(&tree, view_id).set_pan(teksilo_canvas::Vec2::new(50.0, 30.0));
    tree.layout(SizeProposal::exact(200.0, 100.0));
    ...
    for p in [
        ...
        assert_eq!(
            tree.hit_test(teksilo_canvas::Point::new(p.0, p.1)),
            Some(view_id),
            "viewport point {p:?} must hit the panned SceneView",
        );
    }
    // Outside the viewport: no hit.
    assert_eq!(tree.hit_test(teksilo_canvas::Point::new(250.0, 50.0)), None);
}
...
fn ctrl_wheel_zooms_about_cursor_keeping_scene_anchor_fixed() {
    ...
    // viewport center.
    use teksilo_core::event::{Modifiers, WidgetEvent as Ev};
...
fn scene_view_emits_synthetic_at_node_per_visible_item() {
    ...
    use crate::items::RectItem;
    use teksilo_core::accessibility::{SyntheticKind, is_synthetic, synthetic_node_id};
    use teksilo_tokens::Color;
...
fn set_a11y_parent_reparents_item_under_group() {
    ...
    let scene_view_node_id = teksilo_core::accessibility::widget_id_to_node_id(view_id);
...
fn heavyweight_and_lightweight_project_identically_in_an_offset_view() {
    ...
    use crate::items::RectItem;
    use teksilo_canvas::{Canvas, Size};
...
fn heavyweight_and_lightweight_project_identically_at_zoom() {
    ...
    use teksilo_core::widget::PaintContext;
    use teksilo_tokens::Color;
...
fn scene_view_threads_theme_into_item_paint() {
    ...
    let expected =
        ColorProp::from(SurfaceRole::Sunken).resolve(&teksilo_core::presets::intui::light(), true);
...
fn cooperative_default_emits_items_at_root_when_unparented() {
    ...
    use crate::items::RectItem;
    use teksilo_core::accessibility::{is_synthetic, widget_id_to_node_id};
...
fn marquee_drag_ends_with_pending_commit() {
    ...
    let mut scene = Scene::new();
    scene.add_item(
        RectItem::new(Rect::new(50.0, 50.0, 20.0, 20.0)).fill(teksilo_tokens::Color::RED),
        Point::ZERO,
    );
...
fn marquee_drag_records_pending_commit() {
    ...
    let outside = scene.add_item(
        RectItem::new(Rect::new(500.0, 500.0, 20.0, 20.0)).fill(teksilo_tokens::Color::BLUE),
        Point::ZERO,
    );
...
fn looping_item_animation_survives_drag_end_rebuild() {
    ...
    impl SceneItem for Looper {
        ...
        fn paint(&self, _c: &mut teksilo_canvas::Canvas, _x: &SceneItemPaintContext<'_>) {}
...
fn drag_in_empty_area_starts_marquee_not_move() {
    ...
    let mut scene = Scene::new();
    let item_id = scene.add_item(
        RectItem::new(Rect::new(100.0, 100.0, 30.0, 30.0)).fill(teksilo_tokens::Color::RED),
        Point::ZERO,
    );
...
fn z_order_paints_higher_z_after_lower() {
    ...
    let mut scene = Scene::new();
    let a = scene.add_item(
        RectItem::new(Rect::new(10.0, 10.0, 20.0, 20.0)).fill(teksilo_tokens::Color::RED),
        Point::ZERO,
    );
    let b = scene.add_item(
        RectItem::new(Rect::new(15.0, 15.0, 20.0, 20.0)).fill(teksilo_tokens::Color::BLUE),
        Point::ZERO,
    );
    ...
    let red_first = frame.decorations.iter().enumerate().find_map(|(i, d)| {
        if d.color == teksilo_tokens::Color::RED.to_array() {
            Some(i)
        } else {
            None
        }
    ...
    let blue_first = frame.decorations.iter().enumerate().find_map(|(i, d)| {
        if d.color == teksilo_tokens::Color::BLUE.to_array() {
            Some(i)
        } else {
            None
        }
...
fn drag_mode_no_drag_disables_marquee_and_drag_to_move() {
    ...
    let id = scene.add_item(
        RectItem::new(Rect::new(0.0, 0.0, 30.0, 30.0))
            .fill(teksilo_tokens::Color::RED)
            .draggable(true),
        teksilo_canvas::Point::new(10.0, 10.0),
    );
    let mut tree = WidgetTree::new();
    let view_id = tree.add(
        SceneView::new(scene)
    ...
    assert_eq!(
        ...
        Some(teksilo_canvas::Point::new(10.0, 10.0))
    );
}
...
fn cache_mode_item_coordinate_avoids_repeat_paints() {
    ...
    impl SceneItem for CachedRect {
        ...
        fn paint(&self, canvas: &mut teksilo_canvas::Canvas, _ctx: &SceneItemPaintContext<'_>) {
            self.count.set(self.count.get() + 1);
            canvas.fill_rect(self.bounds, teksilo_tokens::Color::new(1.0, 0.0, 0.0, 1.0));
        }
...
fn item_cache_clears_on_glyph_epoch_change() {
    ...
    impl teksilo_canvas::TextBackend for EpochBackend {
...
fn cache_evicts_on_invalidate() {
    ...
    impl SceneItem for CachedRect {
        ...
        fn paint(&self, canvas: &mut teksilo_canvas::Canvas, _ctx: &SceneItemPaintContext<'_>) {
            self.count.set(self.count.get() + 1);
            canvas.fill_rect(self.bounds, teksilo_tokens::Color::new(0.0, 0.5, 1.0, 1.0));
        }
        fn cache_mode(&self) -> CacheMode {
            CacheMode::ItemCoordinate
        }
...
struct TransformRecorder {
    bounds: Rect,
    captured: std::rc::Rc<std::cell::Cell<Option<teksilo_canvas::Transform2D>>>,
}
...
impl crate::item::SceneItem for TransformRecorder {
    ...
    fn paint(
        ...
        canvas.fill_rect(self.bounds, teksilo_tokens::Color::new(0.5, 0.5, 0.5, 1.0));
    }
}
...
fn ignores_xform_debug_overlay_paints_screen_anchored_bounds() {
    ...
    let item_color_arr = teksilo_tokens::Color::new(0.20, 0.75, 0.35, 0.85).to_array();
...
fn group_item_logical_only_dispatch_passes_through_to_item_beneath() {
    ...
    let inner_item =
        RectItem::new(Rect::new(0.0, 0.0, 30.0, 30.0)).fill(teksilo_tokens::Color::RED);
...
fn group_item_visual_dispatch_uses_aabb_as_before() {
    ...
    let group = GroupItem::new(Rect::new(0.0, 0.0, 100.0, 100.0)).fill(teksilo_tokens::Color::BLUE);
...
fn item_thumbnails_returns_visible_lightweight_items_with_colors() {
    ...
    let blue = scene.add_item(
        RectItem::new(Rect::new(0.0, 0.0, 40.0, 50.0)).fill(teksilo_tokens::Color::BLUE),
        Point::new(100.0, 100.0),
    );
    ...
    assert_eq!(color_red, teksilo_tokens::Color::RED);
    let (rect_blue, color_blue) = thumbs[1];
    assert_eq!(rect_blue, Rect::new(100.0, 100.0, 40.0, 50.0));
    assert_eq!(color_blue, teksilo_tokens::Color::BLUE);
    let _ = (red, blue);
}
...
fn on_tap_event_receives_modifiers_and_button() {
    ...
    assert_eq!(ev.button, teksilo_core::event::PointerButton::Primary);
...
fn accept_tap_buttons_gates_middle_click() {
    ...
    let buttons: Rc<RefCell<Vec<teksilo_core::event::PointerButton>>> =
    ...
    // PRIMARY hits.
    click(&mut tree, teksilo_core::event::PointerButton::Primary);
    // MIDDLE is gated out by the default mask.
    click(&mut tree, teksilo_core::event::PointerButton::Middle);
    assert_eq!(
        buttons.borrow().clone(),
        vec![teksilo_core::event::PointerButton::Primary]
    );
    ...
    // Now widen the mask: PRIMARY | MIDDLE.
    {
        ...
        view.scene_mut()
            ...
            .unwrap()
            .accept_tap_buttons(
                teksilo_core::event::ButtonMask::PRIMARY | teksilo_core::event::ButtonMask::MIDDLE,
            );
    }
...
fn item_thumbnails_skips_invisible_and_logical_items() {
    ...
    assert_eq!(thumbs[0].1, teksilo_tokens::Color::RED);
    let _ = visible;
}
