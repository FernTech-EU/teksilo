...
fn runtime_added_widget_emits_draw_commands() {
    ...
    impl Widget for PaintCard {
        ...
        fn paint(&self, bounds: Rect, canvas: &mut teksilo_canvas::Canvas, _: &PaintContext) {
...
fn heavyweight_child_far_in_scene_renders_when_panned_into_view() {
    ...
    fn red_fill_count(frame: &teksilo_canvas::RenderFrame) -> usize {
        frame
            .decorations
            .iter()
            .filter(|d| d.color == teksilo_tokens::Color::RED.to_array())
            .count()
    }
...
#[derive(Debug)]
struct DynRect {
    size: teksilo_core::signal::Signal<f32>,
}
impl crate::item::SceneItem for DynRect {
    ...
    fn paint(&self, _: &mut teksilo_canvas::Canvas, _: &crate::item::SceneItemPaintContext<'_>) {}
