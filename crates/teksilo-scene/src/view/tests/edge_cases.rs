...
fn rect_item(w: f32, h: f32) -> RectItem {
    RectItem::new(Rect::new(0.0, 0.0, w, h)).fill(teksilo_tokens::Color::RED)
}
...
#[derive(Debug)]
struct ColorBox(teksilo_tokens::Color);

impl teksilo_core::widget::Widget for ColorBox {
    fn layout_response(
        &self,
    ...
    fn paint(
        ...
        canvas.fill_rounded_rect(bounds, teksilo_tokens::CornerRadius::uniform(0.0), self.0);
    }
}
...
fn over_band_paints_after_heavyweight_under_band_before() {
    ...
    let mut scene = Scene::new();
    scene.add_item(
        RectItem::new(Rect::new(0.0, 0.0, 20.0, 20.0)).fill(teksilo_tokens::Color::RED),
        Point::new(10.0, 10.0),
    );
    scene.add_widget(
        ColorBox(teksilo_tokens::Color::GREEN),
...
fn find_color(frame: &teksilo_canvas::RenderFrame, dominant: usize) -> Option<usize> {
    frame.draw_order.iter().position(|cmd| {
        let c = match cmd {
            teksilo_canvas::DrawCommand::Shape(i) => frame.shapes.get(*i).map(|s| s.color),
            teksilo_canvas::DrawCommand::Decoration(i) => {
...
fn heavyweight_z_order_restacks_cards() {
    ...
    let a = scene.add_widget(
        ColorBox(teksilo_tokens::Color::RED),
