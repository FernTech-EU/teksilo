...
//!
//! Bindings anywhere in the tree hoist to the outermost bati! block
...
pub(crate) fn lower_root(root: &BatiRoot) -> TokenStream2 {
...
fn lower_body_attach(
    ...
    match item {
        ...
        BodyItem::If(bastyde_if) => lower_if_attach(bastyde_if, ctx_tok, hoisted),
        BodyItem::Match(bastyde_match) => lower_match_attach(bastyde_match, ctx_tok, hoisted),
        BodyItem::For(bastyde_for) => lower_for_attach(bastyde_for, ctx_tok, hoisted),
        BodyItem::Let(_)
        | BodyItem::Rust {
            shape: RustShape::SideEffect,
            ..
...
fn lower_if_attach(
    ...
    let (arm_count, has_final_else) = count_if_arms(bastyde_if);

    if arm_count == 1 && !has_final_else {
        let cond = &bastyde_if.cond;
        let then_expr = lower_element(&bastyde_if.then, ctx_tok, hoisted)?;
        return Ok(quote_spanned! { bastyde_if.span =>
    ...
    if arm_count > 4 {
        return Err(diag::error(
            bastyde_if.span,
            "bati! supports up to 4 if-chain arms; wrap deeper chains in `Box<dyn Widget>` or \
             split into a helper",
        ));
    }

    let branch_path = bastyde_branch_path(arm_count);
    let branch_expr =
        lower_if_chain_as_branch(bastyde_if, ctx_tok, hoisted, arm_count, 0, &branch_path)?;
    Ok(quote_spanned! { bastyde_if.span =>
        .child(#branch_expr)
    })
}

fn count_if_arms(if_expr: &BatiIf) -> (usize, bool) {
    let mut count = 1;
    let mut has_final_else = false;
    let mut cursor = &if_expr.else_branch;
    while let Some(boxed) = cursor {
        match &**boxed {
            BatiElse::ElseIf(nested) => {
                count += 1;
...
fn lower_if_chain_as_branch(
    ...
    let else_tail = match bastyde_if.else_branch.as_deref() {
        Some(BatiElse::ElseIf(nested)) => {
...
fn lower_match_attach(
    bastyde_match: &BatiMatch,
    ctx_tok: &TokenStream2,
    hoisted: &mut Vec<TokenStream2>,
) -> Result<TokenStream2, syn::Error> {
    let arm_count = bastyde_match.arms.len();
    if arm_count < 2 {
        return Err(diag::error(
            bastyde_match.span,
    ...
    if arm_count > 4 {
        return Err(diag::error(
            bastyde_match.span,
            "bati! supports up to 4 match arms; wrap deeper dispatches in `Box<dyn Widget>` or \
             split into a helper",
        ));
    }

    let branch_path = bastyde_branch_path(arm_count);
    let scrutinee = &bastyde_match.scrutinee;

    let mut lowered_arms = Vec::new();
    for (idx, arm) in bastyde_match.arms.iter().enumerate() {
    ...
    Ok(quote_spanned! { bastyde_match.span =>
...
fn lower_for_attach(
    ...
    let pat = &bastyde_for.pat;
    let iter = &bastyde_for.iter;
    let lets = &bastyde_for.lets;
    let elem_expr = lower_element(&bastyde_for.element, ctx_tok, hoisted)?;
    Ok(quote_spanned! { bastyde_for.span =>
...
fn bastyde_branch_path(arm_count: usize) -> TokenStream2 {
    let root = crate::bastyde_core_root();
    match arm_count {
        2 => quote!(#root::BatiBranch),
        3 => quote!(#root::BatiBranch3),
        4 => quote!(#root::BatiBranch4),
        _ => unreachable!("bastyde_branch_path called with unsupported arm count {arm_count}"),
    }
}
