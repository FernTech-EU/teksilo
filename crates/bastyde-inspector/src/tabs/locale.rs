...
//! (read via `bastyde_i18n::current_supported_locales()`), one per row.
...
use bastyde_tokens::{Color, CornerRadius, TextRole};
...
impl Widget for LocaleTab {
    ...
    fn layout_response(&self, proposal: SizeProposal, _ctx: &LayoutContext) -> LayoutResponse {
        let supported = bastyde_i18n::current_supported_locales().unwrap_or_default();
        let active = bastyde_i18n::current_locale().map(|s| s.get().to_string());
