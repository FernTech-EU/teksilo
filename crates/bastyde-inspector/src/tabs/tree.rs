...
use bastyde_tokens::{Color, TextRole};
use bastyde_widgets::primitives::{Expand, Padding, VStack};
use bastyde_widgets::{ScrollArea, TextInput};
    ...
    fn paint(&self, bounds: Rect, canvas: &mut Canvas, ctx: &PaintContext) {
        ...
        for (i, row) in self.rows.borrow().iter().enumerate() {
            ...
            if Some(row.id) == selected_id {
                ...
                canvas.fill_rounded_rect(row_rect, bastyde_tokens::CornerRadius::ZERO, bg);
            }
