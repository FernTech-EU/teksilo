...
/// primitives all live under `bastyde_widgets::primitives::`, so a
...
impl Widget for HighlightLayer {
    ...
    fn paint(&self, bounds: Rect, canvas: &mut Canvas, ctx: &PaintContext) {
            ...
            && rect.height > 0.0
        {
            ...
            canvas.stroke_rounded_rect(rect, bastyde_tokens::CornerRadius::ZERO, stroke, 2.0);
        }
    }
...
fn paint_hover_tooltip(
    ...
    canvas.fill_rounded_rect(bg_rect, bastyde_tokens::CornerRadius::uniform(3.0), bg);
    canvas.draw_text(
        &label,
        Rect::new(
            x + PADDING_X,
...
fn paint_bands(canvas: &mut Canvas, state: &InspectorState, opacity: f32) {
    ...
    for band in bands.iter() {
        ...
        canvas.fill_rounded_rect(band.bounds, bastyde_tokens::CornerRadius::ZERO, color);
    }
}
...
fn paint_all_bounds(
    ...
    for entry in snapshot.iter() {
        ...
        canvas.stroke_rounded_rect(entry.bounds, bastyde_tokens::CornerRadius::ZERO, color, 1.0);
    }
}
