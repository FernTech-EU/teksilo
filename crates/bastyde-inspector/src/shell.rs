...
impl Widget for InspectorShell {
    fn build(&mut self, ctx: &mut BuildContext) -> Vec<WidgetId> {
        ...
        let panel_switcher = PanelShortcutHost::new(
            state.clone(),
            bastyde_widgets::primitives::Switcher::new(panel_index)
                .child(empty_filler())
                .child(panel_block),
        );
    ...
    fn place_children(
        ...
        for child in children.iter_mut() {
            ...
            child.size = bastyde_canvas::Size::new(bounds.width, bounds.height);
        }
    }
...
fn build_panel(state: InspectorState) -> impl Widget + 'static {
    ...
    Panel::new()
        .background(bastyde_tokens::SurfaceRole::Main)
        .border_width(1.0)
...
fn scrollable_tab(content: impl Widget + 'static) -> impl Widget + 'static {
    bastyde_widgets::ScrollArea::new().child(content)
}
