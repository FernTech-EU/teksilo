//! Compile-time-validating resource embedding proc macro for FernUI.
//!
...
fn emit_expr(kind: ResourceKind, rel_path: &str, animated: bool) -> TokenStream2 {
    ...
    quote! {
        {
            #watch
            static __FERN_RES: ::std::sync::LazyLock<#ty> =
...
fn lazy_type_and_init(kind: ResourceKind, rel_path: &str, animated: bool) -> (TokenStream2, TokenStream2) {
    match kind {
        ResourceKind::Svg => (
            ...
            quote! {
                ::fern_ui::canvas::svg::SvgIcon::parse(
                    ::core::include_str!(::core::concat!(::core::env!("CARGO_MANIFEST_DIR"), "/", #rel_path))
                ).expect("fern-resources: SVG validated at compile time")
            },
        ),
        ResourceKind::Png => (
            quote!(::fern_ui::canvas::raster::RasterIcon),
            quote! {
                ::fern_ui::canvas::raster::RasterIcon::decode_png(
                    ::core::include_bytes!(::core::concat!(::core::env!("CARGO_MANIFEST_DIR"), "/", #rel_path))
                ).expect("fern-resources: PNG validated at compile time")
            },
        ),
        ResourceKind::Webp if animated => (
            quote!(::fern_ui::canvas::animated::AnimatedIcon),
            quote! {
                ::fern_ui::canvas::animated::AnimatedIcon::decode_webp(
                    ::core::include_bytes!(::core::concat!(::core::env!("CARGO_MANIFEST_DIR"), "/", #rel_path))
                ).expect("fern-resources: animated WebP validated at compile time")
            },
        ),
        ResourceKind::Webp => (
            quote!(::fern_ui::canvas::raster::RasterIcon),
            quote! {
                ::fern_ui::canvas::raster::RasterIcon::decode_webp(
                    ::core::include_bytes!(::core::concat!(::core::env!("CARGO_MANIFEST_DIR"), "/", #rel_path))
                ).expect("fern-resources: WebP validated at compile time")
            },
        ),
    }
}
...
pub fn res(input: TokenStream) -> TokenStream {
    ...
    let data = match std::fs::read(&abs_path) {
        ...
        Err(e) => {
            return syn::Error::new(
                span,
                format!("fern-resources: cannot read `{}`: {e}", abs_path.display()),
            )
            .to_compile_error()
            .into();
        }
    ...
    if let Some(k) = kind {
        ...
        if let Err(msg) = validation_result {
            return syn::Error::new(span, format!("fern-resources: {msg}"))
                .to_compile_error()
                .into();
        }
    }
