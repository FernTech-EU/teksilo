...
pub fn find_bastyde_macros(source: &str, file: &syn::File) -> Vec<BatiMacroEdit> {
    let mut visitor = BatiMacroVisitor {
        source,
        edits: Vec::new(),
    };
...
struct BatiMacroVisitor<'a> {
    source: &'a str,
    edits: Vec<BatiMacroEdit>,
}

impl<'a, 'ast> Visit<'ast> for BatiMacroVisitor<'a> {
    fn visit_macro(&mut self, m: &'ast syn::Macro) {
        let last = m.path.segments.last();
        let is_bati = last.map(|s| s.ident == "bati").unwrap_or(false);
        if !is_bati {
