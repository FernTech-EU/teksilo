...
pub fn print(source: &str, root: &BatiRoot, trivia: &[Trivia]) -> String {
    let mut p = Printer {
        source,
        trivia,
        trivia_idx: 0,
...
impl<'a> Printer<'a> {
    ...
    fn print_root(&mut self, root: &BatiRoot) {
    ...
    fn print_element(&mut self, e: &BatiElement) {
    ...
    fn print_body_item(&mut self, item: &BodyItem) {
        match item {
            ...
            BodyItem::If(if_) => {
                self.write_verbatim_multiline(&bastyde_if_token_stream(if_));
                let r = item_byte_range(item);
                self.cursor = self.cursor.max(r.end);
            }
            BodyItem::Match(m) => {
                self.write_verbatim_multiline(&bastyde_match_token_stream(m));
                let r = item_byte_range(item);
                self.cursor = self.cursor.max(r.end);
            }
            BodyItem::For(f) => {
                self.write_verbatim_multiline(&bastyde_for_token_stream(f));
                let r = item_byte_range(item);
                self.cursor = self.cursor.max(r.end);
            }
    ...
    fn print_property(&mut self, p: &bastyde_parse::BatiProperty) {
...
fn bastyde_if_token_stream(if_: &BatiIf) -> TokenStream {
    ...
    if let Some(b) = &if_.else_branch {
        match &**b {
            BatiElse::ElseIf(nested) => bastyde_if_to_tokens(nested, &mut ts),
            BatiElse::Element {
                element,
                body_close,
            } => {
...
fn bastyde_if_to_tokens(if_: &BatiIf, ts: &mut TokenStream) {
    if_.cond.to_tokens(ts);
    element_to_tokens(&if_.then, ts);
    push_anchor(ts, if_.body_close, "__bc");
    if let Some(b) = &if_.else_branch {
        match &**b {
            BatiElse::ElseIf(nested) => bastyde_if_to_tokens(nested, ts),
            BatiElse::Element {
                element,
                body_close,
            } => {
...
fn bastyde_match_token_stream(m: &BatiMatch) -> TokenStream {
    let mut ts = TokenStream::new();
...
fn bastyde_for_token_stream(f: &BatiFor) -> TokenStream {
    let mut ts = TokenStream::new();
...
fn element_to_tokens(e: &BatiElement, ts: &mut TokenStream) {
...
fn body_item_to_tokens(item: &BodyItem, ts: &mut TokenStream) {
    match item {
        ...
        BodyItem::If(if_) => ts.extend(bastyde_if_token_stream(if_)),
        BodyItem::Match(m) => ts.extend(bastyde_match_token_stream(m)),
        BodyItem::For(f) => ts.extend(bastyde_for_token_stream(f)),
