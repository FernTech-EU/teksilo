...
/// Extension trait on [`TeksiloAppBuilder`] that wires in the debug
/// inspector. The `install_inspector_in_debug` method is a no-op in
/// release builds so apps can call it unconditionally.
pub trait TeksiloAppBuilderInspectorExt {
    /// Install the debug inspector. In debug builds, this:
    /// - parses `--teksilo-inspector` from `std::env::args()` and
    ///   `TEKSILO_INSPECTOR=1` from the environment to seed the initial
    ///   visibility,
...
impl TeksiloAppBuilderInspectorExt for TeksiloAppBuilder {
    #[cfg(debug_assertions)]
    fn install_inspector_in_debug(self) -> Self {
        state::install(self)
    }
