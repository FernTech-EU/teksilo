...
pub struct InspectorState {
    ...
    /// F12, the toolbar Close button, and the `--teksilo-inspector` /
    /// `TEKSILO_INSPECTOR` boot flags.
    ...
    pub tab_ids: Vec<teksilo_widgets::TabId>,
    ...
    /// `_active_tab_bridge`).
    pub active_tab_id: Signal<Option<teksilo_widgets::TabId>>,
    ...
    _active_tab_bridge: std::rc::Rc<Vec<teksilo_core::ObserverHandle>>,
    ...
    /// `__teksilo_inspector.overflow_overlay`.
    ...
    pub(crate) overflow_snapshot: Signal<Vec<teksilo_canvas::Rect>>,
    /// Current panel height in logical pixels. Drives the panel slot's
    /// `FixedSize::height`. Mutated by the panel resize handle
    /// (top-edge drag) and persisted via `__teksilo_inspector.panel_height`.
...
impl InspectorState {
    pub(crate) fn new(initial_open: bool) -> Self {
        ...
        let tab_ids: Vec<teksilo_widgets::TabId> = (0..NUM_TABS)
            .map(|_| teksilo_widgets::TabId::fresh())
            .collect();
        let active_tab_id: Signal<Option<teksilo_widgets::TabId>> = Signal::new(Some(tab_ids[0]));
...
/// `--teksilo-inspector` and `TEKSILO_INSPECTOR` (`1` / `true`).
fn initial_open_from_env() -> bool {
    let flag = std::env::args().any(|a| a == "--teksilo-inspector");
    let env = std::env::var("TEKSILO_INSPECTOR")
...
/// [`crate::TeksiloAppBuilderInspectorExt::install_inspector_in_debug`].
pub(crate) fn install(builder: TeksiloAppBuilder) -> TeksiloAppBuilder {
    ...
    let post_root = DefaultPostRoot::new(move |tree, root_id| {
        ...
        if !persistence_wired.replace(true)
            && let Some(store) = tree
                .app_context()
                .app_state::<teksilo_settings::SettingsStore>()
        {
