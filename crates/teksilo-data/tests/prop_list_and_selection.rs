...
//! Property tests for `ListModel<T>` (crates/teksilo-data/src/list_model.rs)
//! and `SelectionModel` (crates/teksilo-data/src/selection_model.rs) — and,
//! critically, the two wired together the way production code wires them:
//! `teksilo-widgets/src/data_views.rs`'s `RowSelection::from_index` feeds
...
//! `crates/teksilo-tokens/tests/prop_color.rs` / `crates/teksilo-scene/src/
...
//! `PROPTEST_CASES=N cargo test -p teksilo-data --test prop_list_and_selection`.

use std::cell::RefCell;
use std::rc::Rc;

use proptest::prelude::*;
use teksilo_data::{DataChange, ListModel, SelectionMode, SelectionModel};
...
// `ListModel` exactly the way `teksilo-widgets`'s `RowSelection::from_index`
...
proptest! {
    ...
    fn selection_indices_never_dangle_past_the_end_of_the_list(
        ...
        // `RowSelection::from_index` (crates/teksilo-widgets/src/data_views.rs).
