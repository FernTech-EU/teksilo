...
//! `SortFilterListModel<T>` (crates/teksilo-data/src/sort_filter_list_model.rs),
//! `SortFilterTreeModel<T>` + `TreeFilterMode`
//! (crates/teksilo-data/src/sort_filter_tree_model.rs), and `TreeRowFilter<K, T>`
//! (crates/teksilo-data/src/tree_row_filter.rs).
//!
//! ## Why this exists
//!
...
//! ## Running
//!
//! `cargo test -p teksilo-data --test prop_sort_filter` (add
...
//! ## Sibling suites
//!
//! `crates/teksilo-data/tests/prop_tree_slice.rs`,
//! `prop_list_and_selection.rs`, and `prop_tree_checked.rs` cover the
//! neighbouring data-model types in this crate; `crates/teksilo-tokens/tests/prop_color.rs`
...
use teksilo_data::ListDataSource; // brings `len()` / `with_item()` into scope for SortFilterListModel.
use teksilo_data::tree_data_source::tree_is_desc_or_self;
use teksilo_data::{
