...
//! Property tests for `TreeSlice<T>` (crates/teksilo-data/src/tree_slice.rs)
//! and the `TreeModel<T>` it projects (crates/teksilo-data/src/tree_model.rs).
//!
...
//! iteration count with `PROPTEST_CASES=N cargo test -p teksilo-data --test
...
use teksilo_data::{DropPosition, FlatEntry, NodeId, TreeDataSource, TreeModel, TreeSlice};
...
// the shape this file's sibling `teksilo-scene` suite uses for its
