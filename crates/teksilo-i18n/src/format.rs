...
//!
//! 1. **Bundle-side** — `manager.rs` installs `teksilo_format_callback` via
...
//! 2. **Signal-side** — [`NumberFormatter`] and [`TeksiloDateTimeFormatter`] take
...
// ------------------------------------------------------------
// TeksiloDateTime — the FluentValue::Custom payload
// ------------------------------------------------------------

#[derive(Clone, Debug, PartialEq)]
enum TeksiloDateTimeInner {
...
#[derive(Clone, Debug, PartialEq)]
pub struct TeksiloDateTime {
    inner: TeksiloDateTimeInner,
    options: DateTimeOptions,
}

impl TeksiloDateTime {
    pub fn from_civil(dt: jiff::civil::DateTime) -> Self {
        Self {
            inner: TeksiloDateTimeInner::Civil(dt),
    ...
    pub fn from_zoned(z: jiff::Zoned) -> Self {
        Self {
            inner: TeksiloDateTimeInner::Zoned(z),
            options: DateTimeOptions::default(),
        }
    }
...
impl From<jiff::civil::DateTime> for TeksiloDateTime {
    fn from(dt: jiff::civil::DateTime) -> Self {
        Self::from_civil(dt)
    }
}

impl From<jiff::Zoned> for TeksiloDateTime {
    fn from(z: jiff::Zoned) -> Self {
        Self::from_zoned(z)
    }
}

impl From<TeksiloDateTime> for FluentValue<'static> {
    fn from(f: TeksiloDateTime) -> Self {
        FluentValue::Custom(Box::new(f))
    }
}

impl FluentType for TeksiloDateTime {
    ...
    fn as_string(&self, intls: &intl_memoizer::IntlLangMemoizer) -> Cow<'static, str> {
        ...
        let result =
            intls.with_try_get::<IcuDateTimeFormatter, _, _>((opts,), move |fmt| match &inner {
                TeksiloDateTimeInner::Civil(dt) => fmt.format_civil(dt),
                TeksiloDateTimeInner::Zoned(z) => fmt.format_zoned(z),
            });
    ...
    fn as_string_threadsafe(
        ...
        // teksilo-i18n uses fluent-bundle's default (non-threadsafe) memoizer
...
/// the ICU cache; `FluentValue::Custom` (e.g. our `TeksiloDateTime`) falls
/// through to `MemoizerKind::stringify_value` which calls
/// `FluentType::as_string`.
pub(crate) fn teksilo_format_callback(
    value: &FluentValue,
...
/// `FluentValue::Custom(TeksiloDateTime)`; merges named args (`dateStyle`,
...
pub(crate) fn datetime_function<'a>(
    ...
    let Some(fdt) = any.downcast_ref::<TeksiloDateTime>() else {
        return FluentValue::Error;
    };
...
/// [`NumberFormatter`]. Named with the `Teksilo` prefix to avoid colliding
/// with `icu_datetime::DateTimeFormatter` (which teksilo-i18n uses
/// internally to back this type).
#[derive(Clone, Debug, Default)]
pub struct TeksiloDateTimeFormatter {
    options: DateTimeOptions,
}

impl TeksiloDateTimeFormatter {
    pub fn new() -> Self {
        Self::default()
    }
