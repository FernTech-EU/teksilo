...
//! each sets a different `TEKSILO_I18N_SOURCE_*` env var and we rely on
...
fn ui_nested() {
    ...
    unsafe {
        // Clear the file-mode override so `TEKSILO_I18N_SOURCE_DIR`
        // alone wins in `resolve_source`'s precedence check.
        std::env::remove_var("TEKSILO_I18N_SOURCE_PATH");
        std::env::set_var("TEKSILO_I18N_SOURCE_DIR", fixture_dir);
    }
