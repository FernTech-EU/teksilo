...
use bastyde_core::telemetry::UsageReporter;
use bastyde_settings::{AppPaths, SettingsFileError, SettingsStore};
...
impl TelemetryBundle {
    ...
    /// `bastyde-analytics-plausible`). The adapter's `install_id()`
    /// must return `None`.
    ...
    /// `bastyde-analytics-posthog`). The adapter's `install_id()` must
...
/// `BastydeAppBuilder::install_telemetry`.
#[derive(Clone)]
...
impl OpenedTelemetry {
    ...
    pub fn discard_pending(&self) -> Result<(), bastyde_core::telemetry::TelemetryError> {
