...
//!
//! Same convention as
//! [`teksilo_settings::SettingsExt`]. Apps
//! `use teksilo_telemetry::TelemetryExt;` to reach `ctx.consent()`,
        ...
        self.try_telemetry().unwrap_or_else(|| {
            panic!(
                "TelemetryExt::telemetry(): no OpenedTelemetry registered. \
                 Call TeksiloAppBuilder::telemetry(TelemetryBundle::new(...)) at startup."
            )
        })
    }
