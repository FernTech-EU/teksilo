...
/// the thread-local by `BastydeAppBuilder` (Phase B).
///
...
///   `register_framework_locales` (Phase E will wire bastyde-widgets to this).
...
impl I18nManager {
    ...
    /// Register framework-internal locales (called by `BastydeAppBuilder` from
    /// `bastyde_widgets::framework_locales()` in Phase E). Each entry is a
    ...
    pub fn register_framework_locales(&self, slice: &[(&str, &[&'static str])]) {
        ...
        for (tag, resources) in slice {
            let Ok(loc): Result<LanguageIdentifier, _> = tag.parse() else {
                eprintln!("bastyde-i18n: skipping invalid framework locale tag {tag}");
                continue;
            };
    ...
    pub fn register_widget_overrides(&self, slice: &[(&str, &[&'static str])]) {
        ...
        for (tag, resources) in slice {
            let Ok(loc): Result<LanguageIdentifier, _> = tag.parse() else {
                eprintln!("bastyde-i18n: skipping invalid widget-override locale tag {tag}");
                continue;
            };
            let bundle = build_bundle_from_resources(&loc, resources);
            bundles.insert(loc, bundle);
    ...
    pub fn resolve_app(&self, key: &str, args: &[(&str, FluentValue<'_>)]) -> String {
        ...
        eprintln!("bastyde-i18n: missing key `{key}` in app bundles");
        key.to_string()
    }
    ...
    pub fn resolve_widget(&self, key: &str, args: &[(&str, FluentValue<'_>)]) -> String {
        ...
        eprintln!("bastyde-i18n: missing key `{key}` in widget bundles");
        key.to_string()
    }
}
...
fn configure_bundle(bundle: &mut FluentBundle<FluentResource>) {
    ...
    if let Err(e) = bundle.add_builtins() {
        eprintln!("bastyde-i18n: failed to register Fluent builtins: {e:?}");
    }
    bundle.set_formatter(Some(crate::format::bastyde_format_callback));
    if let Err(e) = bundle.add_function("DATETIME", crate::format::datetime_function) {
        eprintln!("bastyde-i18n: failed to register DATETIME function: {e:?}");
    }
}
        ...
        match FluentResource::try_new((*src).to_string()) {
            Ok(res) => {
                if let Err(errs) = bundle.add_resource(res) {
                    eprintln!("bastyde-i18n: errors adding resource for {locale}: {errs:?}");
                }
            }
            Err((_res, errs)) => {
                eprintln!("bastyde-i18n: parse errors in resource for {locale}: {errs:?}");
            }
        }
    }
    bundle
...
fn build_bundle_from_test_messages(entry: &TestLocaleEntry) -> FluentBundle<FluentResource> {
    ...
    match FluentResource::try_new(combined) {
        Ok(res) => {
            if let Err(errs) = bundle.add_resource(res) {
                eprintln!(
                    "bastyde-i18n: errors building test bundle for {}: {errs:?}",
                    entry.locale
                );
            }
        }
        Err((_res, errs)) => {
            eprintln!(
                "bastyde-i18n: parse errors in test bundle for {}: {errs:?}",
                entry.locale
            );
        }
    }
...
mod tests {
    ...
    fn reload_from_path_replaces_bundle_and_bumps_version() {
        let dir = std::env::temp_dir().join(format!("bastyde-i18n-reload-{}", std::process::id()));
