...
//! 2. **Signal-side path** — `NumberFormatter` / `BastydeDateTimeFormatter`
...
use bastyde_i18n::format::{BastydeDateTime, BastydeDateTimeFormatter, NumberFormatter};
use bastyde_i18n::{I18nConfig, I18nManager, LanguageIdentifier, tr_signal};
...
#[test]
fn bundle_datetime_via_bastyde_datetime_arg() {
    let mgr = install_en_fr();
    let dt = jiff::civil::date(2026, 5, 4).at(14, 35, 0, 0);
    let fdt: fluent_bundle::FluentValue<'static> = BastydeDateTime::from(dt).into();
...
fn datetime_formatter_signal_reacts_to_locale_change() {
    ...
    let display = BastydeDateTimeFormatter::new()
        .date_style(bastyde_i18n::DateStyle::Long)
        .format(dt);
