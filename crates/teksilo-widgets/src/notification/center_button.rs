...
mod tests {
    ...
    fn bell_label_present() {
        ...
        let bell_label = teksilo_i18n::tr_widget!(a11y_builtin_bell()).resolve_now();
        assert!(
    ...
    /// below, which give their trees real `TeksiloWindowId`s. Those
    ...
    fn scoped_bell_only_counts_its_audience_and_broadcast_unread() {
        ...
        let tree_a = tree_with(
            NotificationCenterButton::new(archive.clone()).for_window(TeksiloWindowId::new(1)),
        );
        assert!(
    ...
    // and `teksilo_core::binding::BindingRegistry` for why one signal
    ...
    fn two_window_bells(archive: Rc<NotificationArchiveModel>) -> (WidgetTree, WidgetTree) {
        ...
        use teksilo_core::window::{WindowPlacement, WindowState};
