...
fn checkbox_press_does_not_select_row() {
    ...
    let cb_x =
        row0.x + si::STANDARD_ITEM_BG_HORIZONTAL_INSET + si::STANDARD_ITEM_PADDING_HORIZONTAL + 4.0;
...
fn home_and_end_reach_the_ends_of_the_collection() {
    use teksilo_core::event::{Key, Modifiers};
    let (mut tree, selection, _scroll, p) = keyboard_fixture(teksilo_data::SelectionMode::Single);
    selection.select(40);
...
fn home_and_end_scroll_the_row_they_land_on_into_view() {
    use teksilo_core::event::{Key, Modifiers};
    let (mut tree, selection, scroll, p) = keyboard_fixture(teksilo_data::SelectionMode::Single);
    selection.select(0);
    tree.layout(p);
...
fn the_accelerator_moves_the_cursor_without_disturbing_the_selection() {
    use teksilo_core::event::{Key, Modifiers};
    let (mut tree, selection, _scroll, p) = keyboard_fixture(teksilo_data::SelectionMode::Multi);
    selection.select(40);
...
fn drop_indicator_clears_after_drop() {
    ...
    let feedback = with_list_view::<usize, _>(&tree, lv_id, |lv| lv.drop_feedback_signal().get());
    assert!(
        feedback.is_none(),
...
fn edge_auto_scroll_advances_scroll_y_during_drag() {
    ...
    let (mut tree, _lv_id, _model) = make_reorderable_list((0..50).collect::<Vec<usize>>(), 30.0);
    tree.layout(SizeProposal::exact(400.0, 300.0));
...
fn exportable_multi_selection_drags_the_whole_set() {
    let (mut tree, _model, sel, cap) = export_scene(vec![10, 20, 30, 40], DragTransferMode::Copy);
