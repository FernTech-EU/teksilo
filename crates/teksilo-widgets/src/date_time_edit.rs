...
impl Widget for DateTimeEdit {
    fn build(&mut self, ctx: &mut BuildContext) -> Vec<WidgetId> {
        ...
        // ── Resolve patterns ───────────────────────────────────
        // A locale switch must re-derive the date pattern and the 12-vs-24-hour clock: it is read from
...
impl DateTimeEdit {
    ...
    /// validator + segment-stepping. Returns `(wrapper, inner field)`:
    /// the WidgetId of the fixed-width container so the half visually
    /// aligns inside the unified frame, plus the inner editable field's
    /// own id.
    fn build_date_half(
        &self,
