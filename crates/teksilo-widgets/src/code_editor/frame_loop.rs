...
//! another. That return value is the whole contract: Teksilo is draw-when-needed,
...
pub(crate) fn tick(state: &mut CodeEditorState, delta: f32) -> bool {
    ...
    if viewport_ready && (state.needs_full_layout || layout_stale) {
        ...
        let mask = teksilo_text::text_document::HighlightMask::all();
        match state
            .engine
    ...
    if state.engine.has_full_layout() {
        ...
        let cursors: Vec<teksilo_text::CursorDisplay> = state
            .all_carets()
...
fn caret_mut(
    ...
    i: usize,
) -> &mut teksilo_text::text_document::TextCursor {
    if i == 0 {
        &mut state.cursor
    } else {
