...
///
/// **Why inline, not `crates/teksilo-widgets/tests/`**: `distribute` is a
...
/// (only `PaneSnapshot` and friends from `model`). So `teksilo_widgets::
...
/// `cargo test -p teksilo-widgets --lib splitter::distribute::proptests`.
///
...
/// -p teksilo-widgets --lib splitter::distribute::proptests`.
#[cfg(test)]
mod proptests {
    use super::*;
