...
impl Widget for BreadcrumbSegment {
    fn build(&mut self, ctx: &mut BuildContext) -> Vec<WidgetId> {
        ...
        let handler_set = HandlerSet::new()
            ...
            .on_access_action(move |action, ctx: &mut EventContext| {
                if interactive && action == teksilo_core::accesskit::Action::Click {
        ...
        if let Some(content) = self.composite_tooltip_content.take() {
            ...
            let delay = ctx.theme().motion.tooltip_delay;
            crate::tooltip::attach_plain_tooltip(ctx, self_id, tooltip_text, delay);
        }
    ...
    fn accessibility(&self, builder: &mut AccessNodeBuilder) {
        ...
        if self.current {
            builder.set_aria_current(teksilo_core::accesskit::AriaCurrent::Page);
...
impl Widget for Breadcrumb {
    fn build(&mut self, ctx: &mut BuildContext) -> Vec<WidgetId> {
        ...
        *self.last_flags.borrow_mut() = vec![false; n];
        ...
        self.ellipsis_unit_id = if has_collapsible {
            ...
            let trigger = Button::new(teksilo_i18n::lit!("…"))
                .variant(ButtonVariant::Ghost)
                .tooltip(teksilo_i18n::tr_widget!(breadcrumb_overflow()));
    ...
    fn accessibility(&self, builder: &mut AccessNodeBuilder) {
        builder.set_role(teksilo_core::accesskit::Role::Navigation);
