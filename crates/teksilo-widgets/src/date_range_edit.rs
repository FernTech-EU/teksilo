...
impl Widget for DateRangeEdit {
    fn build(&mut self, ctx: &mut BuildContext) -> Vec<WidgetId> {
        ...
        let start_shrinkable = ctx.add(crate::primitives::Shrinkable::new().child(start_field_id));
        let end_shrinkable = ctx.add(crate::primitives::Shrinkable::new().child(end_field_id));
        let row = HStack::new()
            .spacing(0.0)
            .child(start_shrinkable)
            .child(separator_id)
            .child(end_shrinkable)
            .child(trigger_id);
