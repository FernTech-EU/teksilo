...
use teksilo_core::styles::{AvatarStyleConfig, SharedAvatarStyle};
use teksilo_core::widget::{
    CursorIcon, EventContext, LayoutContext, PaintContext, Widget, WidgetPlacement,
};
use teksilo_core::widget_builder::HandlerSet;
use teksilo_core::widget_id::WidgetId;
use teksilo_tokens::{Color, FontWeight, TextStyle};

pub use teksilo_core::styles::{AvatarCorner, AvatarPresence, AvatarShape, AvatarSize};
...
/// (unlike `DefaultHasher`). Same idiom as `teksilo_core::accessibility`.
    ...
    fn build(&mut self, ctx: &mut BuildContext) -> Vec<WidgetId> {
        ...
        if let Some(action) = self.action.clone() {
            ...
            let handlers = HandlerSet::new()
                ...
                .on_access_action(move |action_kind, ctx| {
                    ...
                    if action_kind == teksilo_core::accesskit::Action::Click {
        ...
        let root = style.make_body(
            &AvatarStyleConfig {
                ...
                // `:focus-visible`: keyboard-only focus ring (gate raw focus
                // on the input-modality signal).
                is_focused: focused.and(&ctx.focus_visible()),
...
mod tests {
    ...
    fn render_avatar(avatar: Avatar) -> std::rc::Rc<teksilo_canvas::RenderFrame> {
    ...
    fn count_shapes(frame: &teksilo_canvas::RenderFrame) -> usize {
    ...
    fn accessibility_image_default_role_is_image() {
        ...
        tree.layout(SizeProposal::exact(32.0, 32.0));
        let info = tree.accessibility_node(id);
        assert_eq!(info.role(), teksilo_core::accesskit::Role::Image);
    ...
    fn glyph_colors(frame: &teksilo_canvas::RenderFrame) -> Vec<[f32; 4]> {
        frame.glyphs.iter().map(|g| g.color).collect()
    }

    fn shape_colors(frame: &teksilo_canvas::RenderFrame) -> Vec<[f32; 4]> {
    ...
    fn has_popup_without_clickable_still_compiles() {
        ...
        let mut tree = WidgetTree::new().with_theme(teksilo_core::presets::intui::light());
        let id = tree.add(
            Avatar::with_initials(lit!("JD")).has_popup(teksilo_core::accesskit::HasPopup::Menu),
        );
        tree.layout(SizeProposal::exact(32.0, 32.0));
    ...
    fn focus_ring_only_paints_when_focused() {
        ...
        // Focus the avatar AND flip the input modality to keyboard. The focus
    ...
    fn focus_ring_uses_theme_focus_ring_color() {
        ...
        let target = teksilo_core::presets::intui::light().colors.focus_ring;
        assert!(
            shape_colors(&frame)
                .iter()
                .any(|c| approx_color_eq(*c, target)),
    ...
    fn presence_swap_changes_dot_color_and_a11y_description() {
        ...
        tree.layout(SizeProposal::exact(32.0, 32.0));
        let online_color = teksilo_core::presets::intui::light()
            .colors
            .status_success_fg;
        assert!(
        ...
        tree.layout(SizeProposal::exact(32.0, 32.0));
        let busy_color = teksilo_core::presets::intui::light().colors.status_error_fg;
        assert!(
            shape_colors(&tree.render())
                .iter()
