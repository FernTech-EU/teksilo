...
/// to avoid adding a `log` dependency to teksilo-widgets — the registry
...
fn warn_missing_install() {
    ...
    WARNED.with(|w| {
        if !w.get() {
            eprintln!(
                "[teksilo-widgets::toast] ctx.show_toast(...) called without install_toast(opts) \
                 on the TeksiloAppBuilder — the toast was dropped. See teksilo::install_toast \
                 or teksilo_widgets::toast docs."
            );
            w.set(true);
        }
    });
