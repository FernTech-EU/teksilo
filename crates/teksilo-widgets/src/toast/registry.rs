...
pub struct ToastRegistry {
    ...
    window_audiences: Rc<RefCell<HashMap<TeksiloWindowId, Signal<Option<ToastAudience>>>>>,
}
...
mod tests {
    ...
    fn show_settings_write_failed_enqueues_a_persistent_error_toast_naming_the_file() {
        ...
        // discarded `teksilo-settings` write into something the user
    ...
    fn forget_window_on_an_unknown_window_is_a_safe_no_op() {
        let r = registry();
        let known = TeksiloWindowId::new(1);
        let unknown = TeksiloWindowId::new(999);
