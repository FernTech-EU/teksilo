...
pub use attach::{
    ...
    attach_plain_tooltip, attach_plain_tooltip_with_placement, attach_rich_tooltip,
...
/// `teksilo-core` so widgets can request `Side` placement in a vertical
/// list without naming the core path.
pub use teksilo_core::overlay::TooltipPlacement;
...
use teksilo_core::styles::{SharedTooltipStyle, TooltipStyleConfig};
...
pub struct TooltipWidget {
    ...
    /// hover targets, as `teksilo-scene` does for lightweight items).
...
impl Widget for TooltipWidget {
    ...
    fn accessibility(&self, builder: &mut AccessNodeBuilder) {
        builder.set_role(teksilo_core::accesskit::Role::Tooltip);
...
mod tests {
    ...
    fn shown_tooltip_bounds(text: &str) -> teksilo_canvas::Rect {
