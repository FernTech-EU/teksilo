...
impl StatusBar {
    ...
    /// [`BorderRole`](teksilo_tokens::BorderRole), or a `Signal<Color>`.
    ...
    /// [`LocalizedString`](teksilo_i18n::LocalizedString) (locale-reactive).
...
impl Widget for StatusBar {
    ...
    fn accessibility(&self, builder: &mut AccessNodeBuilder) {
        ...
        let name = match &self.name {
            Some(prop) => prop.get(),
            None => teksilo_i18n::tr_widget!(a11y_status_bar_name()).resolve_now(),
        };
...
mod tests {
    ...
    fn status_bar_default_has_no_live_region() {
        ...
        let sb_nid = teksilo_core::accessibility::widget_id_to_node_id(sb);
        let sb_node = update
            .nodes
            .iter()
            .find(|(id, _)| *id == sb_nid)
        ...
        assert_eq!(sb_node.role(), teksilo_core::accesskit::Role::Status);
    ...
    fn status_bar_announce_changes_enables_polite_live_region_and_no_group_wrapper() {
        ...
        let sb_node = update
            ...
            .map(|(_, n)| n)
            .expect("status bar node in tree");
        assert_eq!(sb_node.live(), Some(teksilo_core::accesskit::Live::Polite));
        // Panel wrapper should be hidden so StatusBar → HStack directly,
        // no intermediate Role::Group node.
        let groups: Vec<_> = update
            .nodes
            .iter()
            .filter(|(_, n)| n.role() == teksilo_core::accesskit::Role::Group)
            .collect();
        assert!(
            groups.is_empty(),
