...
//!
//! `Repeater` creates one child widget per item in a [`ListModel<T>`](teksilo_data::ListModel)
...
use teksilo_data::{DataChange, ListModel};
...
impl<T: 'static> Repeater<T> {
    ...
    fn build_item_ids(&self, ctx: &mut teksilo_core::build_context::BuildContext) -> Vec<WidgetId> {
