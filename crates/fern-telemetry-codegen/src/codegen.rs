...
fn generate_emit_fn(event: &EventDef, schema_version: u32) -> TokenStream {
    ...
    let mut params: Vec<TokenStream> = vec![
        quote! { reporter: &dyn ::fern_telemetry::UsageReporter },
    ...
    let props_expr = if prop_entries.is_empty() {
        quote! { let props: [::fern_telemetry::Prop<'_>; 0] = []; }
    } else {
    ...
    quote! {
        pub fn #fn_name(#(#params),*) {
            #props_expr
            reporter.record(&::fern_telemetry::Event {
                name: #event_name_lit,
...
fn prop_tokens(event: &EventDef, prop: &PropDef) -> (TokenStream, TokenStream) {
    ...
    match prop.ty.as_str() {
        "dev_static" => (
            quote! { &'static str },
            quote! { ::fern_telemetry::PropValue::StaticStr(#param_name) },
        ),
        "bounded_str" => (
            quote! { &str },
            quote! { ::fern_telemetry::PropValue::BoundedStr(#param_name) },
        ),
        "u32" => (
            quote! { u32 },
            quote! { ::fern_telemetry::PropValue::U32(#param_name) },
        ),
        "i64" => (
            quote! { i64 },
            quote! { ::fern_telemetry::PropValue::I64(#param_name) },
        ),
        "bool" => (
            quote! { bool },
            quote! { ::fern_telemetry::PropValue::Bool(#param_name) },
        ),
        "f64_bucket" => (
            quote! { ::fern_telemetry::F64Bucket },
            quote! { ::fern_telemetry::PropValue::F64Bucket(#param_name) },
        ),
        "enum" => {
            let enum_ty = Ident::new(&enum_type_name(&event.name, &prop.name), Span::call_site());
            (
                quote! { #enum_ty },
                quote! { ::fern_telemetry::PropValue::Enum { variant: #param_name.as_str() } },
            )
        }
...
fn category_tokens(category: &str) -> TokenStream {
    match category {
        "intent" => quote! { ::fern_telemetry::EventCategory::Intent },
        "lifecycle" => quote! { ::fern_telemetry::EventCategory::Lifecycle },
        "navigation" => quote! { ::fern_telemetry::EventCategory::Navigation },
        "census" => quote! { ::fern_telemetry::EventCategory::Census },
        "custom" => quote! { ::fern_telemetry::EventCategory::Custom },
        _ => unreachable!("validated before codegen"),
    }
}
