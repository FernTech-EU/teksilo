...
use bastyde_canvas::geometry::Transform2D;
...
struct ActiveTarget {
    ...
    blur_bounds: Option<bastyde_canvas::Rect>,
...
impl Renderer {
    ...
    pub fn render(
...

        {
            ...
            // Blend mode stack
            let mut blend_stack: Vec<bastyde_canvas::BlendMode> = Vec::new();
            let mut current_blend = bastyde_canvas::BlendMode::Normal;
            ...
            while cmd_idx <= frame.draw_order.len() {
...

                {
                    let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                        label: Some("bastyde_segment_pass"),
                    ...
                    while cmd_idx < frame.draw_order.len() {
                        ...
                        match cmd {
                            bastyde_canvas::DrawCommand::Decoration(idx) => {
                                flush_all!(
                                    pass,
                                    &self.queue,
                                    self.streams,
                            ...
                            bastyde_canvas::DrawCommand::Glyph(idx) => {
                            ...
                            bastyde_canvas::DrawCommand::Shadow(idx) => {
                                flush_all!(
                                    pass,
                                    &self.queue,
                                    self.streams,
                            ...
                            bastyde_canvas::DrawCommand::Image(idx) => {
                            ...
                            bastyde_canvas::DrawCommand::SetClip(rect) => {
                                flush_all!(
                                    pass,
                                    &self.queue,
                                    self.streams,
                            ...
                            bastyde_canvas::DrawCommand::ClearClip => {
                                flush_all!(
                                    pass,
                                    &self.queue,
                                    self.streams,
                            ...
                            bastyde_canvas::DrawCommand::SetOpacity(opacity) => {
                                flush_all!(
                                    pass,
                                    &self.queue,
                                    self.streams,
                            ...
                            bastyde_canvas::DrawCommand::RestoreOpacity => {
                                flush_all!(
                                    pass,
                                    &self.queue,
                                    self.streams,
                            ...
                            bastyde_canvas::DrawCommand::Rasterized(_) => {}
                            bastyde_canvas::DrawCommand::AnimatedQuad(idx) => {
                                ...
                                quad_source = None;
                                match &draw.class {
                                    bastyde_canvas::AnimatedQuadClass::Procedural => {
                                        let verts =
                                    ...
                                    bastyde_canvas::AnimatedQuadClass::Sprite { image_name } => {
                            ...
                            bastyde_canvas::DrawCommand::SetBlendMode(mode) => {
                ...
                match &frame.draw_order[cmd_idx] {
                    bastyde_canvas::DrawCommand::BeginBlurredSubtree { bounds, radius } => {
            ...
            if !final_composites.is_empty() {
                ...
                surface.opened = true;
                let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                    label: Some("bastyde_final_composite_pass"),
                    color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                        view,
                        resolve_target: None,
                        ops: wgpu::Operations {
                ...
                .opened
            {
                ...
                let _ = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                    label: Some("bastyde_empty_clear_pass"),
                    color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                        view,
                        resolve_target: None,
                        ops: wgpu::Operations {
    ...
    fn draw_image(
        ...
        image: &bastyde_canvas::ImageQuad,
        scale_factor: f32,
        viewport_width: u32,
        viewport_height: u32,
        opacity: f32,
        ...
        let (color, flags) = if let Some(tint) = image.tint {
            // Tint colors are sRGB-encoded (from bastyde_tokens::Color) — linearize
...
/// sized for [`MAX_ANIM_SLOTS`] × `size_of::<bastyde_canvas::AnimParams>()`;
...
fn create_anim_proc_pipeline(
    ...
    let buffer_size = (MAX_ANIM_SLOTS * std::mem::size_of::<bastyde_canvas::AnimParams>()) as u64;
...
mod tests {
    ...
    fn glyph_quad_renders_over_shape_in_offscreen_target() {
        ...
        let texture = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("bastyde_render_test_target"),
            size: wgpu::Extent3d {
                width: 32,
                height: 32,
                depth_or_array_layers: 1,
