...
//! macOS theme preset for Teksilo.
//!
//! **Stub.** The constructors return a `teksilo_core::Theme` shaped from
...
pub fn light() -> Theme {
    let mut theme = intui::light().with_id("macos.light");
...
pub fn dark() -> Theme {
    let mut theme = intui::dark().with_id("macos.dark");
