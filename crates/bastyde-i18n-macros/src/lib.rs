...
//! site.
//!
//! The runtime side of the i18n stack lives in `bastyde-i18n`. The macros
//! emit code that calls `::bastyde::i18n::localized(...)` and
//! `::bastyde::i18n::resolve_message[_widget](...)`, so any crate using
//! these macros must depend on `bastyde` (with the `i18n` feature enabled).
//!
//! # Source file resolution
//!
//! `tr!` reads `$CARGO_MANIFEST_DIR/locales/en-US.ftl` by default. For
//! tests that need a different fixture, set the `BASTYDE_I18N_SOURCE_PATH`
//! environment variable at compile time to a path relative to
//! `CARGO_MANIFEST_DIR` (or an absolute path).
//!
//! `tr_widget!` reads the same path. It is used inside bastyde-widgets
...
impl SourceKind {
    ...
    ///
    /// External apps route through `::bastyde::i18n` so they only
    /// need `bastyde` in deps (the serde pattern). Internal bastyde
    /// crates (`bastyde-widgets`, `bastyde-i18n` tests) route through
    /// `::bastyde_i18n` because they can't depend on `bastyde` (circular).
...
/// `BASTYDE_I18N_SOURCE_DIR` environment variable. Applications that
...
/// compiled. Precedence (highest first): `BASTYDE_I18N_SOURCE_DIR`,
/// `BASTYDE_I18N_SOURCE_PATH`, auto-detect `locales/en-US/` directory,
...
fn resolve_source() -> std::result::Result<SourceInfo, String> {
    ...
    if let Ok(dir_override) = std::env::var("BASTYDE_I18N_SOURCE_DIR") {
    ...
    if let Ok(file_override) = std::env::var("BASTYDE_I18N_SOURCE_PATH") {
    ...
    for seg in segments {
        ...
        if s.contains("__") {
            return Err(syn::Error::new(
                seg.span(),
                format!(
                    "bastyde-i18n: path segment `{s}` contains `__`, \
        ...
        if !first_ok || !rest_ok {
            return Err(syn::Error::new(
                seg.span(),
                format!(
                    "bastyde-i18n: path segment `{s}` is not a valid Fluent \
...
fn tr_impl(input: TokenStream, kind: SourceKind, signal: bool) -> TokenStream {
    ...
    let key_map = match load_key_map_for_source(&source_info) {
        ...
        Err(msg) => {
            return syn::Error::new(
                call.path_span(),
                format!(
                    "bastyde-i18n: failed to load source `{}`: {msg}",
                    source_info.display()
                ),
            )
            .to_compile_error()
    ...
    let (expected_args, fallback_parts): (Vec<String>, Option<Vec<FallbackPart>>) =
        match key_map.messages.get(&fluent_key) {
            ...
            None => {
                let mut msg = format!(
                    "bastyde-i18n: translation key `{}` not found in `{}`",
                    fluent_key,
                    source_info.display()
                );
    ...
    for expected in &expected_args {
        if !provided_names.iter().any(|p| p == expected) {
            return syn::Error::new(
                call.path_span(),
                format!("bastyde-i18n: missing argument `{expected}` for key `{fluent_key}`"),
            )
            .to_compile_error()
            .into();
        }
    ...
    for arg in &call.args {
        ...
        if !expected_args.iter().any(|e| e == &name) {
            return syn::Error::new(
                arg.name.span(),
                format!(
                    "bastyde-i18n: unknown argument `{name}` for key `{fluent_key}` (expected: {})",
    ...
    let fallback_body = match fallback_parts {
        Some(parts) => {
                ...
                match part {
                    FallbackPart::Text(text) => {
                        ...
                        fallback_stmts.push(quote! {
                            __bastyde_fallback.push_str(#lit);
                        });
                    }
            ...
            quote! {
                let __bastyde_result =
                    #resolver(#key_lit, &[#(#arg_slice_entries),*]);
                if __bastyde_result == #key_lit {
                    let mut __bastyde_fallback = ::std::string::String::new();
                    #(#fallback_stmts)*
                    __bastyde_fallback
                } else {
                    __bastyde_result
    ...
    let expanded = if signal {
        ...
        let arg_subscribes: Vec<TokenStream2> = arg_idents
            ...
            .map(|ident| {
                quote! {
                    {
                        let __bastyde_resolver_arg = ::std::rc::Rc::clone(&__bastyde_resolver);
                        let __bastyde_weak_arg = __bastyde_weak.clone();
                        let __bastyde_h = #ident.observe(move |_| {
                            if let Some(__t) = __bastyde_weak_arg.upgrade() {
                                __t.set((__bastyde_resolver_arg)());
                            }
                        });
                        __bastyde_out.attach_keepalive(__bastyde_h);
                    }
                }
            })
            .collect();
        ...
        quote! {
            {
                ...
                let __bastyde_resolver: ::std::rc::Rc<dyn ::std::ops::Fn() -> ::std::string::String> = {
                ...
                let __bastyde_out = #i18n_root::Signal::new((__bastyde_resolver)());
                let __bastyde_weak = __bastyde_out
                    .downgrade()
                    .expect("Signal::new returns mutable; downgrade cannot fail");

                if let Some(__bastyde_ver) = #i18n_root::current_version_signal() {
                    let __bastyde_resolver_ver = ::std::rc::Rc::clone(&__bastyde_resolver);
                    let __bastyde_weak_ver = __bastyde_weak.clone();
                    let __bastyde_handle_ver = __bastyde_ver.observe(move |_| {
                        if let Some(__t) = __bastyde_weak_ver.upgrade() {
                            __t.set((__bastyde_resolver_ver)());
                        }
                    });
                    __bastyde_out.attach_keepalive(__bastyde_handle_ver);
                }
