...
use bastyde_preview::find_by_file;
...
impl Default for PreviewerOptions {
    fn default() -> Self {
        Self {
            window_title: "Bastyde Widget Previewer".to_string(),
...
impl PreviewerOptions {
        ...
        for arg in iter {
            ...
            if let Some(value) = arg.strip_prefix("--widget=") {
                ...
                match find_by_file(value) {
                    ...
                    None => {
                        eprintln!(
                            "bastyde-previewer: no widget catalog entry registered \
                             from file matching '{}'",
                            value
                        );
                        std::process::exit(2);
                ...
                if let Some((w, h)) = value.split_once('x') {
                    let w = w.parse::<u32>().unwrap_or_else(|_| {
                        eprintln!("bastyde-previewer: invalid --window width '{}'", w);
                        std::process::exit(2);
                    });
                    let h = h.parse::<u32>().unwrap_or_else(|_| {
                        eprintln!("bastyde-previewer: invalid --window height '{}'", h);
                        std::process::exit(2);
                    });
                    opts.window_size = (w, h);
                } else {
                    eprintln!("bastyde-previewer: --window must be WIDTHxHEIGHT (e.g. 1600x900)");
                    std::process::exit(2);
                }
                ...
                eprintln!("bastyde-previewer: unrecognised argument '{}'", arg);
                print_usage();
                std::process::exit(2);
            }
        }
    ...
    fn validate_against_registry(&mut self) {
        ...
        let want_id = match &self.initial_widget {
            ...
            None => {
                if self.initial_variant.is_some() {
                    eprintln!(
                        "bastyde-previewer: --variant requires --widget to also be \
                         specified."
                    );
                    std::process::exit(2);
                }
        ...
        let entry = bastyde_preview::find_by_id(&want_id);
        if entry.is_none() {
            // Build a hint listing available widget ids.
            let available: Vec<&'static str> =
                bastyde_preview::iter_entries().map(|e| e.id()).collect();
            eprintln!(
                "bastyde-previewer: no widget registered with id '{}'.",
                want_id
            );
        ...
        if let Some(want_variant) = &self.initial_variant {
            ...
            if !names.contains(&want_variant.as_str()) {
                eprintln!(
                    "bastyde-previewer: widget '{}' has no variant named '{}'.",
