...
//! `wgpu::TextureView`. The pipeline mirrors bastyde-render's headless
//! test infrastructure (`bastyde_render::test_support`):
//!
...
use bastyde_render::test_support;
...
/// `~/.bastyde-previewer/exports/<widget>__<variant>__<theme>.png`. The
...
fn output_path(
    ...
    out_dir.push(".bastyde-previewer");
    ...
    let theme_label: &'static str = match canvas_theme {
        ...
        crate::app_state::CanvasTheme::Native => {
            if bastyde_platform::os_theme::query_color_scheme().is_dark() {
                "native-dark"
            } else {
                "native-light"
            }
...
async fn render_offscreen(
    ...
    let (mut renderer, device, queue) =
        match test_support::create_test_renderer("bastyde-preview-ui png export").await {
            Some(t) => t,
            None => {
                return Err(
    ...
    let texture = device.create_texture(&wgpu::TextureDescriptor {
        label: Some("bastyde-preview-ui export texture"),
        size: wgpu::Extent3d {
            width,
            height,
