...
pub fn parse_root(tokens: TokenStream2) -> Result<BatiRoot> {
    Parser::parse2(parse_root_impl, tokens)
}

fn parse_root_impl(input: ParseStream) -> Result<BatiRoot> {
    // Disambiguate the two invocation forms by two-token lookahead:
    //   bati!(ctx => <element>)   — ident followed by `=>`
    //   bati!(<element>)          — anything else
    ...
    if !input.is_empty() {
        return Err(input.error(
            "expected end of bati! input — the root element must be the only top-level element",
        ));
    }

    Ok(BatiRoot { ctx, root })
}
