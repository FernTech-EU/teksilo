...
use teksilo_preview::{KnobOverrides, KnobValues, PreviewVariant};
use teksilo_widgets::{
...
///   `teksilo_platform::os_theme::query_os_theme_colors`. Same semantics
...
impl CanvasTheme {
    ...
    pub fn theme(self) -> teksilo_core::Theme {
        match self {
            CanvasTheme::Native => {
                ...
                teksilo_core::Theme {
                    colors: teksilo_tokens::ColorTokens::from_os_colors(&os),
                    ..base
                }
            }
            CanvasTheme::Light => teksilo_core::presets::intui::light(),
            CanvasTheme::Dark => teksilo_core::presets::intui::dark(),
        }
    }
}
...
impl AppState {
    ...
    pub fn knobs_for(&self, widget_id: &'static str, variant_name: &'static str) -> KnobValues {
        ...
        let entry = match teksilo_preview::find_by_id(widget_id) {
            Some(e) => e,
            None => panic!(
    ...
    pub fn select_first_registered(&self) {
        if let Some(entry) = teksilo_preview::iter_entries().next() {
    ...
    pub fn select_widget(&self, widget_id: &'static str) {
        ...
        let entry = teksilo_preview::find_by_id(widget_id).expect("entry exists");
...
impl PreviewerRoot {
    ...
    fn resolve_initial_widget(&self) -> Option<&'static str> {
        ...
        teksilo_preview::find_by_id(id).map(|e| e.id())
    }
...
impl Widget for PreviewerRoot {
        ...
        if state.selected_widget.get().is_none() {
            if let Some(widget_id) = self.resolve_initial_widget() {
                ...
                let variant = self.resolve_initial_variant(widget_id).or_else(|| {
                    teksilo_preview::find_by_id(widget_id)
    ...
    fn accessibility(&self, builder: &mut AccessNodeBuilder) {
        builder.set_role(teksilo_core::accesskit::Role::Window);
        builder.set_name("Teksilo Widget Previewer");
    }
}
