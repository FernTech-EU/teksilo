...
fn build_header(ctx: &mut BuildContext, state: &AppState) -> WidgetId {
    ...
    let count = TextWidget::new(lit!(""))
        ...
        .text(state.navigator_filter.clone().map({
            move |q| {
                let total = teksilo_preview::iter_entries().count();
                if q.is_empty() {
    ...
    let row = HStack::new()
        .spacing(8.0)
        .child(title)
        .child(teksilo_widgets::Spacer::new())
        .child(count);
...
fn build_list(ctx: &mut BuildContext, state: &AppState) -> WidgetId {
    ...
    let entries: Vec<_> = teksilo_preview::entries_by_group();
...
fn build_entry_row(
    ...
    state: &AppState,
    entry: &'static dyn teksilo_preview::CatalogEntry,
) -> WidgetId {
