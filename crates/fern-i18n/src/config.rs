...
///
/// Built up fluently and passed to `FernAppBuilder::i18n(...)` (Phase B).
...
pub struct I18nConfig {
    ...
    /// Framework bundles supplied by libraries like `fern-widgets` via
...
impl I18nConfig {
    ...
    ///
    /// Typical usage: pass `fern_widgets::framework_locales()` so the
    ...
    /// independently, so an application can combine fern-widgets with
    ...
    /// of fern-widgets' a11y labels when fern-widgets itself only
    ...
    /// Read-only accessor used by `FernAppBuilder::run` to construct
