...
//!
//! 1. **Bundle-side** — `manager.rs` installs [`fern_format_callback`] via
...
//! 2. **Signal-side** — [`NumberFormatter`] and [`FernDateTimeFormatter`] take
...
// ------------------------------------------------------------
// FernDateTime — the FluentValue::Custom payload
// ------------------------------------------------------------

#[derive(Clone, Debug, PartialEq)]
enum FernDateTimeInner {
...
#[derive(Clone, Debug, PartialEq)]
pub struct FernDateTime {
    inner: FernDateTimeInner,
...
impl FernDateTime {
    pub fn from_civil(dt: jiff::civil::DateTime) -> Self {
        Self {
            inner: FernDateTimeInner::Civil(dt),
    ...
    pub fn from_zoned(z: jiff::Zoned) -> Self {
        Self {
            inner: FernDateTimeInner::Zoned(z),
            options: DateTimeOptions::default(),
        }
    }
...
impl From<jiff::civil::DateTime> for FernDateTime {
    fn from(dt: jiff::civil::DateTime) -> Self {
        Self::from_civil(dt)
    }
}

impl From<jiff::Zoned> for FernDateTime {
    fn from(z: jiff::Zoned) -> Self {
        Self::from_zoned(z)
    }
}

impl From<FernDateTime> for FluentValue<'static> {
    fn from(f: FernDateTime) -> Self {
        FluentValue::Custom(Box::new(f))
    }
}

impl FluentType for FernDateTime {
    ...
    fn as_string_threadsafe(
        ...
        // fern-i18n uses fluent-bundle's default (non-threadsafe) memoizer
...
/// the ICU cache; `FluentValue::Custom` (e.g. our `FernDateTime`) falls
/// through to `MemoizerKind::stringify_value` which calls
/// `FluentType::as_string`.
pub(crate) fn fern_format_callback(
    value: &FluentValue,
...
/// `FluentValue::Custom(FernDateTime)`; merges named args (`dateStyle`,
...
pub(crate) fn datetime_function<'a>(
    ...
    let Some(fdt) = any.downcast_ref::<FernDateTime>() else {
        return FluentValue::Error;
    };
...
/// [`NumberFormatter`]. Named with the `Fern` prefix to avoid colliding
/// with `icu_datetime::DateTimeFormatter` (which fern-i18n uses
/// internally to back this type).
#[derive(Clone, Debug, Default)]
pub struct FernDateTimeFormatter {
    options: DateTimeOptions,
}

impl FernDateTimeFormatter {
    pub fn new() -> Self {
        Self::default()
    }
