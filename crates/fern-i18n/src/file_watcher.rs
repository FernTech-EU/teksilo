...
//! `FernAppBuilder::run` then spins up a single `FtlFileWatcher` that
...
//! that fern-app implements by posting a `AppEvent::I18nReload` through
//! the winit `EventLoopProxy`. Keeping the sink as a plain `Fn` trait
//! object means `fern-i18n` does not need to know anything about winit
//! — the coupling lives in fern-app.
//!
    ...
    pub fn new(
        ...
        for (locale, path) in entries {
            let canonical = match path.canonicalize() {
                ...
                Err(e) => {
                    eprintln!(
                        "fern-i18n: cannot watch `{}` ({e}); hot-reload disabled for {locale}",
                        path.display()
                    );
                    continue;
                }
        ...
        let mut watcher = notify::recommended_watcher(
            move |res: Result<notify::Event, notify::Error>| match res {
                ...
                Ok(_) => {}
                Err(e) => {
                    eprintln!("fern-i18n: watcher error: {e}");
                }
            },
        )?;
