...
//! Formatter for the `teksu!` DSL.
//!
//! Pure library — the CLI lives in `cargo-teksilo-fmt`. Two entry points:
//!
//! - [`format_block`] — format the body of a single `teksu!(...)` macro
//!   invocation. Input is the contents inside the parens, including any
//!   `ctx => ` preamble.
//! - [`format_file`] — format every `teksu!(...)` invocation in a Rust
//!   source file. Returns the full file content with each macro body
//!   replaced by its formatted form. Source outside `teksu!` blocks is
//!   untouched.
//!
...
// Re-exports for downstream tooling (e.g. `teksilo-designer`) that needs
// to locate, scan, and re-indent `teksu!` blocks rather than just format
// whole files.
pub use trivia::{Trivia, TriviaKind, scan as scan_trivia};
pub use visit::{TeksiMacroEdit, find_teksilo_macros};
...
pub enum FmtError {
    /// The `teksu!` body failed to parse. Carries the underlying syn error
    /// with its span pointing into the input source.
    #[error("teksu! body parse error: {0}")]
    Parse(syn::Error),
...
/// Format the body of a single `teksu!(...)` invocation.
///
/// `source` is the text between the macro parens — for `teksu!(ctx => VStack {})`
/// pass `"ctx => VStack {}"`. The returned string is the reformatted body
/// without surrounding parens or `teksu!`.
///
...
pub fn format_block(source: &str, _config: &FmtConfig) -> Result<String, FmtError> {
    ...
    let root = teksilo_parse::parse_root(tokens.clone()).map_err(FmtError::Parse)?;
    let trivia = trivia::scan(source, &tokens);
    Ok(printer::print(source, &root, &trivia))
}

/// Format every `teksu!(...)` invocation in a Rust source file.
///
/// `source` is the full Rust source text. The returned string is the
/// same source with each `teksu!` macro body replaced by its formatted
/// form. Source outside `teksu!` blocks is byte-for-byte unchanged. The
...
pub fn format_file(source: &str, config: &FmtConfig) -> Result<String, FmtError> {
    ...
    let macros = visit::find_teksilo_macros(source, &file);
    ...
    let mut edits: Vec<visit::TeksiMacroEdit> = macros;
    ...
    for edit in edits {
        ...
        // expression and leaves the rest alone. Because teksu! separates fields
