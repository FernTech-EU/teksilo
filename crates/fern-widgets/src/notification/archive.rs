...
//!
//! Wraps a [`ListModel<NotificationEntry>`](fern_data::ListModel)
//! with two extras:
...
//!   [`PersistedListModel`](fern_settings::PersistedListModel).
...
//!   gets re-serialized on the shared fern-settings I/O thread.
...
use fern_settings::{AppPaths, Migrator, PersistedListModel, SettingsFileError};
...
// the install helper in `fern-ui` puts the model in `app_state` as
...
mod tests {
    ...
    fn entry_serde_round_trip() {
        ...
        let wrapper = fern_settings::ListFile {
            version: 1,
            items: vec![original.clone()],
        };
        let serialized = toml::to_string(&wrapper).expect("serialize");
        let parsed: fern_settings::ListFile<NotificationEntry> =
