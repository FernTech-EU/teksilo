...
use fern_core::styles::{SharedToastStyle, ToastStyleConfig};
...
pub struct ToastSurfaceData {
    ...
    pub priority: fern_core::styles::ToastPriority,
    pub title: String,
    pub body: Option<String>,
    pub announcement: Option<String>,
    pub actions: Rc<Vec<ToastAction>>,
    pub show_close_button: bool,
    pub on_click: Option<Rc<dyn Fn(&mut fern_core::widget::EventContext)>>,
...
impl ToastSurface {
    ...
    fn at_role(&self) -> fern_core::accesskit::Role {
        use fern_core::styles::ToastPriority;
        let elevated_priority = matches!(
            self.data.priority,
            ToastPriority::High | ToastPriority::Urgent
        );
        match (self.data.severity, elevated_priority) {
            (ToastSeverity::Error, _) => fern_core::accesskit::Role::Alert,
            (ToastSeverity::Warning, true) => fern_core::accesskit::Role::Alert,
            _ => fern_core::accesskit::Role::Status,
        }
    }

    /// `Live::Polite` for Status, `Live::Assertive` for Alert.
    /// `Urgent` priority forces Assertive regardless of severity.
    fn at_live(&self) -> fern_core::accesskit::Live {
        use fern_core::styles::ToastPriority;
        if matches!(self.data.priority, ToastPriority::Urgent) {
            return fern_core::accesskit::Live::Assertive;
        }
        match self.at_role() {
            fern_core::accesskit::Role::Alert => fern_core::accesskit::Live::Assertive,
            _ => fern_core::accesskit::Live::Polite,
        }
    }
