...
use fern_canvas::{Point, Rect, Size, SizeProposal, Transform2D};
...
mod tests {
    ...
    fn zero_angle_emits_identity_skip() {
        ...
        let push_count = frame
            .draw_order
            .iter()
            .filter(|c| matches!(c, fern_canvas::DrawCommand::PushTransform(_)))
            .count();
    ...
    fn nonzero_angle_emits_rotation_matrix() {
        ...
        let pushes: Vec<&Transform2D> = frame
            ...
            .filter_map(|c| match c {
                fern_canvas::DrawCommand::PushTransform(t) => Some(t),
                _ => None,
            })
            .collect();
