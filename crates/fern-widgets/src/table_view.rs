...
pub use fern_data::SortDirection;
...
pub struct TableView<T: 'static> {
    ...
    on_cell_edit_request: Option<Rc<dyn Fn(usize, &str, &mut fern_core::widget::EventContext)>>,
...
impl<T: 'static> TableView<T> {
    ...
    /// [`SortFilterListModel<T>`](fern_data::SortFilterListModel)).
    ...
    pub fn on_cell_edit_request(
        mut self,
        f: impl Fn(usize, &str, &mut fern_core::widget::EventContext) + 'static,
    ) -> Self {
    ...
    pub fn on_row_activate(
        mut self,
        f: impl Fn(usize, &mut fern_core::widget::EventContext) + 'static,
    ) -> Self {
    ...
    /// Bind a [`SortFilterListModel`](fern_data::SortFilterListModel) to
...
impl<T: 'static> Widget for TableView<T> {
        ...
        *self.display_indices.borrow_mut() = display_indices_now.clone();
        ...
        let handlers = HandlerSet::new()
            ...
            .on_drag_hover(|payload, _pos, _ctx| {
                ...
                {
                    fern_core::DropFeedback::HighlightRect {
                        rect: fern_canvas::Rect::ZERO,
                        color: fern_tokens::Color::TRANSPARENT,
                    }
                } else {
                    fern_core::DropFeedback::NoFeedback
                }
            })
    ...
    fn accessibility(&self, builder: &mut AccessNodeBuilder) {
        builder.set_role(fern_core::accesskit::Role::Table);
