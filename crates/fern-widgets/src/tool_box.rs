...
//!   [`MotionTokens`](fern_tokens::MotionTokens). Matches the existing
...
impl Widget for ToolBoxHeader {
    fn build(&mut self, ctx: &mut BuildContext) -> Vec<WidgetId> {
        ...
        let focus_border_width = focus_origin.map(move |o| match o {
            Some(fern_core::focus::FocusOrigin::Keyboard) => focus_ring_width,
            _ => 0.0,
        });
        let focus_border_color = focus_origin.map(|o| match o {
            Some(fern_core::focus::FocusOrigin::Keyboard) => BorderRole::Focused,
        ...
        let handler_set = HandlerSet::new()
            ...
            .on_access_action(move |action, _ctx| {
                match action {
                    ...
                    // invariant. Swallow.
                    fern_core::accesskit::Action::Collapse => EventResponse::Handled,
                    _ => EventResponse::Ignored,
                }
            })
            .focusable(enabled)
        ...
        fn find_button_inside(
            ...
            for child in t.children(root) {
                if child != outer {
                    ...
                    if info.role() == fern_core::accesskit::Role::Button {
                        return Some(child);
                    }
                }
