...
//! The model is a [`ListModel<NotificationEntry>`](fern_data::ListModel)
...
//!   [`fern_settings::PersistedListModel`] so the archive survives
//!   app restarts. Standard atomic write-temp+rename through the
//!   shared fern-settings I/O thread.
//!
...
/// and `Serialize` would have to be re-exported through fern-tokens.
