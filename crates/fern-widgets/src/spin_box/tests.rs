...
fn focus_field(tree: &mut WidgetTree, spin_id: fern_core::widget_id::WidgetId) {
    let field = tree
...
fn a11y_role_is_spin_button() {
    ...
    assert_eq!(info.role(), fern_core::accesskit::Role::SpinButton);
    // Increment / Decrement / SetValue / Focus actions all exposed.
    let actions = info.actions();
    assert!(actions.contains(&fern_core::accesskit::Action::Increment));
    assert!(actions.contains(&fern_core::accesskit::Action::Decrement));
    assert!(actions.contains(&fern_core::accesskit::Action::Focus));
}
...
fn show_buttons_sugar_matches_button_layout() {
    ...
    fn count_focusable(tree: &WidgetTree, root: fern_core::widget_id::WidgetId) -> usize {
...
fn a11y_numeric_value_matches_signal() {
    ...
    for required in [
        ...
        fern_core::accesskit::Action::SetValue,
        fern_core::accesskit::Action::Focus,
    ] {
        assert!(
