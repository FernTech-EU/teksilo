...
impl Widget for CellA11y {
    ...
    fn accessibility(&self, builder: &mut AccessNodeBuilder) {
        builder.set_role(if self.is_row_header {
            fern_core::accesskit::Role::RowHeader
        } else {
            fern_core::accesskit::Role::Cell
        });
    ...
    fn accessibility(&self, builder: &mut AccessNodeBuilder) {
        ...
        if let Some(dir) = self.sort {
            let ak_dir = match dir {
                SortDirection::Ascending => fern_core::accesskit::SortDirection::Ascending,
                SortDirection::Descending => fern_core::accesskit::SortDirection::Descending,
            };
