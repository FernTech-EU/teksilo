//! Avatar image masking — anti-aliased circle / rounded-square / square
//! alpha mask applied to RGBA8 pixel buffers at construction time.
//!
//! The retained renderer's `Canvas::set_clip` is rectangular-only, so to
//! crop an Avatar's image into a circle we modulate the source image's
//! alpha channel with a per-pixel coverage value computed analytically.
//! 4× × 4× super-sampling (16 sub-samples per pixel) gives a smooth edge
//! at the small sizes Avatars actually use (≤96 logical pixels).
//!
//! The same RGBA pixel-walking pattern is used by
//! [`fern_canvas::raster::RasterIcon::to_alpha_mask`] for icon tinting.
//! The functions here are crate-private — they're exercised through the
//! `Avatar` constructor surface.

/// Shape used to derive per-pixel coverage. The `RoundedSquare` radius
/// is in source-image pixels (clamped to `min(width, height) / 2`).
...
pub(super) fn center_crop_square(pixels: &[u8], width: u32, height: u32) -> (Vec<u8>, u32) {
...
mod tests {
    ...
    fn mask_circle_zeros_corners() {
        ...
        apply_alpha_mask(&mut pixels, 32, 32, MaskShape::Circle);
    ...
    fn mask_rounded_square_radius_zero_is_passthrough() {
        ...
        apply_alpha_mask(&mut pixels, 16, 16, MaskShape::RoundedSquare(0.0));
    ...
    fn mask_rounded_square_full_radius_equals_circle() {
        ...
        apply_alpha_mask(&mut a, 24, 24, MaskShape::Circle);
        apply_alpha_mask(&mut b, 24, 24, MaskShape::RoundedSquare(12.0));
    ...
    fn mask_preserves_rgb() {
        let mut pixels = solid(16, 16);
        apply_alpha_mask(&mut pixels, 16, 16, MaskShape::Circle);
    ...
    #[test]
    fn mask_square_is_noop() {
        let mut pixels = solid(8, 8);
        apply_alpha_mask(&mut pixels, 8, 8, MaskShape::Square);
        for j in 0..8 {
            for i in 0..8 {
    ...
    fn mask_handles_size_one_image() {
        ...
        apply_alpha_mask(&mut pixels, 1, 1, MaskShape::Circle);
