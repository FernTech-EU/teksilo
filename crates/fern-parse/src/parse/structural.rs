...
//!
//! Each form's body is a single fern element (spec §5.1–§5.3). Users
...
use crate::diag;
use crate::ir::{FernElse, FernFor, FernIf, FernMatch, FernMatchArm};

use super::parse_element;

/// Parse an `if` chain: `if cond { Elem } [else if cond { Elem }]* [else { Elem }]?`.
pub(crate) fn parse_if(input: ParseStream) -> Result<FernIf> {
    let if_token: Token![if] = input.parse()?;
    let span = if_token.span;
    parse_if_tail(input, span)
}

fn parse_if_tail(input: ParseStream, span: proc_macro2::Span) -> Result<FernIf> {
...
pub(crate) fn parse_match(input: ParseStream) -> Result<FernMatch> {
...
fn parse_match_arm(input: ParseStream) -> Result<FernMatchArm> {
...
pub(crate) fn parse_for(input: ParseStream) -> Result<FernFor> {
