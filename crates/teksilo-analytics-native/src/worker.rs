...
use teksilo_core::telemetry::{OwnedEvent, OwnedProp, OwnedPropValue};
use teksilo_telemetry::EventQueue;
...
use crate::config::TeksiloConfig;
...
pub(crate) fn spawn_worker(
    ...
    // shutdown path that joins from the TeksiloAdapter::Drop site.
    let handle = thread::Builder::new()
        .name("teksilo-collector-client".to_string())
        .spawn(move || {
            rt_for_thread.block_on(worker_loop(config, queue, rx, stats));
        })
        .expect("spawn teksilo-collector worker thread");
    ...
    match send_batch(config, channel, batch_id, batch).await {
        ...
        Err((events, reason)) => {
            ...
            *channel = None;
            ...
            stats.queued.store(queue.len(), Ordering::Relaxed);
            tracing_log(format!(
                "teksilo-collector send failed (batch={batch_id}, count={batch_count}): {reason}"
            ));
            Err(reason)
        }
    }
...
fn owned_to_proto_event(ev: &OwnedEvent) -> proto::Event {
    ...
    let category = match ev.category {
        teksilo_core::telemetry::EventCategory::Intent => proto::EventCategory::Intent,
        teksilo_core::telemetry::EventCategory::Lifecycle => proto::EventCategory::Lifecycle,
        teksilo_core::telemetry::EventCategory::Navigation => proto::EventCategory::Navigation,
        teksilo_core::telemetry::EventCategory::Census => proto::EventCategory::Census,
        teksilo_core::telemetry::EventCategory::Custom => proto::EventCategory::Custom,
    } as i32;
    proto::Event {
        name: ev.name.clone(),
        category,
...
use teksilo_core::telemetry::{RemoteEvent, RemoteValue};
