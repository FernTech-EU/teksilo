...
//!
//! `Theme` lives in `fern-core` (not `fern-tokens`) so the per-widget
//! style trait protocols and the typed `Arc<dyn FooStyle>` slots that
//! will eventually replace [`fern_tokens::ComponentStyles`] can sit on
...
impl Theme {
    ...
    /// preset constructor (e.g. `fern_core::presets::intui::light`)
