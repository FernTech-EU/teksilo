...
pub enum DrawCommand {
    ...
    /// `WidgetArena::effective_transform` in `bastyde-core` for the
...
/// `bastyde-render/src/shaders/anim_procedural.wgsl` (and the sprite
/// variant). `repr(C)` with explicit `_pad` fields for `std140`
/// compatibility.
///
/// Lives in `bastyde-canvas` (not `bastyde-core`) because it is the
