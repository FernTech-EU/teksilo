...
//! the FernAdapter at it, fire events, confirm acks and Parquet
//! files. Mirrors the structure of the Plausible adapter integration
//! test ([crates/fern-analytics-plausible/tests/integration.rs]).
//!
//! These tests cover sub-phase A acceptance: "a FernUI app emits
...
use fern_collector_proto::v1::telemetry_server::{Telemetry, TelemetryServer};
...
fn events_persist_across_simulated_process_restart() {
    ...
    let queue_path = dir.path().join("fern-queue.redb");
...
fn per_product_scope_enforced_at_batch_level() {
    ...
    let adapter_a = FernAdapter::builder()
        .endpoint(server.endpoint())
    ...
    let adapter_b = FernAdapter::builder()
        .endpoint(server.endpoint())
...
fn anonymous_adapter_returns_unsupported_for_fetch_and_erase() {
    ...
    assert!(matches!(
        ...
        Err(fern_core::telemetry::TelemetryError::FetchUnsupported)
    ));
    assert!(matches!(
        adapter.erase_remote_data(),
        Err(fern_core::telemetry::TelemetryError::ErasureUnsupported)
    ));
...
fn pseudonymous_adapter_round_trips_fetch_and_erase() {
    ...
    assert_eq!(export.adapter, "fern-collector");
