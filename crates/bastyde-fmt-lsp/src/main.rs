...
//! - `textDocument/formatting` — run `bastyde_fmt::format_file` on the
...
//! ```
//!
//! Most editors already run `rust-analyzer` for `.rs` files; bastyde-fmt-lsp
//! is meant to coexist as a *secondary* formatter. Users who don't
//! have a way to chain formatters can keep using `cargo bastyde-fmt` on
...
impl Server {
    ...
    fn handle_formatting(&self, id: Value, params: &Value) -> Value {
        ...
        match format_file(text, &cfg) {
            ...
            // A parse error in the host file or in a bati! body is not
