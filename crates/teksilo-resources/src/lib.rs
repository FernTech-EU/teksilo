...
//! Compile-time-validating resource embedding proc macro for Teksilo.
//!
...
/// `teksilo-canvas`, which the generated code calls at runtime.
///
...
fn validate_svg(data: &[u8], path: &str) -> std::result::Result<(), String> {
    ...
    let icon = teksilo_canvas::svg::SvgIcon::parse(text).map_err(|e| format!("{path}: {e}"))?;
    if icon.is_empty() {
        return Err(format!(
...
fn emit_expr(kind: ResourceKind, rel_path: &str, animated: bool) -> TokenStream2 {
    ...
    quote! {
        {
            #watch
            static __TEKSILO_RES: ::std::sync::LazyLock<#ty> =
...
/// Return the (type, initializer_body) for a LazyLock based on resource kind.
///
/// Paths go through `::teksilo::canvas::` so consuming crates only need
/// to depend on `teksilo` (the umbrella), not on `teksilo-canvas` directly.
...
fn lazy_type_and_init(
    ...
    match kind {
        ResourceKind::Svg => (
            quote!(::teksilo::canvas::svg::SvgIcon),
            quote! {
                ::teksilo::canvas::svg::SvgIcon::parse(
                    ::core::include_str!(::core::concat!(::core::env!("CARGO_MANIFEST_DIR"), "/", #rel_path))
                ).expect("teksilo-resources: SVG validated at compile time")
            },
        ),
        ResourceKind::Png => (
            quote!(::teksilo::canvas::raster::RasterIcon),
            quote! {
                ::teksilo::canvas::raster::RasterIcon::decode_png(
                    ::core::include_bytes!(::core::concat!(::core::env!("CARGO_MANIFEST_DIR"), "/", #rel_path))
                ).expect("teksilo-resources: PNG validated at compile time")
            },
        ),
        ResourceKind::Webp if animated => (
            quote!(::teksilo::canvas::animated::AnimatedIcon),
            quote! {
                ::teksilo::canvas::animated::AnimatedIcon::decode_webp(
                    ::core::include_bytes!(::core::concat!(::core::env!("CARGO_MANIFEST_DIR"), "/", #rel_path))
                ).expect("teksilo-resources: animated WebP validated at compile time")
            },
        ),
        ResourceKind::Webp => (
            quote!(::teksilo::canvas::raster::RasterIcon),
            quote! {
                ::teksilo::canvas::raster::RasterIcon::decode_webp(
                    ::core::include_bytes!(::core::concat!(::core::env!("CARGO_MANIFEST_DIR"), "/", #rel_path))
                ).expect("teksilo-resources: WebP validated at compile time")
            },
        ),
    }
}
...
pub fn res(input: TokenStream) -> TokenStream {
    ...
    let data = match std::fs::read(&abs_path) {
        ...
        Err(e) => {
            return syn::Error::new(
                span,
                format!(
                    "teksilo-resources: cannot read `{}`: {e}",
                    abs_path.display()
                ),
            )
            .to_compile_error()
    ...
    if let Some(k) = kind {
        ...
        if let Err(msg) = validation_result {
            return syn::Error::new(span, format!("teksilo-resources: {msg}"))
                .to_compile_error()
                .into();
        }
    }
