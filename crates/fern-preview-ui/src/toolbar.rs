...
//! scaling transform onto the canvas) which fern-canvas does not
...
fn labelled(label: &str, control: WidgetId) -> impl fern_core::widget::Widget + 'static {
...
fn build_locale_picker(ctx: &mut BuildContext, state: &AppState) -> WidgetId {
    ...
    let _ = ComboBox::<String>::new(Vec::<String>::new(), fern_core::signal::Signal::new(None));
    ctx.add(row)
}
