...
use fern_preview::{KnobOverrides, KnobValues, PreviewVariant};
use fern_widgets::{Center, Expand, SplitView, TextWidget, VStack};
...
impl AppState {
    ...
    pub fn knobs_for(
        ...
        let entry = match fern_preview::find_by_id(widget_id) {
            Some(e) => e,
            None => panic!(
    ...
    pub fn select_first_registered(&self) {
        if let Some(entry) = fern_preview::iter_entries().next() {
    ...
    pub fn select_widget(&self, widget_id: &'static str) {
        ...
        let entry = fern_preview::find_by_id(widget_id).expect("entry exists");
...
impl PreviewerRoot {
    ...
    fn resolve_initial_widget(&self) -> Option<&'static str> {
        ...
        fern_preview::find_by_id(id).map(|e| e.id())
    }
...
impl Widget for PreviewerRoot {
    fn build(&mut self, ctx: &mut BuildContext) -> Vec<WidgetId> {
        ...
        if fern_preview::iter_entries().next().is_none() {
            let empty = ctx.add(
                Center::new().child(
    ...
    fn accessibility(&self, builder: &mut AccessNodeBuilder) {
        builder.set_role(fern_core::accesskit::Role::Window);
        builder.set_name("FernUI Widget Previewer");
    }
}
