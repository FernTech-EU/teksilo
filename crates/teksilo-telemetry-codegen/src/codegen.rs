...
fn generate_emit_fn(event: &EventDef, schema_version: u32) -> TokenStream {
    ...
    let mut params: Vec<TokenStream> = vec![
        quote! { reporter: &dyn ::teksilo_telemetry::UsageReporter },
    ...
    let props_expr = if prop_entries.is_empty() {
        quote! { let props: [::teksilo_telemetry::Prop<'_>; 0] = []; }
    } else {
    ...
    quote! {
        pub fn #fn_name(#(#params),*) {
            #props_expr
            reporter.record(&::teksilo_telemetry::Event {
                name: #event_name_lit,
...
fn prop_tokens(event: &EventDef, prop: &PropDef) -> (TokenStream, TokenStream) {
    ...
    match prop.ty.as_str() {
        "dev_static" => (
            quote! { &'static str },
            quote! { ::teksilo_telemetry::PropValue::StaticStr(#param_name) },
        ),
        "bounded_str" => (
            quote! { &str },
            quote! { ::teksilo_telemetry::PropValue::BoundedStr(#param_name) },
        ),
        "u32" => (
            quote! { u32 },
            quote! { ::teksilo_telemetry::PropValue::U32(#param_name) },
        ),
        "i64" => (
            quote! { i64 },
            quote! { ::teksilo_telemetry::PropValue::I64(#param_name) },
        ),
        "bool" => (
            quote! { bool },
            quote! { ::teksilo_telemetry::PropValue::Bool(#param_name) },
        ),
        "f64_bucket" => (
            quote! { ::teksilo_telemetry::F64Bucket },
            quote! { ::teksilo_telemetry::PropValue::F64Bucket(#param_name) },
        ),
        "enum" => {
            let enum_ty = Ident::new(&enum_type_name(&event.name, &prop.name), Span::call_site());
            (
                quote! { #enum_ty },
                quote! { ::teksilo_telemetry::PropValue::Enum { variant: #param_name.as_str() } },
            )
        }
...
fn category_tokens(category: &str) -> TokenStream {
    match category {
        "intent" => quote! { ::teksilo_telemetry::EventCategory::Intent },
        "lifecycle" => quote! { ::teksilo_telemetry::EventCategory::Lifecycle },
        "navigation" => quote! { ::teksilo_telemetry::EventCategory::Navigation },
        "census" => quote! { ::teksilo_telemetry::EventCategory::Census },
        "custom" => quote! { ::teksilo_telemetry::EventCategory::Custom },
