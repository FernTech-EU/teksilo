    ...
    pub(crate) fn build_body(&self, events: &[OwnedEvent]) -> serde_json::Value {
        let mut resource_attrs = vec![
            ...
            KeyValue {
                ...
                value: AnyValue::String(Cow::Borrowed("fern-analytics-otlp")),
            },
            KeyValue {
...
fn category_str(c: fern_core::telemetry::EventCategory) -> &'static str {
...
mod tests {
    ...
    #[test]
    fn props_become_fern_namespaced_attributes() {
        let b = WireBuilder {
            service_name: "test.app".into(),
            service_version: "1.0".into(),
        };
        ...
        let prop_name = attrs
            .iter()
            .find(|a| a["key"] == "fern.name")
            .expect("fern.name attribute");
        assert_eq!(prop_name["value"]["stringValue"], "app.save");
        let prop_n = attrs
            .iter()
            .find(|a| a["key"] == "fern.n")
            .expect("fern.n attribute");
