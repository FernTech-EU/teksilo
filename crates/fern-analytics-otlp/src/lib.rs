//! `fern-analytics-otlp` — OpenTelemetry-compatible analytics
//! adapter for FernUI.
//!
...
//! event.schema_version     attributes["fern.schema_version"]
//! ```
//!
...
//!
//! ```ignore
//! use fern_analytics_otlp::OtlpAdapter;
//! use std::rc::Rc;
//!
//! let adapter = Rc::new(
//!     OtlpAdapter::builder()
...
//!         .build(),
//! ) as Rc<dyn fern_telemetry::UsageReporter>;
//! ```
//!
//! # Fetch + erase
//!
...
//! # Queue durability
//!
//! Unlike `fern-analytics-plausible` and `fern-analytics-fern`,
...
//! Plausible/Fern shape is straightforward — the worker already
...
impl OtlpAdapterBuilder {
    ...
    /// matches the same shape as the Plausible/Fern adapters.
