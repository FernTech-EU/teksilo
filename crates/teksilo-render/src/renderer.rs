...
struct ActiveTarget {
    ...
    blur_bounds: Option<teksilo_canvas::Rect>,
...
impl Renderer {
    ...
    pub fn render(
            ...
            vec![None; frame.paths.len()];
        {
            ...
            for cmd in &frame.draw_order {
                match cmd {
                    teksilo_canvas::DrawCommand::SetTransform(t) => {
                    ...
                    teksilo_canvas::DrawCommand::Path(idx) => {
...

        {
            ...
            // Blend mode stack
            let mut blend_stack: Vec<teksilo_canvas::BlendMode> = Vec::new();
            let mut current_blend = teksilo_canvas::BlendMode::Normal;
            ...
            while cmd_idx <= frame.draw_order.len() {
...

                {
                    let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                        label: Some("teksilo_segment_pass"),
                    ...
                    while cmd_idx < frame.draw_order.len() {
                        ...
                        match cmd {
                            teksilo_canvas::DrawCommand::Decoration(idx) => {
                                flush_all!(
                                    pass,
                                    &self.queue,
                                    self.streams,
                            ...
                            teksilo_canvas::DrawCommand::CosmeticLine(idx) => {
                                flush_all!(
                                    pass,
                                    &self.queue,
                                    self.streams,
                            ...
                            teksilo_canvas::DrawCommand::Shape(idx) => {
                                flush_all!(
                                    pass,
                                    &self.queue,
                                    self.streams,
                                ...
                                let verts = if shape.stroke_space
                                    == teksilo_canvas::StrokeSpace::Device
                            ...
                            teksilo_canvas::DrawCommand::Glyph(idx) => {
                            ...
                            teksilo_canvas::DrawCommand::Shadow(idx) => {
                                flush_all!(
                                    pass,
                                    &self.queue,
                                    self.streams,
                            ...
                            teksilo_canvas::DrawCommand::Image(idx) => {
                            ...
                            teksilo_canvas::DrawCommand::Path(idx) => {
                                ...
                                if let Some(Some(region)) = path_regions.get(*idx) {
                                    ...
                                    if matches!(entry.paint_data, teksilo_canvas::PaintData::Solid)
                                    {
                            ...
                            teksilo_canvas::DrawCommand::SetClip(rect) => {
                                flush_all!(
                                    pass,
                                    &self.queue,
                                    self.streams,
                            ...
                            teksilo_canvas::DrawCommand::ClearClip => {
                                flush_all!(
                                    pass,
                                    &self.queue,
                                    self.streams,
                            ...
                            teksilo_canvas::DrawCommand::SetOpacity(opacity) => {
                                flush_all!(
                                    pass,
                                    &self.queue,
                                    self.streams,
                            ...
                            teksilo_canvas::DrawCommand::RestoreOpacity => {
                                flush_all!(
                                    pass,
                                    &self.queue,
                                    self.streams,
                            ...
                            teksilo_canvas::DrawCommand::AnimatedQuad(idx) => {
                                ...
                                quad_source = None;
                                match &draw.class {
                                    teksilo_canvas::AnimatedQuadClass::Procedural => {
                                        let verts =
                                    ...
                                    teksilo_canvas::AnimatedQuadClass::Sprite { image_name } => {
                            ...
                            teksilo_canvas::DrawCommand::SetBlendMode(mode) => {
                                blend_stack.push(current_blend);
                                current_blend = *mode;
                            }
                            teksilo_canvas::DrawCommand::RestoreBlendMode => {
                                current_blend = blend_stack
                                    .pop()
                                    .unwrap_or(teksilo_canvas::BlendMode::Normal);
                            }
                            teksilo_canvas::DrawCommand::SetTransform(t) => {
                                flush_all!(
                                    pass,
            ...
            if !final_composites.is_empty() {
                ...
                surface.opened = true;
                let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                    label: Some("teksilo_final_composite_pass"),
                    color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                        view,
                        resolve_target: None,
                        ops: wgpu::Operations {
                ...
                .opened
            {
                ...
                let _ = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                    label: Some("teksilo_empty_clear_pass"),
                    color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                        view,
                        resolve_target: None,
                        ops: wgpu::Operations {
    ...
    fn draw_image(
        ...
        image: &teksilo_canvas::ImageQuad,
        scale_factor: f32,
        viewport_width: u32,
        viewport_height: u32,
        opacity: f32,
        ...
        let (color, flags) = if let Some(tint) = image.tint {
            // Tint colors are sRGB-encoded (from teksilo_tokens::Color) — linearize
...
pub(crate) fn stream_quad_counts(frame: &RenderFrame) -> StreamQuadCounts {
    ...
    let gradient_paths = frame
        .paths
        .iter()
        .filter(|p| !matches!(p.paint_data, teksilo_canvas::PaintData::Solid))
        .count();
...
fn create_anim_proc_pipeline(
    ...
    let buffer_size = (MAX_ANIM_SLOTS * std::mem::size_of::<teksilo_canvas::AnimParams>()) as u64;
...
mod tests {
    use teksilo_canvas::RenderFrame;
    use teksilo_canvas::render_frame::{DrawCommand, GlyphQuad, PaintData, ShapeKind, ShapeQuad};
    ...
    fn stream_quad_counts_includes_sprite_anim_quads() {
        ...
        // was on screen.
        use teksilo_canvas::render_frame::{AnimatedQuadClass, AnimatedQuadDraw};
    ...
    fn stream_quad_counts_splits_solid_and_gradient_paths() {
        ...
        use teksilo_canvas::render_frame::PathEntry;
        use teksilo_canvas::{FillRule, GradientStop, Path, StrokeStyle};
    ...
    fn gradient_path_renders_nonflat_on_gpu() {
        ...
        use teksilo_canvas::render_frame::PathEntry;
        use teksilo_canvas::{FillRule, GradientStop, Path, Rect, StrokeStyle};
        use teksilo_tokens::Color;
        ...
        let texture = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("teksilo_render_gradient_path_target"),
            size: wgpu::Extent3d {
                width: 32,
                height: 32,
                depth_or_array_layers: 1,
    ...
    fn glyph_quad_renders_over_shape_in_offscreen_target() {
        ...
        frame.shapes.push(ShapeQuad {
            ...
            stroke_width: 0.0,
            stroke_space: teksilo_canvas::StrokeSpace::Logical,
            corner_radii: [0.0; 4],
            paint_data: PaintData::Solid,
        });
        ...
        let texture = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("teksilo_render_test_target"),
            size: wgpu::Extent3d {
                width: 32,
                height: 32,
                depth_or_array_layers: 1,
    ...
    fn fractional_origin_glyph_renders_pixel_exact() {
        ...
        let texture = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("teksilo_render_snap_test_target"),
            size: wgpu::Extent3d {
                width: 32,
                height: 32,
                depth_or_array_layers: 1,
