...
//! Integration tests for the Phase B i18n wiring on `BastydeAppBuilder` and
...
use bastyde_i18n::{I18nConfig, LanguageIdentifier, LocalizedString, localized, resolve_message};

fn lid(s: &str) -> LanguageIdentifier {
    s.parse().unwrap()
}

fn build_trilingual_headless() -> bastyde_app::HeadlessApp {
...
fn i18n_manager_is_installed_after_build_headless() {
    ...
    let locale = bastyde_i18n::current_locale().expect("manager not installed");
    assert_eq!(locale.get().to_string(), "en-US");

    let direction = bastyde_i18n::current_direction().expect("direction signal missing");
...
fn no_i18n_config_keeps_manager_unset_and_set_locale_is_noop() {
    ...
    let mut app = BastydeAppBuilder::new().build_headless();
