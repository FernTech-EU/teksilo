...
//! Integration test for the `BastydeAppBuilder::settings(...)` pipeline.
//!
//! Verifies that:
...
fn settings_off_by_default() {
    let app = BastydeAppBuilder::new().build_headless();
...
fn application_panics_only_when_paths_unresolvable() {
    ...
    let _ = std::panic::catch_unwind(|| {
        BastydeAppBuilder::new()
            .application("test", "bastyde-ci", "settings-pipeline-smoke")
            .build_headless()
    });
}
