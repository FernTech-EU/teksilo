...
//! [`install_automation_bridge_in_debug`](BastydeAppBuilderAutomationExt::install_automation_bridge_in_debug),
//! a background thread binds a private Unix-domain socket and lets
//! `bastyde-automation-mcp --connect <sock> --token <uuid>` drive the *live*
//! running app: it reads framed [`AutomationRequest`](bastyde_automation::dto::AutomationRequest)s, posts an
...
//! [`execute`](bastyde_automation::execute) (or, for screenshots,
//! [`PlatformWindow::capture_offscreen`](bastyde_platform::PlatformWindow::capture_offscreen)).
//!
...
use crate::BastydeAppBuilder;

/// Adds [`install_automation_bridge_in_debug`](Self::install_automation_bridge_in_debug)
/// to [`BastydeAppBuilder`]. Mirrors `BastydeAppBuilderInspectorExt`.
pub trait BastydeAppBuilderAutomationExt {
    /// In a **debug** build: generate a per-process token, bind a private
    /// `0600` Unix socket, print its path + `BASTYDE_AUTOMATION_TOKEN=<uuid>`
...
impl BastydeAppBuilderAutomationExt for BastydeAppBuilder {
    fn install_automation_bridge_in_debug(self) -> Self {
        install(self)
    }
}
...
#[cfg(debug_assertions)]
use bastyde_automation::dto::{AutomationReply, SettleSpec};
...
pub struct AutomationPayload {
    /// Target window (`BastydeWindowId` raw); `None` → focused else primary.
    pub window_id: Option<u64>,
    /// Informational sequence number (single-inflight, so unused for
    /// matching).
    pub request_id: u64,
    /// The operation to perform.
    pub op: bastyde_automation::dto::AutomationOp,
    /// Settle policy.
...
fn socket_dir() -> String {
    ...
    format!("{dir}/bastyde-automation-{}", std::process::id())
}
...
fn install(builder: BastydeAppBuilder) -> BastydeAppBuilder {
    ...
    let token = std::env::var("BASTYDE_AUTOMATION_TOKEN")
        .unwrap_or_else(|_| uuid::Uuid::new_v4().to_string());
    let path = socket_path();
    eprintln!("bastyde-automation: bridge socket = {path}");
    eprintln!("BASTYDE_AUTOMATION_TOKEN={token}");
    eprintln!(
        "bastyde-automation: connect with `bastyde-automation-mcp --connect {path} --token {token}`"
    );
    builder.on_ready(move |proxy| {
        if let Err(e) = spawn_bridge_thread(proxy, token) {
            eprintln!("bastyde-automation: bridge failed to start: {e}");
        }
    })
}
...
pub fn spawn_bridge_thread(proxy: crate::app::AppEventProxy, token: String) -> std::io::Result<()> {
    ...
    std::thread::Builder::new()
        .name("bastyde-automation-bridge".into())
        .spawn(move || {
...
pub fn spawn_bridge_thread(
    ...
    eprintln!(
        "bastyde-automation: the live bridge needs a Unix-domain socket and is unavailable on \
         this platform; use `bastyde-automation-mcp --headless` instead."
    );
    Ok(())
}
...
fn handle_connection(
    ...
    loop {
        ...
        let req: bastyde_automation::dto::AutomationRequest = match serde_json::from_slice(&buf) {
            Ok(r) => r,
            Err(e) => {
