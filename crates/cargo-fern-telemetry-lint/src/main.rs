//! `cargo fern-telemetry-lint` — schema-drift linter for fern-telemetry
//! event manifests.
//!
//! Run as a cargo subcommand: `cargo fern-telemetry-lint`
//! Install via: `cargo install --path crates/cargo-fern-telemetry-lint`
//!
//! # Checks performed
//!
...
//!
//! ```text
//! cargo fern-telemetry-lint [OPTIONS]
//!
//! Options:
...
fn main() {
    ...
    // When invoked as `cargo fern-telemetry-lint`, Cargo passes the
...
fn print_help() {
    println!(
        r#"cargo fern-telemetry-lint — FernUI telemetry schema drift linter
...
EXAMPLES:
    ...
    cargo fern-telemetry-lint --manifest telemetry/app_events.yaml --src src --src lib
    cargo fern-telemetry-lint --fail-on-warnings   # for CI
"#
    );
}
