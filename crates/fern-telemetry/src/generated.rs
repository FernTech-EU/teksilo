...
pub fn emit_lifecycle_app_started(
    ...
    let props = [
        ...
        Prop {
            key: "fern_ui_version",
            value: PropValue::BoundedStr(fern_ui_version),
        },
        Prop {
            key: "os",
...
/// Emit `lifecycle.app_exited`. Called from the `FernAppBuilder` graceful
...
/// Adapters with a flush-on-shutdown drain (Plausible, Fern) get
/// the census on the way out for free; the periodic firing requires
/// an app-side timer.
///
/// Caller pulls the histogram from
/// [`fern_core::widget_tree::WidgetTree::widget_type_histogram`].
