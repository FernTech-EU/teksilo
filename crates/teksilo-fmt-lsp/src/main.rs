...
//! `teksilo-fmt-lsp` — minimal Language Server Protocol server for the
//! `teksu!` DSL formatter.
//!
...
//! - `textDocument/formatting` — run `teksilo_fmt::format_file` on the
...
//! ```
//!
//! Most editors already run `rust-analyzer` for `.rs` files; teksilo-fmt-lsp
//! is meant to coexist as a *secondary* formatter. Users who don't
//! have a way to chain formatters can keep using `cargo teksilo-fmt` on
...
impl Server {
    ...
    fn handle_formatting(&self, id: Value, params: &Value) -> Value {
        ...
        match format_file(text, &cfg) {
            ...
            // A parse error in the host file or in a teksu! body is not
