...
//! coordinate space top-left origin, matching teksilo's window-logical pixels.
//!
//! Each drag phase posts an [`ExternalDndEventPayload`] through the app's
//! [`AppEventPoster`]; `teksilo-app` routes it to the window's `WidgetTree`.
...
define_class!(
    ...
    impl DropView {
        // Top-left origin so converted drag coordinates match teksilo's
