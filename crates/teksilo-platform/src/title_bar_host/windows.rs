...
//! 2. `SetWindowSubclass` installs [`teksilo_titlebar_proc`] on the
...
/// same HWND. Starting at a teksilo-flavored magic number keeps
...
impl WindowsHost {
    pub fn new(
        ...
        let installed =
            unsafe { SetWindowSubclass(hwnd, Some(teksilo_titlebar_proc), subclass_id, raw_ptr) };
...
impl Drop for WindowsHost {
    fn drop(&mut self) {
        ...
        unsafe {
            let _ = RemoveWindowSubclass(self.hwnd, Some(teksilo_titlebar_proc), self.subclass_id);
        }
    }
}
...
unsafe extern "system" fn teksilo_titlebar_proc(
    hwnd: HWND,
    msg: u32,
    wparam: WPARAM,
    lparam: LPARAM,
        ...
        WM_NCLBUTTONUP => {
            ...
            if let Some(target) = target {
                let payload = TitleBarSyntheticEvent {
                    teksilo_id: data.callbacks.teksilo_id,
                    target,
                };
