...
//! [`WindowStateService`](fern_settings::WindowStateService) registered
//! via `FernAppBuilder::settings(...)`.
//!
//! Two integration points run inside [`WindowManager::create_window`]:
//!
//! 1. **Restore** — before the winit window is built, the saved
//!    [`PerWindowState`](fern_settings::PerWindowState) (if any) is
...
//!    is clamped down. See
//!    [`PerWindowState::sanitize`](fern_settings::PerWindowState::sanitize).
