//! `install_toast` — wire the Toast system into a [`FernAppBuilder`].
//!
//! Mirrors the [`FernAppBuilderInspectorExt`] pattern (see
//! [`fern_inspector`]) — the extension trait declaration and its
//! impl on `FernAppBuilder` live in the same crate (`fern-ui`),
//! which depends on both `fern-app` and `fern-widgets`, so the
...
use fern_widgets::notification::{NotificationArchive, NotificationArchiveModel};
use fern_widgets::primitives::ZStack;
use fern_widgets::toast::{ToastHost, ToastInstallOptions, ToastRegistry};

/// Extension trait on [`FernAppBuilder`] that wires up the Toast
/// system. The two methods are no-ops without the `toast` feature
/// on `fern-ui`; with the feature on (the default), they install
/// the registry + archive + per-window `ToastHost` wrapper.
pub trait FernAppBuilderToastExt {
    /// Install the Toast subsystem with the given options.
    ///
    /// # Panics
    ///
    /// Panics if `options.archive` is `Some(NotificationArchive::Persistent { … })`
    /// but no [`AppPaths`](fern_settings::AppPaths) was configured
...
impl FernAppBuilderToastExt for FernAppBuilder {
    fn install_toast(self, options: ToastInstallOptions) -> Self {
        ...
        let archive: Option<Rc<NotificationArchiveModel>> = match &options.archive {
            ...
            Some(NotificationArchive::InMemory { .. }) => Some(Rc::new(
                NotificationArchiveModel::open(
                    ...
                    &fern_settings::AppPaths::for_testing(std::path::Path::new("")),
            ...
            Some(NotificationArchive::Persistent { .. }) => {
                ...
                Some(Rc::new(
                    NotificationArchiveModel::open(
                        ...
                        // matches `fern_settings::DEFAULT_DEBOUNCE`;
                        // archives use the same so writes coalesce
                        // with other settings writes.
                        fern_settings::DEFAULT_DEBOUNCE,
                    )
...
mod tests {
    use super::*;
    use fern_widgets::notification::NotificationArchive;
    ...
    fn install_with_in_memory_archive_does_not_require_app_paths() {
        ...
        let _builder = FernAppBuilder::new().install_toast(opts);
    }
    ...
    fn install_persistent_with_app_paths_succeeds() {
        ...
        let _builder = FernAppBuilder::new()
            .app_paths(fern_settings::AppPaths::for_testing(dir.path()))
            .install_toast(opts);
    }
