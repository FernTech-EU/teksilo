...
use teksilo_canvas::quantize_raster_scale;
...
impl WidgetTree {
    ...
    /// flush). Called by `teksilo-app` during its paint pipeline.
...
fn paint_widget_cached(
    ...
    if let Some(r) = blur_radius {
        frame
            .draw_order
            .push(teksilo_canvas::DrawCommand::BeginBlurredSubtree {
                bounds: blur_bounds,
                radius: r,
            });
    }
    ...
    if let Some(o) = opacity {
        frame
            .draw_order
            .push(teksilo_canvas::DrawCommand::SetOpacity(o));
    }
    ...
    let push_transform = transform.filter(|t| *t != teksilo_canvas::Transform2D::IDENTITY);
    ...
    if clip_outside_transform {
        frame
            .draw_order
            .push(teksilo_canvas::DrawCommand::SetClip(bounds));
    }
    if let Some(t) = push_transform {
        frame
            .draw_order
            .push(teksilo_canvas::DrawCommand::PushTransform(t));
    }
    ...
    if clips && !clip_outside_transform {
        frame
            .draw_order
            .push(teksilo_canvas::DrawCommand::ClearClip);
    }

    if push_transform.is_some() {
        frame
            .draw_order
            .push(teksilo_canvas::DrawCommand::PopTransform);
    }
    ...
    if opacity.is_some() {
        frame
            .draw_order
            .push(teksilo_canvas::DrawCommand::RestoreOpacity);
    }

    if blur_radius.is_some() {
        frame
            .draw_order
            .push(teksilo_canvas::DrawCommand::EndBlurredSubtree);
    }
...
fn debug_validate_layout_keys(
    tb: &dyn teksilo_canvas::TextBackend,
    layout_keys: &[u64],
    widget: Option<WidgetId>,
) {
    use teksilo_canvas::GlyphValidation;
    std::thread_local! {
    ...
    for key in layout_keys {
        match tb.debug_validate_layout(*key) {
            ...
            GlyphValidation::StaleKey => {
                ...
                if first_report {
                    eprintln!(
                        "[teksilo] WARNING: retained paint cache replays layout_key={key} \
...
mod tests {
    ...
    impl teksilo_canvas::TextBackend for RecordingTextBackend {
        ...
        fn layout_single_line(
            &mut self,
            text: &str,
            _style: &teksilo_tokens::TextStyle,
        ...
        fn ensure_glyphs(
            &mut self,
            _layout: &teksilo_canvas::TextLayout,
    ...
    impl Widget for TextPaintWidget {
        ...
        fn paint(
            ...
            let style = teksilo_tokens::TextStyle::default();
    ...
    fn scaled_subtree_tree(
        ...
        let mut tree = WidgetTree::new()
            ...
            .with_text_backend(backend_rc.clone()
                as std::rc::Rc<std::cell::RefCell<dyn teksilo_canvas::TextBackend>>);
    ...
    fn walker_sets_raster_scale_for_scaled_subtree_and_restores() {
        let (mut tree, backend_rc, layout_scales) =
            scaled_subtree_tree(teksilo_canvas::Transform2D::scale(2.0, 2.0));
        let _ = tree.render();
    ...
    fn raster_scale_change_repaints_clean_descendants() {
        let transform = crate::signal::Signal::new(teksilo_canvas::Transform2D::scale(2.0, 2.0));
        ...
        transform.set(teksilo_canvas::Transform2D::scale(3.0, 3.0));
        let _ = tree.render();
        assert_eq!(
            layout_scales.borrow().as_slice(),
            &[1.25_f32.powi(5)],
        ...
        layout_scales.borrow_mut().clear();
        transform.set(teksilo_canvas::Transform2D::IDENTITY);
        let _ = tree.render();
    ...
    fn nested_scale_transforms_multiply() {
        ...
        tree.set_transform(outer, teksilo_canvas::Transform2D::scale(2.0, 2.0));
        tree.set_content_transform(inner, teksilo_canvas::Transform2D::scale(1.5, 1.5));
    ...
    fn fill_widget_produces_shape_in_frame() {
        ...
        assert_eq!(
            ...
            teksilo_canvas::ShapeKind::RoundedRect
        );
    }
    ...
    fn subtree_theme_override() {
        ...
        tree.set_theme_override(parent, |theme| {
            theme.colors = teksilo_tokens::ColorTokens::dark_default();
        });

        tree.layout(SizeProposal::exact(100.0, 50.0));
        let frame = tree.render();

        let dark_accent = teksilo_tokens::ColorTokens::dark_default()
            .accent
            .to_array();
    ...
    fn theme_override_only_affects_subtree() {
        ...
        let light_accent = teksilo_tokens::ColorTokens::light_default()
            .accent
            .to_array();
        let dark_accent = teksilo_tokens::ColorTokens::dark_default()
            .accent
    ...
    fn opacity_prop_emits_set_and_restore_around_subtree() {
        ...
        for cmd in &frame.draw_order {
            match cmd {
                teksilo_canvas::DrawCommand::SetOpacity(v) => {
                    assert!((v - 0.5).abs() < 1e-6, "expected 0.5, got {}", v);
                    set_count += 1;
                }
                teksilo_canvas::DrawCommand::RestoreOpacity => restore_count += 1,
                _ => {}
            }
        }
    ...
    fn opacity_prop_zero_skips_subtree_entirely() {
        ...
        for cmd in &frame.draw_order {
            assert!(
                !matches!(cmd, teksilo_canvas::DrawCommand::SetOpacity(_)),
    ...
    fn transform_prop_emits_push_and_pop_around_subtree() {
        ...
        let parent = tree.add(StackWidget::new());
        let _child = tree.add_child(parent, FillWidget::new().background(Color::RED));
        let scale_2x = teksilo_canvas::Transform2D::scale(2.0, 2.0);
        ...
        for cmd in &frame.draw_order {
            match cmd {
                ...
                teksilo_canvas::DrawCommand::PopTransform => pop_count += 1,
                _ => {}
            }
        }
    ...
    fn content_transform_clip_wraps_outside_the_transform() {
        ...
        tree.set_content_transform(parent, teksilo_canvas::Transform2D::translate(50.0, 30.0));
        let frame = tree.render();
        ...
        for (i, cmd) in frame.draw_order.iter().enumerate() {
            match cmd {
                teksilo_canvas::DrawCommand::SetClip(_) => set_clip = set_clip.or(Some(i)),
                teksilo_canvas::DrawCommand::PushTransform(_) => push = push.or(Some(i)),
                teksilo_canvas::DrawCommand::PopTransform => pop = Some(i),
                teksilo_canvas::DrawCommand::ClearClip => clear = Some(i),
                _ => {}
            }
        }
        let (sc, pt, pop, cc) = (
    ...
    fn identity_transform_prop_skipped() {
        ...
        let parent = tree.add(StackWidget::new());
        tree.add_child(parent, FillWidget::new().background(Color::RED));
        tree.set_transform(parent, teksilo_canvas::Transform2D::IDENTITY);
        tree.layout(SizeProposal::exact(100.0, 50.0));
        let frame = tree.render();
        ...
        for cmd in &frame.draw_order {
            assert!(
                !matches!(
                    cmd,
                    teksilo_canvas::DrawCommand::PushTransform(_)
                        | teksilo_canvas::DrawCommand::PopTransform
                ),
    ...
    fn transform_scope_paint_order_opacity_outer_transform_inner() {
        ...
        tree.set_transform(parent, teksilo_canvas::Transform2D::scale(2.0, 2.0));
        tree.layout(SizeProposal::exact(100.0, 50.0));
        let frame = tree.render();
        ...
        for (i, cmd) in frame.draw_order.iter().enumerate() {
            match cmd {
                teksilo_canvas::DrawCommand::SetOpacity(_) => set_opacity_idx = Some(i),
                teksilo_canvas::DrawCommand::PushTransform(_) => push_transform_idx = Some(i),
                teksilo_canvas::DrawCommand::PopTransform => pop_transform_idx = Some(i),
                teksilo_canvas::DrawCommand::RestoreOpacity => restore_opacity_idx = Some(i),
                _ => {}
            }
        }
    ...
    fn post_paint_emits_after_children() {
        ...
        let color_of = |cmd: &teksilo_canvas::DrawCommand| -> Option<[f32; 4]> {
            match cmd {
                teksilo_canvas::DrawCommand::Shape(i) => frame.shapes.get(*i).map(|s| s.color),
                teksilo_canvas::DrawCommand::Decoration(i) => {
    ...
    fn blur_prop_emits_begin_end_pair_around_subtree() {
        ...
        for cmd in &frame.draw_order {
            match cmd {
                teksilo_canvas::DrawCommand::BeginBlurredSubtree { radius, .. } => {
                    begin_count += 1;
                    begin_radius = Some(*radius);
                }
                teksilo_canvas::DrawCommand::EndBlurredSubtree => end_count += 1,
                _ => {}
            }
        }
    ...
    fn blur_prop_subperceptual_radius_skipped() {
        ...
        for cmd in &frame.draw_order {
            assert!(
                !matches!(
                    cmd,
                    teksilo_canvas::DrawCommand::BeginBlurredSubtree { .. }
                        | teksilo_canvas::DrawCommand::EndBlurredSubtree
                ),
    ...
    fn blur_scope_is_outermost_when_combined_with_opacity_and_transform() {
        ...
        for (i, cmd) in frame.draw_order.iter().enumerate() {
            match cmd {
                teksilo_canvas::DrawCommand::BeginBlurredSubtree { .. } => begin_idx = Some(i),
                teksilo_canvas::DrawCommand::SetOpacity(_) => set_opacity_idx = Some(i),
                teksilo_canvas::DrawCommand::PushTransform(_) => push_transform_idx = Some(i),
                teksilo_canvas::DrawCommand::PopTransform => pop_transform_idx = Some(i),
                teksilo_canvas::DrawCommand::RestoreOpacity => restore_opacity_idx = Some(i),
                teksilo_canvas::DrawCommand::EndBlurredSubtree => end_idx = Some(i),
                _ => {}
            }
        }
