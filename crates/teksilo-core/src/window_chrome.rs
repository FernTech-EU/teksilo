...
//! widget (in `teksilo-widgets`) and the per-OS implementations that own the
//! window handle (in `teksilo-platform`). The trait is intentionally `!Send +
...
/// `WM_NCHITTEST`) so widget land never saw it. The teksilo-app
...
///   and `teksilo-app` routes it to `WindowManager::queue_close`.
...
///   teksilo-core stays winit-free.
/// - `teksilo_id`: the host's window id, copied into the synthetic
...
pub struct TitleBarHostCallbacks {
    ...
    /// teksilo-side window id. The Windows backend stamps this into
