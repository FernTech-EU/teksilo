...
use teksilo_canvas::{GradientStop, Paint, Point, Size};
...
impl PaintProp {
    ...
    pub fn from_fill(fill: &FillRecipe, colors: &teksilo_tokens::ColorTokens) -> Self {
        match fill {
            FillRecipe::Solid(c) => PaintProp::Solid((*c).into()),
            FillRecipe::None => {
                PaintProp::Solid(ColorProp::Static(teksilo_tokens::Color::TRANSPARENT))
            }
            FillRecipe::StateLayer { .. } => {
                // Pre-composite against the supplied tokens (frozen).
                let flat = fill
                    .resolve_flat(colors)
                    .unwrap_or(teksilo_tokens::Color::TRANSPARENT);
