//! FernUI's own widget previewer binary.
//!
//! Run:
//!
//! ```text
...
//! everything happens inside `fern_preview_ui::run_previewer`.
...
fn main() {
    ...
    if argv.iter().any(|a| a == "--list") {
        for entry in fern_preview::iter_entries() {
            println!(
                "{}\t{}\t{}\t{}",
                entry.group(),
                entry.id(),
    ...
    let opts = fern_preview_ui::PreviewerOptions::from_args();
    fern_preview_ui::run_previewer(opts);
}
...
#[cfg(test)]
mod tests {
    use fern_preview::{find_by_id, iter_entries};
    ...
    fn cli_widget_accepts_every_registered_id() {
        for entry in iter_entries() {
            ...
            let opts = fern_preview_ui::PreviewerOptions::from_iter([arg.as_str()]);
    ...
    fn knob_overrides_apply_to_runtime_values() {
        ...
        if let fern_preview::PreviewVariant::Knobs { overrides, .. } = disabled {
            let knobs = fern_preview::KnobValues::from_spec(&entry.knobs(), Some(&overrides));
            assert!(
