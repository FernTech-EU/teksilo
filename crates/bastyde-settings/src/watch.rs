    ...
    pub fn new(dirs: Vec<PathBuf>, sink: SettingsReloadSink) -> Result<Self, notify::Error> {
        ...
        for dir in dirs {
            let canonical = match dir.canonicalize() {
                ...
                Err(e) => {
                    eprintln!(
                        "bastyde-settings: cannot watch `{}` ({e}); live settings reload \
        ...
        let mut watcher = notify::recommended_watcher(
            move |res: Result<notify::Event, notify::Error>| match res {
                ...
                Ok(_) => {}
                Err(e) => {
                    eprintln!("bastyde-settings: watcher error: {e}");
                }
            },
        )?;
...
/// `bastyde-i18n::file_watcher::should_reload` exactly.
...
mod tests {
    ...
    /// `bastyde-app`'s wiring). A `SettingsWatcher`'s sink, in contrast,
