...
//! `teksilo-analytics-otlp` — OpenTelemetry-compatible analytics
//! adapter for Teksilo.
//!
...
//! event.schema_version     attributes["teksilo.schema_version"]
//! ```
//!
...
//!
//! ```ignore
//! use teksilo_analytics_otlp::OtlpAdapter;
//! use std::rc::Rc;
//!
//! let adapter = Rc::new(
//!     OtlpAdapter::builder()
...
//!         .build(),
//! ) as Rc<dyn teksilo_telemetry::UsageReporter>;
//! ```
//!
//! # Fetch + erase
//!
...
//! # Queue durability
//!
//! Unlike `teksilo-analytics-plausible` and `teksilo-analytics-native`,
...
//! Plausible/Teksilo shape is straightforward — the worker already
...
impl OtlpAdapterBuilder {
    ...
    /// matches the same shape as the Plausible/Teksilo adapters.
