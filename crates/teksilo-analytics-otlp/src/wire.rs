    ...
    pub(crate) fn build_body(&self, events: &[OwnedEvent]) -> serde_json::Value {
        let mut resource_attrs = vec![
            ...
            KeyValue {
                ...
                value: AnyValue::String(Cow::Borrowed("teksilo-analytics-otlp")),
            },
            KeyValue {
...
fn record_for(ev: &OwnedEvent) -> LogRecord<'_> {
    ...
    // Attributes: every prop becomes `teksilo.<key>`, plus session_id
    // and the event category.
    let mut attributes = vec![
        KeyValue {
            key: "teksilo.category".into(),
            value: AnyValue::String(Cow::Borrowed(category_str(ev.category))),
        },
        KeyValue {
            key: "teksilo.session_id".into(),
            value: AnyValue::String(Cow::Borrowed(ev.session_id.as_str())),
        },
        KeyValue {
            key: "teksilo.schema_version".into(),
...
fn category_str(c: teksilo_core::telemetry::EventCategory) -> &'static str {
...
mod tests {
    ...
    #[test]
    fn props_become_teksilo_namespaced_attributes() {
        let b = WireBuilder {
            service_name: "test.app".into(),
            service_version: "1.0".into(),
        };
        ...
        let prop_name = attrs
            .iter()
            .find(|a| a["key"] == "teksilo.name")
            .expect("teksilo.name attribute");
        assert_eq!(prop_name["value"]["stringValue"], "app.save");
        let prop_n = attrs
            .iter()
            .find(|a| a["key"] == "teksilo.n")
            .expect("teksilo.n attribute");
