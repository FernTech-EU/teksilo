...
//! Property tests for `Color` (crates/teksilo-tokens/src/color.rs) and the
//! `ColorTokens::for_inactive_window` accent-desaturation transform
//! (crates/teksilo-tokens/src/theme.rs).
//!
...
//! Manual override knob: `PROPTEST_CASES=N cargo test -p teksilo-tokens
//! --test prop_color`.
//!
