...
//! widget (in `bastyde-widgets`) and the per-OS implementations that own the
//! window handle (in `bastyde-platform`). The trait is intentionally `!Send +
...
/// bastyde-app dispatcher then looks up the matching widget id via
...
/// `WM_NCHITTEST`) so widget land never saw it. The bastyde-app
...
///   and `bastyde-app` routes it to `WindowManager::queue_close`.
...
///   bastyde-core stays winit-free.
/// - `bastyde_id`: the host's window id, copied into the synthetic
...
pub struct TitleBarHostCallbacks {
    ...
    /// bastyde-side window id. The Windows backend stamps this into
...
impl TitleBarHostCallbacks {
    ...
    pub fn noop() -> Self {
        Self {
            ...
            bastyde_id: BastydeWindowId::new(0),
        }
    }
}
