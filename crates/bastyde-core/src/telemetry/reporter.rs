...
/// (`bastyde-analytics-plausible`, `bastyde-analytics-posthog`, etc.) and
/// by `bastyde-telemetry::DynamicReporter` (which forwards to whichever
...
///
/// **Single-threaded.** Bastyde is single-threaded by design (the
...
/// Top-level consent state, persisted via `bastyde-telemetry::ConsentStore`.
