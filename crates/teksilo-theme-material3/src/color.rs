...
//! Material 3 baseline color tokens mapped onto Teksilo's [`ColorTokens`].
//!
//! Values are the static Material 3 reference scheme derived from the
//! canonical seed `#6750A4` (the "baseline" purple). Teksilo's
...
//! "state layer" over the base). Teksilo recipes take opaque colors, so
