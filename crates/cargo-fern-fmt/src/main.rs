//! `cargo fern-fmt` — formatter for `fern!` DSL blocks in Rust source.
//!
//! Run as a cargo subcommand: `cargo fern-fmt [paths...]`
//! Install via: `cargo install --path crates/cargo-fern-fmt`
//!
//! # Modes
//!
//! - **Format in place** (default): rewrite each `.rs` file whose
//!   `fern!` blocks aren't canonical. Writes are atomic (write-temp +
...
//!
//! Source outside `fern!(...)` blocks is left byte-for-byte unchanged
//! — `cargo fmt` owns Rust formatting.
//!
//! # Usage
//!
//! ```text
//! cargo fern-fmt [paths...]
//!
//! Options:
...
fn main() {
    ...
    let cfg = match Config::parse(args) {
        ...
        Err(e) => {
            eprintln!("error: {e}");
            eprintln!("Run `cargo fern-fmt --help` for usage.");
            process::exit(2);
        }
    };
    ...
    if cfg.version {
        println!("cargo-fern-fmt {}", env!("CARGO_PKG_VERSION"));
        return;
    }
...
fn print_help() {
    println!(
        r#"cargo fern-fmt — formatter for fern! DSL blocks
...
EXAMPLES:
    ...
    cargo fern-fmt --check                    # CI mode
    cargo fern-fmt examples/widget_catalog    # format one example
    cargo fern-fmt src/main.rs                # format one file
"#
    );
}
