...
//! `EventContext`.
//!
//! `fern-settings` cannot live below `fern-core` (it depends on
//! `fern-core` for `Signal`, `ObserverHandle`, etc.), so the
...
//! `WindowStateService` is registered via `FernAppBuilder::settings`,
//! every `WindowConfig` carrying an `id(...)` is automatically
//! restored on creation and recorded on every change by `fern-app`'s
...
pub trait SettingsExt {
    /// The K/V settings store. Panics if `FernAppBuilder::settings(...)`
    /// was not called.
    fn settings(&self) -> &SettingsStore {
        self.try_settings().unwrap_or_else(|| {
            panic!(
                "SettingsExt::settings(): no SettingsStore registered. \
                 Call FernAppBuilder::settings(SettingsBundle::new()) at startup."
            )
        })
    }
    ...
    /// `FernAppBuilder::app_state(mru_handle.clone())`.
