...
//! key into a [`bastyde_data::ChartSelection`] — no separate lookup
//! structure is needed.

use bastyde_canvas::{Canvas, Point, Rect, StrokeStyle};
use bastyde_core::accessibility::{AccessNodeBuilder, SyntheticKind};
use bastyde_core::accesskit;
use bastyde_core::styles::{BorderRecipe, BorderStyle, Theme};
use bastyde_data::SeriesId;
...
mod tests {
    ...
    fn fake_series_id() -> SeriesId {
        let model: bastyde_data::ChartModel<i32> = bastyde_data::ChartModel::new();
        model.add_series("s")
    }
    ...
    fn gridline_stroke_dash_override_wins_over_recipe_style() {
        let recipe = BorderRecipe {
            width: 1.0,
            color: bastyde_core::styles::RecipeColor::Static(bastyde_tokens::Color::BLACK),
            style: BorderStyle::Solid,
            position: bastyde_core::styles::BorderPosition::Center,
            sides: None,
        };
