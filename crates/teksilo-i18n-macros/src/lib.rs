...
//! Compile-time-validating proc macros for Teksilo's i18n runtime.
//!
...
/// runtime. Only used inside `teksilo-widgets`.
#[proc_macro]
...
impl SourceKind {
    ...
    ///
    /// External apps route through `::teksilo::i18n` so they only
    /// need `teksilo` in deps (the serde pattern). Internal teksilo
    /// crates (`teksilo-widgets`, `teksilo-i18n` tests) route through
    /// `::teksilo_i18n` because they can't depend on `teksilo` (circular).
    fn i18n_root(self) -> TokenStream2 {
        match self {
            SourceKind::Widget => quote!(::teksilo_i18n),
            SourceKind::App => {
                // Internal workspace crates can't depend on `teksilo` (cycle),
                // so they route through `::teksilo_i18n`; external apps route
                // through `::teksilo::i18n` (the serde pattern — they only need
                // `teksilo` in deps). A proc-macro can only see the *compiled*
                // crate's name, and every internal crate starts with
                // `teksilo-`, so that prefix is the default signal.
                //
                // Escape hatch for the one collision — a downstream crate
                // legitimately named `teksilo-*` that depends on `teksilo`, not
                // `teksilo-i18n`: set `TEKSILO_I18N_ROUTE=external` (or
                // `=internal` to force the other way). Honoured first.
                match std::env::var("TEKSILO_I18N_ROUTE").as_deref() {
                    Ok("external") => return quote!(::teksilo::i18n),
                    Ok("internal") => return quote!(::teksilo_i18n),
                    _ => {}
                }
                let pkg = std::env::var("CARGO_PKG_NAME").unwrap_or_default();
                if pkg.starts_with("teksilo-") {
                    quote!(::teksilo_i18n)
                } else {
                    quote!(::teksilo::i18n)
                }
...
/// `TEKSILO_I18N_SOURCE_DIR` environment variable. Applications that
...
/// compiled. Precedence (highest first): `TEKSILO_I18N_SOURCE_DIR`,
/// `TEKSILO_I18N_SOURCE_PATH`, auto-detect `locales/en-US/` directory,
...
fn resolve_source() -> std::result::Result<SourceInfo, String> {
    ...
    if let Ok(dir_override) = std::env::var("TEKSILO_I18N_SOURCE_DIR") {
    ...
    if let Ok(file_override) = std::env::var("TEKSILO_I18N_SOURCE_PATH") {
    ...
    for seg in segments {
        ...
        if s.contains("__") {
            return Err(syn::Error::new(
                seg.span(),
                format!(
                    "teksilo-i18n: path segment `{s}` contains `__`, \
        ...
        if !first_ok || !rest_ok {
            return Err(syn::Error::new(
                seg.span(),
                format!(
                    "teksilo-i18n: path segment `{s}` is not a valid Fluent \
...
fn tr_impl(input: TokenStream, kind: SourceKind, signal: bool) -> TokenStream {
    ...
    let key_map = match load_key_map_for_source(&source_info) {
        ...
        Err(msg) => {
            return syn::Error::new(
                call.path_span(),
                format!(
                    "teksilo-i18n: failed to load source `{}`: {msg}",
                    source_info.display()
                ),
            )
            .to_compile_error()
    ...
    let (expected_args, fallback_parts): (Vec<String>, Option<Vec<FallbackPart>>) =
        match key_map.messages.get(&fluent_key) {
            ...
            None => {
                let mut msg = format!(
                    "teksilo-i18n: translation key `{}` not found in `{}`",
                    fluent_key,
                    source_info.display()
                );
    ...
    for expected in &expected_args {
        if !provided_names.iter().any(|p| p == expected) {
            return syn::Error::new(
                call.path_span(),
                format!("teksilo-i18n: missing argument `{expected}` for key `{fluent_key}`"),
            )
            .to_compile_error()
            .into();
        }
    ...
    for arg in &call.args {
        ...
        if !expected_args.iter().any(|e| e == &name) {
            return syn::Error::new(
                arg.name.span(),
                format!(
                    "teksilo-i18n: unknown argument `{name}` for key `{fluent_key}` (expected: {})",
        ...
        Some(parts) => {
                ...
                match part {
                    FallbackPart::Text(text) => {
                        ...
                        fallback_stmts.push(quote! {
                            __teksilo_fallback.push_str(#lit);
                        });
                    }
            ...
            quote! {
                ...
                if __teksilo_result == #key_lit {
                    let mut __teksilo_fallback = ::std::string::String::new();
                    #(#fallback_stmts)*
                    __teksilo_fallback
                } else {
                    __teksilo_result
    ...
    let expanded = if signal {
        ...
        let arg_subscribes: Vec<TokenStream2> = arg_idents
            ...
            .map(|ident| {
                quote! {
                    {
                        let __teksilo_resolver_arg = ::std::rc::Rc::clone(&__teksilo_resolver);
                        let __teksilo_weak_arg = __teksilo_weak.clone();
                        let __teksilo_h = #ident.observe(move |_| {
                            if let Some(__t) = __teksilo_weak_arg.upgrade() {
                                __t.set((__teksilo_resolver_arg)());
                            }
                        });
                        __teksilo_out.attach_keepalive(__teksilo_h);
                    }
                }
            })
            .collect();
        ...
        quote! {
            {
                ...
                let __teksilo_out = #i18n_root::Signal::new((__teksilo_resolver)());
                let __teksilo_weak = __teksilo_out
                    .downgrade()
                    .expect("Signal::new returns mutable; downgrade cannot fail");

                if let Some(__teksilo_ver) = #i18n_root::current_version_signal() {
                    let __teksilo_resolver_ver = ::std::rc::Rc::clone(&__teksilo_resolver);
                    let __teksilo_weak_ver = __teksilo_weak.clone();
                    let __teksilo_handle_ver = __teksilo_ver.observe(move |_| {
                        if let Some(__t) = __teksilo_weak_ver.upgrade() {
                            __t.set((__teksilo_resolver_ver)());
                        }
                    });
                    __teksilo_out.attach_keepalive(__teksilo_handle_ver);
                }
