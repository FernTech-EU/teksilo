...
impl<T: Clone + std::fmt::Display + 'static> Widget for BarChart<T> {
    ...
    fn accessibility(&self, builder: &mut AccessNodeBuilder) {
        builder.set_role(fern_core::accesskit::Role::GraphicsDocument);
        let series_vec = self.series.get();
...
mod tests {
    ...
    fn accessibility_role_is_graphics_document() {
        ...
        assert_eq!(info.role(), fern_core::accesskit::Role::GraphicsDocument);
    }
    ...
    fn value_labels_emit_glyphs() {
        ...
        use fern_canvas::text_backend::MockTextBackend;
        use std::cell::RefCell;
        use std::rc::Rc;

        let backend: Rc<RefCell<dyn fern_canvas::TextBackend>> =
