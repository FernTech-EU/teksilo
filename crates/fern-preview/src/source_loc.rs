...
mod tests {
    ...
    fn matches_path_handles_suffix_matches() {
        let loc = SourceLoc::new("crates/fern-widgets/src/button.rs", 42);
        assert!(loc.matches_path("crates/fern-widgets/src/button.rs"));
        assert!(loc.matches_path("button.rs"));
        assert!(loc.matches_path("src/button.rs"));
        assert!(!loc.matches_path("crates/fern-widgets/src/checkbox.rs"));
    }

    #[test]
    fn matches_path_normalises_separators() {
        let loc = SourceLoc::new("crates\\fern-widgets\\src\\button.rs", 1);
        assert!(loc.matches_path("button.rs"));
        assert!(loc.matches_path("src/button.rs"));
    }
}
