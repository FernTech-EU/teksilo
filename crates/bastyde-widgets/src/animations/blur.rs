...
mod tests {
    ...
    fn collect_blur_radii(frame: &bastyde_canvas::RenderFrame) -> Vec<f32> {
        frame
            .draw_order
            .iter()
            .filter_map(|c| match c {
                bastyde_canvas::DrawCommand::BeginBlurredSubtree { radius, .. } => Some(*radius),
                _ => None,
            })
            .collect()
    }
    ...
    fn static_radius_emits_begin_end_pair() {
        ...
        let ends = frame
            .draw_order
            .iter()
            .filter(|c| matches!(c, bastyde_canvas::DrawCommand::EndBlurredSubtree))
            .count();
        assert_eq!(ends, 1);
    }
