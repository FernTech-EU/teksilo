...
impl Widget for WizardHeader {
    ...
    fn accessibility(&self, builder: &mut AccessNodeBuilder) {
        ...
        builder.set_name(bastyde_i18n::tr_widget!(a11y_wizard_progress_name()).resolve_now());
...
impl Widget for WizardFlow {
    ...
    fn accessibility(&self, builder: &mut AccessNodeBuilder) {
        ...
        builder.set_name(bastyde_i18n::tr_widget!(a11y_wizard_content_name()).resolve_now());
    }
    ...
    fn build(&mut self, ctx: &mut BuildContext) -> Vec<WidgetId> {
        ...
        let root_id = if let Some(trigger) = self.pending_trigger.take() {
            ctx.add(
                OverlayTrigger::new(
                    trigger,
                    bastyde_core::widget_builder::HandlerSet::new()
                        .focusable(true)
