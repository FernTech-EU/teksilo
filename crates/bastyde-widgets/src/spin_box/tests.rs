...
fn setup_int(
    ...
    min: i32,
    max: i32,
) -> (WidgetTree, Signal<i32>, bastyde_core::widget_id::WidgetId) {
...
fn focus_field(tree: &mut WidgetTree, spin_id: bastyde_core::widget_id::WidgetId) {
    let field = tree
...
fn a11y_role_is_spin_button() {
    ...
    assert!(actions.contains(&bastyde_core::accesskit::Action::Increment));
    assert!(actions.contains(&bastyde_core::accesskit::Action::Decrement));
    assert!(actions.contains(&bastyde_core::accesskit::Action::Focus));
}
...
fn show_buttons_sugar_matches_button_layout() {
    ...
    fn count_focusable(tree: &WidgetTree, root: bastyde_core::widget_id::WidgetId) -> usize {
...
fn a11y_numeric_value_matches_signal() {
    ...
    for required in [
        ...
        bastyde_core::accesskit::Action::SetValue,
        bastyde_core::accesskit::Action::Focus,
    ] {
        assert!(
