...
impl Widget for TabHeader {
    fn build(&mut self, ctx: &mut BuildContext) -> Vec<WidgetId> {
        ...
        let orientation_for_cfg = match self.orientation {
            ...
            super::delegate::TabBarOrientation::Vertical => {
                bastyde_core::styles::TabBarOrientation::Vertical
            }
        };
    ...
    fn accessibility(&self, builder: &mut AccessNodeBuilder) {
        ...
        if self.on_reorder_to.is_some() && self.initial_enabled && !self.pinned {
            ...
            if self.index > 0 {
                actions.push(bastyde_core::accesskit::CustomAction {
                    id: 0,
            ...
            if !actions.is_empty() {
                builder.add_action(bastyde_core::accesskit::Action::CustomAction);
