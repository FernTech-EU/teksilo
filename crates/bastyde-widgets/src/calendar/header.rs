...
use bastyde_core::styles::{CalendarHeaderConfig, SharedCalendarStyle};
...
impl Widget for CalendarHeader {
    fn build(&mut self, ctx: &mut BuildContext) -> Vec<WidgetId> {
        ...
        let step_single = {
            ...
            std::rc::Rc::new(
                move |dir: i32, ctx_evt: &mut bastyde_core::widget::EventContext| {
                    let cur = visible.get();
...
struct NavArrow {
    kind: ArrowKind,
    label: String,
    on_activate: std::rc::Rc<dyn Fn(&mut bastyde_core::widget::EventContext)>,
    root_id: Option<WidgetId>,
}
...
impl NavArrow {
    ...
    where
        F: Fn(&mut bastyde_core::widget::EventContext) + 'static,
    {
        Self {
            kind,
            label,
