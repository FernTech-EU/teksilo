...
//! test ([crates/bastyde-analytics-plausible/tests/integration.rs]).
//!
//! These tests cover sub-phase A acceptance: "a Bastyde app emits
...
use bastyde_collector_proto::v1::telemetry_server::{Telemetry, TelemetryServer};
use bastyde_core::telemetry::{Event, EventCategory, Prop, PropValue, UsageReporter};
...
fn per_product_scope_enforced_at_batch_level() {
    ...
    let adapter_a = BastydeAdapter::builder()
        .endpoint(server.endpoint())
    ...
    let adapter_b = BastydeAdapter::builder()
        .endpoint(server.endpoint())
...
fn anonymous_adapter_returns_unsupported_for_fetch_and_erase() {
    ...
    assert!(matches!(
        ...
        Err(bastyde_core::telemetry::TelemetryError::FetchUnsupported)
    ));
    assert!(matches!(
        adapter.erase_remote_data(),
        Err(bastyde_core::telemetry::TelemetryError::ErasureUnsupported)
    ));
...
fn pseudonymous_adapter_round_trips_fetch_and_erase() {
    ...
    assert_eq!(export.adapter, "bastyde-collector");
