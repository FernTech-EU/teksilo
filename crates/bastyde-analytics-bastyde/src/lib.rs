...
//! Bastyde analytics adapter for the home-grown
//! [`bastyde-collector`](../../../bastyde-collector) gRPC backend.
//!
//! # Sub-phase A
//!
//! Localhost ingest only — no auth, no TLS. Wire format defined in
//! `bastyde-collector/proto/telemetry/v1.proto` and consumed via the
//! [`bastyde_collector_proto`] crate.
//!
//! # Architecture
//!
//! Same layered shape as [`bastyde-analytics-plausible`]: a sync
//! [`UsageReporter`] surface (called from the Bastyde UI thread)
...
//! [`BastydeAdapterBuilder::runtime`] to avoid double allocation.

mod config;
mod worker;

pub use config::{BastydeConfig, TlsClientConfig};
...
/// Adapter for the Bastyde-operated `bastyde-collector` gRPC service.
///
/// Construct with [`BastydeAdapter::builder`]. `Drop` joins the worker
/// task with a best-effort final flush.
pub struct BastydeAdapter {
    config: BastydeConfig,
...
impl BastydeAdapter {
    pub fn builder() -> BastydeAdapterBuilder {
        BastydeAdapterBuilder::default()
    }
    ...
    fn send(&self, cmd: WorkerCommand) {
        ...
        // (the Bastyde UI thread). Returns Err only if the worker
...
impl std::fmt::Debug for BastydeAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("BastydeAdapter")
...
impl UsageReporter for BastydeAdapter {
...
#[derive(Default)]
pub struct BastydeAdapterBuilder {
    config: BastydeConfig,
...
impl std::fmt::Debug for BastydeAdapterBuilder {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("BastydeAdapterBuilder")
...
impl BastydeAdapterBuilder {
    ...
    /// `bastyde_telemetry::scopes::TELEMETRY_ENDPOINT_OVERRIDE` settings
    ...
    /// `bastyde-collector token mint`. Server rejects unauthenticated
    ...
    pub fn build(self) -> BastydeAdapter {
        assert!(
            !self.config.endpoint.is_empty(),
            "BastydeAdapter::builder().endpoint(...) is required",
        );
        assert!(
            !self.config.product_id.is_empty(),
            "BastydeAdapter::builder().product_id(...) is required",
        );
        ...
        let queue: Arc<dyn EventQueue> = match (self.explicit_queue, self.queue_path) {
            ...
            (None, Some(path)) => Arc::new(
                ...
                .expect("BastydeAdapterBuilder::persistent_queue_path: open redb"),
            ),
        ...
        let runtime = self.runtime.unwrap_or_else(|| {
            Arc::new(
                tokio::runtime::Builder::new_multi_thread()
                    .worker_threads(1)
                    .thread_name("bastyde-collector-worker")
                    .enable_all()
                    .build()
                    .expect("build tokio runtime for BastydeAdapter"),
            )
        });
