...
use bastyde_collector_proto::v1 as proto;
use bastyde_core::telemetry::{OwnedEvent, OwnedProp, OwnedPropValue};
use bastyde_telemetry::EventQueue;
...
use crate::config::BastydeConfig;
...
pub(crate) fn spawn_worker(
    ...
    // shutdown path that joins from the BastydeAdapter::Drop site.
    let handle = thread::Builder::new()
        .name("bastyde-collector-client".to_string())
        .spawn(move || {
            rt_for_thread.block_on(worker_loop(config, queue, rx, stats));
        })
        .expect("spawn bastyde-collector worker thread");
    ...
    match send_batch(config, channel, batch_id, batch).await {
        ...
        Err((events, reason)) => {
            ...
            *channel = None;
            ...
            stats.queued.store(queue.len(), Ordering::Relaxed);
            tracing_log(format!(
                "bastyde-collector send failed (batch={batch_id}, count={batch_count}): {reason}"
            ));
            Err(reason)
        }
    }
...
fn owned_to_proto_event(ev: &OwnedEvent) -> proto::Event {
    ...
    let category = match ev.category {
        bastyde_core::telemetry::EventCategory::Intent => proto::EventCategory::Intent,
        bastyde_core::telemetry::EventCategory::Lifecycle => proto::EventCategory::Lifecycle,
        bastyde_core::telemetry::EventCategory::Navigation => proto::EventCategory::Navigation,
        bastyde_core::telemetry::EventCategory::Census => proto::EventCategory::Census,
        bastyde_core::telemetry::EventCategory::Custom => proto::EventCategory::Custom,
    } as i32;
    proto::Event {
        name: ev.name.clone(),
        category,
...
use bastyde_core::telemetry::{RemoteEvent, RemoteValue};
