...
use bastyde_automation::dto::{AutomationOp, AutomationReply, AutomationRequest, SettleSpec};
...
fn tool_router_matches_catalog() {
    ...
    for entry in bastyde_automation::TOOL_CATALOG {
        assert!(
...
async fn screenshot_decodes_to_png_or_reports_no_gpu() {
    ...
    match hr {
        ...
        HostReply::Reply(AutomationReply::Err { code, .. }) => {
            ...
            assert_eq!(code, bastyde_automation::dto::codes::GPU_UNAVAILABLE);
        }
...
async fn golden_full_window() {
    ...
    let png = match hr {
        ...
        HostReply::Reply(AutomationReply::Err { code, .. })
            if code == bastyde_automation::dto::codes::GPU_UNAVAILABLE =>
        {
