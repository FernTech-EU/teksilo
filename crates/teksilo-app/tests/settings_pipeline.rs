...
//! Integration test for the `TeksiloAppBuilder::settings(...)` pipeline.
//!
//! Verifies that:
...
fn settings_off_by_default() {
    let app = TeksiloAppBuilder::new().build_headless();
...
fn application_panics_only_when_paths_unresolvable() {
    ...
    let _ = std::panic::catch_unwind(|| {
        TeksiloAppBuilder::new()
            .application("test", "teksilo-ci", "settings-pipeline-smoke")
            .build_headless()
    });
}
