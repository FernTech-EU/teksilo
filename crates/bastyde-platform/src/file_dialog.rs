...
//!   [`FileDialogEventPayload`] via [`bastyde_core::AppEventPoster::post_external`];
//!   `bastyde-app` picks the payload up in its `AppEvent::External` arm,
...
//! [`AppEvent::External`](bastyde_core::AppEvent::External) and the
...
//! `BastydeWindowId`. When that window closes, [`FileDialogHandle::purge_window`]
//! (called by `bastyde-app`'s window-close hook) drops the callback box
...
/// dialog. `bastyde-app`'s app-event handler downcasts to this type and
...
pub trait FileDialogBackend {
    ...
    /// [`AppEventPoster::post_external`](bastyde_core::AppEventPoster::post_external)
    ...
    fn dispatch(
        ...
        poster: Arc<dyn bastyde_core::AppEventPoster>,
    );
}
...
/// [`BastydeAppBuilder::install_file_dialog`](https://docs.rs/bastyde-app);
...
impl FileDialogHandle {
    ...
    /// Called by `bastyde-app` from the `AppEvent::External` arm. If
    ...
    /// `window_id`. Called by `bastyde-app`'s window-close path so
...
///
/// Apps `use bastyde_platform::file_dialog::EventContextFileDialogExt`
/// (or `use bastyde::prelude::*` once the umbrella re-exports it).
///
...
///      [`bastyde_core::AppEventPoster`] via app-state.
...
/// did not call `BastydeAppBuilder::install_file_dialog`).
///
    ...
    let handle = ctx
        ...
        .ok_or_else(|| {
            "FileDialogHandle not installed in app-state — call \
             BastydeAppBuilder::install_file_dialog (or app_state(...)) at startup"
                .to_string()
        })?
        .clone();
    let poster = ctx
...
mod tests {
    ...
    impl AppEventPoster for CapturingPoster {
        fn post_subscription_event(
            &self,
            _sub_id: bastyde_core::SubscriptionId,
    ...
    fn bastyde_id(n: u64) -> BastydeWindowId {
    ...
    fn purge_drops_callbacks_for_matching_window() {
        ...
        handle.purge_window(bastyde_id(7));
        assert_eq!(handle.pending_count(), 1);
        handle.purge_window(bastyde_id(8));
    ...
    fn payload_round_trips_through_capturing_poster() {
        ...
        assert_eq!(payload.window_id_owner, bastyde_id(42));
    ...
    fn deliver_after_purge_is_silent() {
        ...
        handle.purge_window(bastyde_id(5));
