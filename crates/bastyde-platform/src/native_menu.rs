...
//!
//! Mirrors a logical menu tree (the `bastyde-widgets` `MenuModel`) into the
...
//!   snapshot down, so `bastyde-platform` never depends on `bastyde-widgets`.
...
//! [`NativeMenuEventPayload`] through [`bastyde_core::AppEventPoster::post_external`].
//! `bastyde-app` picks it up in its `AppEvent::External` arm, looks the
...
//! `bastyde-app` calls [`NativeMenuHandle::activate_window`] on focus change so
...
/// `bastyde-app` downcasts to this and routes the [`MenuItemId`] back to the
...
pub trait NativeMenuBackend {
    ...
    fn activate_window(&mut self, window_id: BastydeWindowId);

    /// Forget `window_id`'s menu (window closed).
    fn clear_window(&mut self, window_id: BastydeWindowId);
...
struct NativeMenuState {
    ...
    activations: RefCell<HashMap<BastydeWindowId, WindowActivations>>,
}

/// Per-app native-menu service. Registered in app-state by
/// `BastydeAppBuilder::install_native_menu` (or `.app_state(NativeMenuHandle::new(..))`
...
impl NativeMenuHandle {
    ...
    pub fn activate_window(&self, window_id: BastydeWindowId) {
        self.inner.backend.borrow_mut().activate_window(window_id);
    }

    /// Forget a window's menu + activations (window closed).
    pub fn clear_window(&self, window_id: BastydeWindowId) {
...
impl NativeMenuBackend for NoopNativeMenuBackend {
    ...
    fn activate_window(&mut self, _window_id: BastydeWindowId) {}
    fn clear_window(&mut self, _window_id: BastydeWindowId) {}
...
struct MemoryRecording {
    menus: HashMap<BastydeWindowId, NativeMenuSnapshot>,
    active: Option<BastydeWindowId>,
    deltas: Vec<(MenuItemId, MenuItemDelta)>,
    cleared: Vec<BastydeWindowId>,
}
...
impl MemoryNativeMenuBackend {
    ...
    pub fn menu_for(&self, window_id: BastydeWindowId) -> Option<NativeMenuSnapshot> {
    ...
    pub fn active_window(&self) -> Option<BastydeWindowId> {
    ...
    pub fn cleared(&self) -> Vec<BastydeWindowId> {
