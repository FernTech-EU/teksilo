...
//! targets a per-window `BastydeMenuTarget` object whose action selector reads
//! that tag and posts a [`NativeMenuEventPayload`] through the app's
//! [`AppEventPoster`]. `bastyde-app` routes the payload back into the window's
...
// ============================================================
// BastydeMenuTarget — the action receiver for a window's items
// ============================================================
...
define_class!(
    ...
    #[name = "BastydeMenuTarget"]
    #[ivars = MenuTargetIvars]
    struct BastydeMenuTarget;

    unsafe impl NSObjectProtocol for BastydeMenuTarget {}

    impl BastydeMenuTarget {
        ...
        #[unsafe(method(bastydeMenuAction:))]
...
struct WindowMenu {
    ...
    _target: Retained<BastydeMenuTarget>,
...
pub struct MacOsNativeMenuBackend {
    windows: HashMap<BastydeWindowId, WindowMenu>,
...
impl MacOsNativeMenuBackend {
    ...
    fn apply_main_menu(&self, mtm: MainThreadMarker, window_id: BastydeWindowId) {
...
impl NativeMenuBackend for MacOsNativeMenuBackend {
    fn set_window_menu(
        ...
        let target = BastydeMenuTarget::new(mtm, window_id, poster);
