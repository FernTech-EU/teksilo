...
//! reaches winit instead, whose built-in XDND handling Bastyde does not
...
impl ExternalDndBackend for X11ExternalDndBackend {
    fn attach(
        ...
        let Ok(waker) = Waker::new() else {
            eprintln!("bastyde-platform: X11 DnD disabled (could not create a wake pipe)");
            return Box::new(NoopDndGuard);
        };
        ...
        let handle = std::thread::Builder::new()
            .name(format!("bastyde-xdnd-{}", window_id.raw()))
            .spawn(move || {
                if let Err(err) = run_thread(
                    ...
                    eprintln!("bastyde-platform: X11 drag-and-drop thread stopped: {err}");
                }
            })
            .ok();
...
impl DndThread {
        ...
        match event {
            ...
            Event::PropertyNotify(notify) => {
                if notify.window == self.proxy
                    && notify.atom == atoms.bastyde_transfer
    ...
    fn on_selection_notify(&mut self, property: Atom) {
        ...
        let value = self
            .conn
            .get_property_and_delete(self.proxy, self.conn.atoms().bastyde_transfer)
            .ok()
            .flatten();
