...
pub struct Atoms {
    ...
    pub bastyde_transfer: Atom,
    /// Scratch property for the server-timestamp round trip.
    ///
    /// Deliberately **not** [`Self::bastyde_transfer`]: the timestamp trick
...
impl Atoms {
    fn intern(conn: &RustConnection) -> Result<Self, X11Error> {
        ...
        const NAMES: &[&[u8]] = &[
            ...
            b"TARGETS",
            b"TIMESTAMP",
            b"_BASTYDE_DND_TRANSFER",
            b"_BASTYDE_DND_TIMESTAMP",
            b"text/uri-list",
        ...
        Ok(Self {
            ...
            targets: take(),
            timestamp: take(),
            bastyde_transfer: take(),
            bastyde_timestamp: take(),
            text_uri_list: take(),
    ...
    pub fn fetch_timestamp(&self, window: Window) -> Result<Timestamp, X11Error> {
        ...
        for _ in 0..64 {
            ...
            if let Event::PropertyNotify(ref notify) = event
                ...
                && notify.atom == self.atoms.bastyde_timestamp
            {
