...
//! Install via: `cargo install --path crates/cargo-bastyde-fmt`
//!
//! # Modes
//!
//! - **Format in place** (default): rewrite each `.rs` file whose
//!   `bati!` blocks aren't canonical. Writes are atomic (write-temp +
...
//!
//! Source outside `bati!(...)` blocks is left byte-for-byte unchanged
//! — `cargo fmt` owns Rust formatting.
//!
//! # Usage
//!
//! ```text
//! cargo bastyde-fmt [paths...]
//!
//! Options:
...
fn main() {
    ...
    let cfg = match Config::parse(args) {
        ...
        Err(e) => {
            eprintln!("error: {e}");
            eprintln!("Run `cargo bastyde-fmt --help` for usage.");
            process::exit(2);
        }
    };
    ...
    if cfg.version {
        println!("cargo-bastyde-fmt {}", env!("CARGO_PKG_VERSION"));
        return;
    }
...
fn skip_cargo_subcommand(args: &[String]) -> &[String] {
    // Invoked as `cargo bastyde-fmt`: argv[0]=binary, argv[1]="bastyde-fmt"
    if args.get(1).map(|s| s == "bastyde-fmt").unwrap_or(false) {
        &args[2..]
    } else if args.len() > 1 {
        &args[1..]
    } else {
...
fn print_help() {
    println!(
        r#"cargo bastyde-fmt — formatter for bati! DSL blocks
...
EXAMPLES:
    ...
    cargo bastyde-fmt --check                    # CI mode
    cargo bastyde-fmt examples/widget_catalog    # format one example
    cargo bastyde-fmt src/main.rs                # format one file
"#
    );
}
