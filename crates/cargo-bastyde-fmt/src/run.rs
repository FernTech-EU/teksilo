...
//! by `bastyde-settings`'s flush path. This keeps interrupted runs from
//! leaving truncated source files on disk.

use std::io::Write;
use std::path::Path;

use bastyde_fmt::{FmtConfig, FmtError, format_file};
...
fn process_one(
    ...
    // Cheap pre-filter: if the file has no `bati!` token, skip parsing.
    ...
    if !source.contains("bati!") {
        return Ok(FileResult::NoBatiMacros);
    }
