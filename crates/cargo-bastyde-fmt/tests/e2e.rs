...
fn binary_path() -> std::path::PathBuf {
    ...
    std::path::PathBuf::from(env!("CARGO_BIN_EXE_cargo-bastyde-fmt"))
}
...
fn run(args: &[&str], cwd: &Path) -> Output {
    Command::new(binary_path())
        ...
        .current_dir(cwd)
        .output()
        .expect("failed to spawn cargo-bastyde-fmt")
}
...
fn version_runs() {
    ...
    assert!(out.status.success());
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(stdout.contains("cargo-bastyde-fmt "));
}
...
fn check_fails_on_dirty_file() {
    ...
    assert!(after.contains("bati!(VStack { spacing: 8.0 Button(\"ok\") })"));
}
...
#[test]
fn no_op_on_file_without_bastyde_macro() {
    let tmp = TempDir::new().unwrap();
    let f = tmp.path().join("plain.rs");
    let original = "fn main() {\n    println!(\"no bati here\");\n}\n";
    write(&f, original);
...
fn formats_multiple_files_recursively() {
    ...
    write(
        ...
        "fn x() { bati!(VStack { spacing: 1.0 Button(\"a\") }); }\n",
    );
    write(
        &tmp.path().join("b.rs"),
        "fn y() { bati!(VStack { spacing: 2.0 Button(\"b\") }); }\n",
    );
